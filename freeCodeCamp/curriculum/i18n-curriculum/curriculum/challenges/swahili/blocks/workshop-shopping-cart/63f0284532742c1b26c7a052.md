...
Dodajte svoju metodu poziva na ruti `forEach`. Trebalo bi da primi parametar `btn`. Zatim, u tom pozivu, dodajte slušaoca događaja na `btn`. Slušaoc događaja treba da prati događaj `click`, i trebalo bi da primi drugi poziv sa parametrom `event`. Drugi poziv treba da bude prazan.

# --hints--

Trebalo bi koristiti strelčastu sintaksu da dodate funkciju za povratni poziv u metodu `forEach`, koja prima parametar od `btn`.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(\s*(\(\s*btn\s*\)|btn)\s*=>\s*(?:\{|\s*)/);
```

Trebalo bi dodati listener događaja na svojstvo `btn`.
...
Treba da slušaš događaj `click` na parametru `btn`.
...
Treba dodati praznu callback funkciju za slušalicu događaja. Zapamtite da mu date parametar sa `event`.
