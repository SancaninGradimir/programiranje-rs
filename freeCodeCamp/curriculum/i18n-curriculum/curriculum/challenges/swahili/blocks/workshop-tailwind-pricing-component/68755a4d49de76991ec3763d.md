...
Sad, postavi vrednost `span` unutar elementa `p` klase objekata `text-base`, `font-medium`, i `text-gray-500`.

# --hints--

`span` Unutar vašeg elementa za `p` treba da ima klasu objekata `text-base`.
...
`span` u tvojoj komponenti od `p` mora da ima klasu objekata `font-medium`.
...
`span` unutar elementa `p` treba imati klasu objekata `text-gray-500`.
