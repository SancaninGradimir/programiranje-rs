...
U dijelu izjave za `if`, vratite nešto što kombinuje `defaultPlayer` i `JSON.parse(saved)` zasebno. Uverite se da su sve razdvojene zarezom, jer su to dve različite stvari koje se spajaju u jednu.

# --hints--

Tvoj deo ``if (saved)`` treba da vrati nešto što distribuira ``defaultPlayer`` zajedno sa pohranjenim i analiziranim podacima.
