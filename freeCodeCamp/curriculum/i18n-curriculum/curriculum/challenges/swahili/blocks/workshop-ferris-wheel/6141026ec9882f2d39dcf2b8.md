...
Trebalo bi da kreirate new `50%` selektor u vašem `@keyframes cabins` pravilu.
...
Vaš selektor `50%` bi trebalo da bude između Vašeg selektora `0%` i Vašeg selektora `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

Vaš `50%` selektor bi trebalo da ima svojstvo `background-color` set do `purple`.
