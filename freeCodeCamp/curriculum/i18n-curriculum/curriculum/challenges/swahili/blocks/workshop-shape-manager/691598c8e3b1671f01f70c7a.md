...
Unutar funkcije `handleShapeSelect`, pozovi metodu `.preventDefault()` na `e`, i pozovi funkciju `clearInputFields()` za brisanje polja za unos.

# --hints--

Treba ti da pozoveš `.preventDefault()` na `e`.

```js
assert.match(handleShapeSelect.toString(), /e\.preventDefault\(\);?/)
```

Trebalo bi da pozoveš funkciju `clearInputFields`.
