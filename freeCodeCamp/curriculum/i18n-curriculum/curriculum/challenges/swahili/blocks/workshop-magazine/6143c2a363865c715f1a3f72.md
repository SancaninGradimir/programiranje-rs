...
Unutar elementa `.image-wrapper`, dodelite vašem prvom `img` elementu `src` od `https://cdn.freecodecamp.org/testable-projects-fcc/images/random-quote-machine.png`, i `alt` od `image of the quote machine project`, `class` od `image-1`, atribut `loading` postavljen na `lazy`, atribut `width` od `600`, i atribut `height` od `400`.
