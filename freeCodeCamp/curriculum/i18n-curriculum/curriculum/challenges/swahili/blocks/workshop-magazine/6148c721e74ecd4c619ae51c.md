...
Vaš `.text` element nije CSS Grid, ali možete kreirati kolone unutar elementa bez korišćenja Grida koristeći svojstvo `column-width`.

Dajte svom selektoru `.text` svojstvo `column-width` postavljeno na `25rem`.
