...
Deo ukupnih troškova će uključivati porez, pa je potrebno da ga takođe izračunate. Kreirajte metodu `calculateTaxes` u klasu objekata `ShoppingCart`. Ova metoda bi trebalo da uzme parametar `amount`.

# --hints--

Trebali biste kreirati metodu `calculateTaxes` u klasu objekata `ShoppingCart`.

```js
assert.isFunction(cart.calculateTaxes);
```

Tvoj put ``calculateTaxes`` bi trebalo da uzme parametar ``amount``.
