...
Pošto je interfejs za `PlayerData` definisan, ažurirajte deo `PlayerCard` ili prihvatite parametre `player` koji imaju tip `PlayerData` ili odgovaraju interfejsu `PlayerData`.

Zapamti da možeš preći sledeće parametre:
...
Morate ažurirati `PlayerCard` ili prihvatiti parametre `player` koji imaju tip `PlayerData`.
