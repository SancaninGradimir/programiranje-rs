...
Sada, u fajlu JavaScript, napravi izmene na `happyBtn`.

Koristi `querySelector` za čuvanje reference elementa `#happy-btn` u promenljivim `happyBtn`.

# --hints--

Trebalo bi da imaš izmenu nazvanu `happyBtn`.

```js
assert.exists(happyBtn);
```

Izmene za `happyBtn` moraju da sačuvaju referencu na element `#happy-btn`.
