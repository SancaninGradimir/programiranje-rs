...
Biće bolje imati tekst `label` iznad elemenata forme.

Izaberite sve elemente za `input`, `textarea` i `select`, da zauzmu punu širinu svojih roditeljskih elemenata.

Takođe, dodajte `10px` od `margin` na izabrane elemente. Postavite ostale/druge na `0`.

# --hints--

Trebalo bi da koristite separator sa zareza za odabir elemenata `input`, `textarea` i `select`.
...
Trebalo bi da postaviš vrednost `width` na `100%`.
...
Trebalo bi da postaviš atribut `margin-top` na `10px`.
...
Trebalo bi da postaviš atribut `margin-bottom` na `0`.
...
Treba da postaviš atribut `margin-left` na `0`.
...
Trebalo bi da postaviš vrednost za `margin-right` na `0`.
