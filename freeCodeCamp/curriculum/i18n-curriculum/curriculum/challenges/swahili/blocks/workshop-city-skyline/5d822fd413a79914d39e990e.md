...
Definiši klasu stvari za `bb4-window` `width` od `18%`, `height` od `90%`, i dodaj svoju varijablu za `--window-color4` kao `background-color`.

# --hints--

Trebalo bi mu dati `.bb4-window` `width` od `18%`.
...
Trebalo bi da mu/judiš `.bb4-window` `background-color` od `--window-color4`.
