...
Dodajte još jedan `div` sa `className` vrednošću `form-group`. Unutar njega dodajte element `label` sa `className` vrednošću `label`, `htmlFor` vrednošću `imageUrl` i tekstom `Image URL`. Zatim dodajte element `input` sa `id` vrednošću `imageUrl`, `className` vrednošću `input` i `type` vrednošću `text`.
