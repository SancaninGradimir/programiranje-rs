...
Konačno, dodaj deo `else` koji štampa korak dozvoljen pozivom `make_allowed_move`. Pokušaj da identifikuješ ispravan argument.
...
Trebalo bi da imaš `print(f'Move {i + 1} allowed between {source} and {target}')` unutar bloka `else`.
...
Treba da imaš `make_allowed_move(source, target)` unutar bloka `else`.
