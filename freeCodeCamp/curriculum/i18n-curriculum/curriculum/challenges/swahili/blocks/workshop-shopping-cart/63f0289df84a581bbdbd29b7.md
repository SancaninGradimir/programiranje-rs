...
U povratnom pozivu vašeg listenera događaja (event listener), pozovite metodu `.addItem()` vašeg objekta `cart`, i prosledite `event.target.id`. Zapamtite da će `id` ovde biti niz karaktera (string), pa je morate konvertovati u broj.

Pasiraj red vaših podataka `products` kao drugi argument.

# --hints--

Callback vašeg slušaoca događaja bi trebalo da pozove metodu `.addItem()` vaše objekta `cart`.

```js
assert.match(code, /cart\.addItem\(/);
```

Tvoja putanja `.addItem()` treba da se zove `event.target.id` kao prvi argument. Zapamti da zameniš `id` brojem koristeći `Number()`.

```js
assert.match(code, /cart\.addItem\(\s*Number\(\s*event\.target\.id\s*\)\s*/);
```

Tvoja putanja ``.addItem()`` bi trebalo da bude pozvana sa nizom podataka ``products`` kao drugi argument.
