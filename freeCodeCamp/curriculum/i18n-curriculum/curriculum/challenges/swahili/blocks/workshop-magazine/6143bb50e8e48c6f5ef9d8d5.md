...
Unutar vašeg `aside` elementa, kreirajte dva `img` elementa, jedan `blockquote` element i treći `img` element. Dajte `blockquote` elementu `class` podešeno na `image-quote`.
