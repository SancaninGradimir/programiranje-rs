...
Još unutar funkcije `playSong`, postavite svojstvo `audio.src` jednako `song.src`. Ovo govori audio elementu gde da pronađe audio podatke za odabranu pesmu.

Takođe, postavite svojstvo `audio.title` jednako `song.title`. Ovo govori audio elementu šta da prikaže kao naslov pesme.
