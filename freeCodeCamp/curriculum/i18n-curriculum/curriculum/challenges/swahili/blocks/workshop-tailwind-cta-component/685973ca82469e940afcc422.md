...
Da biste završili stil dugmadi, dodajte klasu utilitarnog elementa `font-semibold` svakom anker elementu `a` kako bi njihov tekst bio podebljan. Takođe, dodajte klasu elementa `rounded` da uklonite oštre uglove sa dugmadi.

# --hints--

Tvoj prvi jangkorni element treba da ima klasu objekata usluge `font-semibold`.
...
Tvoj prvi ankorni element treba da ima klasu objekta usluge `rounded`.
...
Vaša druga karakteristika sidra bi trebalo da ima klasu objekata usluge `font-semibold`.
...
Tvoja druga komponenta nokta treba da ima servisni objekat klasu `rounded`.
