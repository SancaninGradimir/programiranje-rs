...
Sada je vreme da dodamo validaciju u metodu `__init__`. Na početku te metode, kreiraj deklaraciju `if` koja proverava da li je `name` ili `level` nije instanca od `str`.

Unutar deklaracije `if`, ukloni `TypeError` sa porukom `'name' and 'level' attribute must be of type 'str'.`

# --hints--

Treba ti je deklaracija `if` u putanji `__init__`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_function("__init__").find_ifs()[0]`)) })
```

Vaša izjava o `if` bi trebalo da proveri da li je `name` ili `level` primer `str`.
...
Vaša izjava o `if` treba da proizvede `TypeError` sa porukom od `'name' and 'level' attribute must be of type 'str'.`
