...
Trebalo bi da imate funkciju pod nazivom `renderCard`.
...
Vaša funkcija `renderCard` treba da ima četiri parametra.
...
Vaš prvi parametar bi trebalo da bude `drawingType` tipa `string`.
...
Vaš drugi parametar bi trebalo da bude `isReversed` tipa `boolean`.
...
Vaš treći parametar bi trebalo da bude `shortName` tipa `string`.
...
Vaš četvrti parametar bi trebalo da bude `img` tipa `string`.
...
Vaša `renderCard` funkcija treba da ima return tip od `string`.
...
Vaša funkcija `renderCard` bi trebalo da return šablon literala.
