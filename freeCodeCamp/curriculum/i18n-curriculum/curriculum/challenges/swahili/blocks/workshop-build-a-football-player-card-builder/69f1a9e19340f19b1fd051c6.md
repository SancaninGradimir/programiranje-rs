...
Dodati dve komponente od `span` koje imaju `className` od `stat-value` i `stat-label`, kontrolisane za svaki `stat-row`.

# --hints--

Svaki `.stat-row` u `.stat-col` drugog treba da ima `span` sa klasom `stat-value` i `span` sa klasom `stat-label`.
