...
Uklonite element `.fb3a` `width` iz `80%` od `height` iz `15%`. Zatim uklonite element `.fb3b` `width` iz `100%` od `height` iz `35%`.

# --hints--

Trebalo bi mu dati `.fb3a` `width` od `80%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.width, "80%");
```

Trebalo bi mu dati `.fb3a` `height` od `15%`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.height, "15%");
```

Treba da mu daš `.fb3b` `width` od `100%`.
