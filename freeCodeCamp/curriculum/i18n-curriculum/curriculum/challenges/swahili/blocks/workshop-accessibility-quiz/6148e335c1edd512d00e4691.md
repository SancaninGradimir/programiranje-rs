...
Poboljšanje navigacionih linkova bi trebalo da uzrokuje da vidljivo područje skokne direktno na relevantni deo. Međutim, ovaj skok može zbuniti neke korisnike.

Izaberite sve elemente i postavite `scroll-behavior` na `smooth`.
...
Trebalo bi mu dati `*` `scroll-behavior` od `smooth`.
