...
Trebalo bi da dodate još jedan `if` statement sa uslovom `!(cardElement instanceof HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

Trebalo bi da imate `return` unutar vaše `if` deklaracije (ili: *izjave*).
