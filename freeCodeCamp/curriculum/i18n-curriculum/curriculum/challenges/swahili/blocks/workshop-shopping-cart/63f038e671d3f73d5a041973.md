...
Prva stvar koju treba da uradiš je proveriti da li niz podataka `items` je prazan. Ako je prazno, prikaži `alert` korisniku i poruku `Your shopping cart is already empty`, zatim se vrati iz funkcije.

Zapamti da je `0` lažna vrednost, tako da možeš koristiti operator `!` za proveru da li je red podataka prazan.

Nakon prikazivanja upozorenja, izađite iz funkcije da zaustavite izvršavanje.

# --hints--

Treba da kreirate deklaraciju za `if` koja proverava da li je red podataka `items` prazan, koristeći operator negacije.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*!\s*this\s*\.\s*items\s*\.\s*length\s*\)/);
```

Vaša izjava o `if` bi trebalo da prikaže `alert` korisniku i tekst `Your shopping cart is already empty`.
