...
Kreiraj `.hero-title` selektor i daj mu svojstvo `text-align` postavljeno na `center`, svojstvo `color` postavljeno na `orangered` i svojstvo `font-size` postavljeno na `8rem`.
