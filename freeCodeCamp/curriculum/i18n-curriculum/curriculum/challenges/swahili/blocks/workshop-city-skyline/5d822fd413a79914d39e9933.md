...
Dodaj još jedan `repeating-linear-gradient` ispod onoga što si upravo postavio. Postavi smer za `90deg`, koristi boju zgrade od `0%` do `12%` i boju prozora od `12%` do `44%`. Ovo će stvoriti mnogo pravougaono prozora.

# --hints--

Trebalo bi mu/joj dati `.fb5` `repeating-linear-gradient` drugi u svojstvu `background`.
...
Trebalo bi mu dati `repeating-linear-gradient` u smeru `90deg`.
...
Treba da mu/j joj daš kod `repeating-linear-gradient` za prvi bojični deo od `--building-color2` do `0%` do `12%`.
...
Treba ti da mu/datiš `repeating-linear-gradient` od druge boje drugog `--window-color2`, od `12%` do `44%`.
