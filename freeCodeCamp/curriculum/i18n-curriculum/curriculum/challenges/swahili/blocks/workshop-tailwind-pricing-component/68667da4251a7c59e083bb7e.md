...
Za elemente `span` unutar svake stavke liste, dodajte klasu elemenata `text-green-500` i `mr-2` kako bi oznake bile zelene i malo ih pomerile od teksta.

Pia, primeni atribut elementa `aria-hidden` kao što si uradila za elemente `span` prvog paketa cene.

# --hints--

Tvoji elementi od `span` bi trebalo da imaju klasu objekata `text-green-500`.
...
Vaši elementi za `span` trebalo bi da imaju klasu objekata `mr-2`.
...
Vaše komponente za `span` treba da imaju svojstvo elementa `aria-hidden`.
