...
Testni kod `kingBooks` potvrdio je da `findByAuthor` radi ispravno. Pre nego što pređemo na `groupByDecade`, očistite izlaz konzole uklanjanjem toga.

Uklonite liniju `const kingBooks = ...`, `console.log(kingBooks.length)`, i petlju `for` koja prikazuje naslove knjiga i godine.
