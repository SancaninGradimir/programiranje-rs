...
Izvučeni pravougaonik je napravljen na mreži *m* × *n*. Ako *m* i *n* nisu oba ne-praviline brojeve, moguće je da se kroz pravougaonik preseče put duž granica mreže tako da se pravougaonik podeli na dva povezana komada jednakog oblika (nakon rotacije jednog komada za 180°). Svi takvi putevi za mreže 2 × 2 i 4 × 3 prikazani su ispod.
...
Napišite funkciju koja računa broj različitih načina da se iseče pravougaonik dimenzija *m* × *n*.

# --hints--

`cutRectangle` Trebalo bi da bude funkcija.

```js
assert(typeof cutRectangle == 'function');
```

`cutRectangle(2, 2)` Trebalo bi da vrati broj.

```js
assert(typeof cutRectangle(2, 2) == 'number');
```

`cutRectangle(2, 2)` treba da vrati `2`.

```js
assert.equal(cutRectangle(2, 2), 2);
```

`cutRectangle(4, 3)` treba da vrati `9`.
...
`cutRectangle(8, 3)` treba da vrati `53`.

```js
assert.equal(cutRectangle(8, 3), 53);
```

`cutRectangle(7, 4)` treba da vrati `151`.
