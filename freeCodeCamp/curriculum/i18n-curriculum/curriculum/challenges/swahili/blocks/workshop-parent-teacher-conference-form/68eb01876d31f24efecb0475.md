...
Sada kada je HTML struktura gotova, trebalo bi da pređete na CSS stilizovanje.

Otvorite svoj `styles.css` fajl. Prvo, postavite tamnu pozadinsku boju za `body` i svetlu boju za tekst.

Izaberite element `body` i postavite mu njegov `background-color` na `midnightblue`, a `color` na `whitesmoke`.
