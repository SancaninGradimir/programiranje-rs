...
Slika koju ste dodali nije centrirana u odnosu na zaglavlje sekcije `Coffee` iznad nje. Elementi `img` su kao elementi jedne linije.

Da bi slika radila kao elementi na nivou bloka, poput naslova sekcija, kreiraj selektor tipa `img`. Postavi svojstvo `display` na `block`, i koristi svojstva `margin-left` i `margin-right` da ga savršeno centriraju.
...
Trebalo bi da postavite atribut `display` na `block`.
...
Tvoj element za `img` mora imati `display` od `block`.
...
Vaš element za `img` mora da ima `margin-left` i `margin-right` od `auto`.
