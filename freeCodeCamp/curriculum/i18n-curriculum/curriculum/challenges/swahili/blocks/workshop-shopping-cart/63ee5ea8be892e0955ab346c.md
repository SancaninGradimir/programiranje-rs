...
Sada si kreirao klasu `Dessert`, i klasu `ShoppingCart`.

# --hints--

Trebalo bi da objaviš parametar `ShoppingCart`.

```js
assert.match(code, /ShoppingCart/);
```

Trebalo bi da koristiš glavnu reč `class` za deklarisanje klase objekata `ShoppingCart`.

```js
assert.match(code, /class\s+ShoppingCart\s*/);
```

Vaša klasa objekata za `ShoppingCart` treba da bude prazna.
