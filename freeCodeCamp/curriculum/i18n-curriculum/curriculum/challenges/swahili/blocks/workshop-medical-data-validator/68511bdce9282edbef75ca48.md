...
Kao što ste naučili u prethodnoj lekciji, list kompozicija se može koristiti za kreiranje liste koja počinje od postojećeg iterabilnog objekta:
...
Svaki element u listi `medications` treba da bude string. U ovom i sledećem koraku, napisaćete izraz za proveru toga. Koristite operater `and` da dodate još jedan izraz vrednosti ključa `medications`.

Na desnoj strani operatera `and`, koristite sintaksu list kompozicije za kreiranje liste napravljene evaluacijom `isinstance(i, str)` za svaki `i` u `medications`.
