...
Trebalo bi da koristite svojstvo `transform` za rotaciju elemenata.
...
Trebalo bi da date `.back-mountain` a `transform` od `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.
...
Trebalo bi da date `.back-mountain` svojstvo `left`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left);
```

Trebalo bi da date `.back-mountain` svojstvo `left` sa `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.left, '110px');
```

Trebalo bi da date `.back-mountain` svojstvo `top`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.top);
```

Trebalo bi da date `.back-mountain` svojstvo `top` sa `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.
