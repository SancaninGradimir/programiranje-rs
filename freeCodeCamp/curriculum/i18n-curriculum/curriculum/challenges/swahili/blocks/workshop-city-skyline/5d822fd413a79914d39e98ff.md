...
Sledeće, uklonite `width` i `height` iz `.bb2a`, i zamenite `border-left` sa `border-right` koristeći `5vw` umesto `1vw`. Element će sada imati nulti/nula veličine i ivice će se sastajati po sredini.

# --hints--

Morate ukloniti `width` iz `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.width);
```

Trebalo bi da uklonite `height` iz `.bb2a`.

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".bb2a")?.height);
```

Trebalo bi da zamenite `border-left` sa `5vw`.
