...
Izaberite samo elemente od `.inline`, i dodelite im `width` iz `unset`. Ovo će pronaći prvobitno pravilo koje je postavilo sve elemente od `input` na `width: 100%`.

# --hints--

Treba ti da koristiš selektor `.inline`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.inline'));
```

Trebalo bi da im date elemente `.inline` `width` od `unset`.
