...
Takođe je potrebno početi da brišeš HTML. Postavi svojstvo `innerHTML` od `productsContainer` da postane prazan niz karaktera.

# --hints--

U vašoj deklaraciji za `if`, trebate postaviti atribut `innerHTML` od `productsContainer` da vrati prazan niz znakova.
