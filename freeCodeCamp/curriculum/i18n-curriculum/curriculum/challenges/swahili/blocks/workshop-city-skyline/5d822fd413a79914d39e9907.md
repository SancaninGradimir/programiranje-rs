...
Dostavite nove komponente za `div` sa ovim vrednostima od `width` i `height`: `3%` i `10%` za `.bb4a`, `80%` i `5%` za `.bb4b`, i `100%` i `85%` za `.bb4c`.
...
Trebalo bi da mu/jajmo `.bb4a` `height` od `10%`.
...
Trebao bi mu dati `.bb4c` `width` od `100%`.
