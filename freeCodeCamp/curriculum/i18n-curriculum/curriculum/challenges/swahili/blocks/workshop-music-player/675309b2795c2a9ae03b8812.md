...
Primećujete da su umetničke slike albuma u HTML-u i pesme u nizu `userData.songs` promenjene. Zamenili smo originalne pesme kraćim koje možete koristiti za testiranje vaše aplikacije.

Sve osnovne funkcionalnosti su sada na mestu. Jedini problem je što sledeća pesma ne počinje automatski kada se trenutno puštajuća pesma završi. 

Da biste to popravili, možete postaviti slušalicu događaja (event listener) koja će detektovati kada se trenutno puštajuća pesma završi. `"ended"` slušalica događaja je odgovarajuća za ovo. Pokreće se kada reprodukcija medija dođe do kraja.

Dodajte slušalicu događaja na element `audio` koja prati događaj `"ended"` i prosledite referencu na funkciju `playNextSong`. Time je projekat muzičkog plejera završen!
