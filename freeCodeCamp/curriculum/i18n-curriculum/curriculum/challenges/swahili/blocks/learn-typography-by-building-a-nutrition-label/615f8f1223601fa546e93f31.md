...
Kreiraj poslednji element od `p` za vaš deo `.daily-value`. Dodaj tekst `Potassium 235mg 6%`. Poravnaj tekst `6%` desno, i ukloni donju ivicu elemenata `p`.
...
Tvoje novo polje `p` treba da ima svojstvo polja `class` postavljeno na `no-divider`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.classList?.contains('no-divider'));
```

Tvoja nova karakteristika `p` treba da ima tekst `Potassium 235mg 6%`.
...
Vaše polje `span` ne sme da sadrži podebljano slovo.
