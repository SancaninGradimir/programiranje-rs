...
Potreban vam je način da bezbedno dobijete element `select` i zaštitite ga od vrednosti `null`. Za to, kreirajte konstantu `val` sa vrednošću `e.currentTarget` i postavite tip `HTMLSelectElement` koristeći ključnu reč `as`.

Ispod te konstante, koristi deklaraciju `if` da proveri da li je `val` nevažeći odbijanjem. U sekciji `if`, vrati `target value not found`.

# --hints--

Trebalo bi da imate stabilizator nazvan `val`.

```js
assert.match(code, /const\s+val/)
```

Tvoja konstanta za `val` mora imati vrednost `e.currentTarget` tipa `HTMLSelectElement` koristeći ključnu reč `as`.

```js
assert.match(code, /const\s+val\s*=\s*e\.currentTarget\s+as\s+HTMLSelectElement;?/)
```

Trebalo bi da imaš deklaraciju `if` koja proverava da li je `val` null.

```js
assert.match(handleShapeSelect.toString(), /if\s*\((!val|val===null)\)\s*\{?\s*/)
```

Vaša izjava za `if` treba da vrati `target value not found`.
