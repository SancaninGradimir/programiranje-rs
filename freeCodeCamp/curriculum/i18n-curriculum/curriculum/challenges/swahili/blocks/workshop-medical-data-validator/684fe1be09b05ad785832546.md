...
Sada kada vaš regex odgovara slovu `p` praćenom jednim ili više cifara, poslednja stvar koju morate proveriti je da u stringu nema dodatnih karaktera.

Da biste to uradili, možete koristiti drugu funkciju iz modula `re`. Funkcija `fullmatch` vraća objekat podudaranja kada regex obrazac odgovara celom stringu i `None` u suprotnom.
...
Zamenite poziv `search` pozivom `fullmatch`, zadržavajući iste argumente.
