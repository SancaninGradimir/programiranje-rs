...
Unutar prvog `tr`, dodaj element sa `th` koji ima tekst `Loans The outstanding balance on our startup loan.`. Obuhvati taj tekst, osim `Loans `, unutar elementa `span` sa atributom elementa `class` postavljenim na `description`.

Dodajte tri elementa od `td` ispod toga, i dodelite im sledeći tekst u redosledu: `$500`, `$250`, i `$0`. Specifično treći od `td` dodelite atribut elementu `class` postavljenom na `current`.
...
Vaše polje `th` trebalo bi da ima tekst `Loans The outstanding balance on our startup loan.`.
...
Trebalo bi da staviš tekst `The outstanding balance on our startup loan.` unutar elementa `span`.
