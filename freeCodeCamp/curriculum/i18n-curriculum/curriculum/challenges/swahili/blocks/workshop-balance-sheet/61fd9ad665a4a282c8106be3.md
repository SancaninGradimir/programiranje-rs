...
U `thead`, kreiraj element `tr`. Unutar njega, dodaj `td` i tri elementa od `th`. U svakom elementu od `th`, dodaj element `span` sa atributom elementa `class` postavljenim na `sr-only` i sledećim tekstovima, u ovom redosledu: `2019`, `2020`, i `2021`.
...
Svaki tvoj novi element `span` mora imati svojstvo elementa `class` postavljeno na `sr-only`.
