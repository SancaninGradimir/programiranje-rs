...
Trebalo bi da priložite događaj `click` na element `multipleCardsBtn`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Unutar događaja `multipleCardsBtn` callback event-a, trebalo bi da pozovete metodu `multipleCardSelected`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardsBtn[\s\S]*?addEventListener[\s\S]*?\([\s\S]*?multipleCardSelected\s*\(/);
```

Trebalo bi da priložite događaj `click` na element `newReadingBtn`.

```js
assert.match(__helpers.removeJSComments(code), /newReadingBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Unutar `newReadingBtn` događaja povratnog poziva, trebalo bi da pozovete metodu `newReading`.
