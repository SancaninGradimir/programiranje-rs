...
Prateći isti format, izvršite pretragu za vaš unos `number` u poljima `#dinner`, `#snacks` i `#exercise`. Unesite njihove vrednosti u parametre prateći redosled imena poslednje dve faze.
...
Trebalo bi da postaviš vrednost rezultata svog poziva `document.querySelectorAll()` u `dinnerNumberInputs`.
...
Trebalo bi da pozoveš `document.querySelectorAll()` za `#snacks input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#snacks input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Treba da postaviš vrednost rezultata svog poziva od `document.querySelectorAll()` u `snacksNumberInputs`.
...
Vi treba da izvršite poziv od `document.querySelectorAll()` ka `#exercise input[type='number']`.

```js
assert.match(calculateCalories.toString(), /document\.querySelectorAll\(\s*('|"|`)#exercise input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Trebalo bi da postaviš vrednost rezultata tvog poziva `document.querySelectorAll()` u `exerciseNumberInputs`.
