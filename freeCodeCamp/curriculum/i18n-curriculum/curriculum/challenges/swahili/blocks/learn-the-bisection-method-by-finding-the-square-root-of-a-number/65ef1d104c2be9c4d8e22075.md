...
Ako nisu zadati svi uslovi za `if` i `elif`, vrednost `mid` će biti veća od `square_target`. U ovom slučaju, kreirajte polje `else` i postavite vrednost `mid` za `high`.
...
Trebalo bi da staviš vrednost `mid` u `high` u delu `else`.
