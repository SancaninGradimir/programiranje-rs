...
Trebalo bi da deklarišete `private` svojstvo pod nazivom `elements` unutar `Game` class.
...
Tvoj `elements` privatni atribut bi trebalo da ima `singleCardBtn` svojstvo tipizovano as `HTMLElement`.
...
Tvoje `elements` privatno svojstvo trebalo bi da ima `multipleCardsBtn` svojstvo tipa as `HTMLElement`.
...
Vaše `elements` privatno svojstvo bi trebalo da ima `singleCard` svojstvo tipizovano as `HTMLElement`.
