...
Sada, unutar *template* literal funkcije `renderCard`, kreiraj element `div`. Unutar `div`, dodaj element `h2` koji ispisuje `drawingType`, i element `figure` sa klasom `card_container ${isReversed ? "reversed-card" : ""}` i atributom `data-id="${shortName}"`.

# --hints--

Trebalo bi da imate element `div` u tekstualnom literalu funkcije `renderCard`.
...
Trebalo bi da imate element `h2` koji interpolira `drawingType` unutar elementa `div`.
...
Trebalo bi da imate element `figure` unutar elementa `div`.
...
Vaš `figure` element bi trebalo da ima class `card_container` i koristiti ternarni operator za uslovno dodavanje `reversed-card`.
...
Vaš element `figure` bi trebalo da ima atribut `data-id` set do `${shortName}`.
