...
Nakon potvrde da je pronađen važeći čvor, obeležite ga kao posetjen kako bi algoritam nije obrađivao ponovo.

Nakon `if` sa uslovom od `current === -1`, postavi `visited[current]` da bude `true`.

# --hints--

Trebalo bi da staviš `visited[current]` na mesto `true`.
