...
Iterirajte kroz red podataka `POSITIONS` koristeći streličnu funkciju koja prima svaku sekvencu razmak karaktera kao `pos` i vraća element/atribut `option`. Izbor mora imati `key` od `pos` i vrednost takođe `pos`. Takođe, prikažite `pos` kao tekst svakog kreiranog `option`.

Molimo koristite bilo šta kao parametar mape, ne sme biti `pos`.

# --hints--

`select` Vaš prostor treba da ima karakteristike `option` koje su pregledane iz skupa podataka `POSITIONS`.
...
`value` Svaka opcija mora da odgovara prikazanom tekstu i da bude važeće polje.
