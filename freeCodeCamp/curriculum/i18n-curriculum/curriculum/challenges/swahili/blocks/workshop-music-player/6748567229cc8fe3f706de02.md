...
Sada morate raditi na pauziranju trenutno reprodukovane pesme.

Definišite funkciju `pauseSong` koja ne prima parametre. Unutar vaše nove funkcije, da biste sačuvali trenutno vreme pesme kada je pauzirana, postavite `songCurrentTime` objekta `userData` na `currentTime` varijable `audio`.
