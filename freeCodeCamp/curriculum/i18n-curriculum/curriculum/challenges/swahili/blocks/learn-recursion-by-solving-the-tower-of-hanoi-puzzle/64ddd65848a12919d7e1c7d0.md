...
Sada je vreme da se kod premesti iz funkcije `move()` u funkciju `make_allowed_move()`. Premesti kod definisan unutar prvog deklarativa/izjave `if` (osim prvog poziva `print()`) u svoju novu funkciju. Obrati pažnju na indentaciju linije.
Ne zaboravi da ukloniš ključnu reč `pass`.

# --hints--

Treba da premestiš kod koji je strukturiran unutar prve deklaracije od `if` (osim prvog poziva od `print()`) u svoju novu funkciju.
