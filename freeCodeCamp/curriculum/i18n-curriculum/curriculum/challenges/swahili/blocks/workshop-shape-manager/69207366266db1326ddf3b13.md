...
Dodajte događaj `DOMContentLoaded` u glavni dokument i prosledite `initializeApp` kao funkciju povratnog poziva za izvršavanje. Ovo osigurava da se sva inicijalizacija dogodi tek nakon što je DOM potpuno učitan.

# --hints--

Trebalo bi da dodaš događaj `DOMContentLoaded` u glavni `document`.

```js
assert.match(code, /document\.addEventListener\(["']DOMContentLoaded["']/)
```

Treba da se referišete na `initializeApp` kao funkciju za izvršavanje u vašem slušaocu događaja.
