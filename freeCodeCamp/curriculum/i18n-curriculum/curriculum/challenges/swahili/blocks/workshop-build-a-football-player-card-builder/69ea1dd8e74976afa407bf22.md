...
Sledeće, kreiraj tip `Position` sa vrednošću `typeof POSITIONS[number]`. Ovo će otvoriti red podataka za `POSITIONS` u kombinaciju niza svojih pojedinačnih karaktera, čineći da izgleda kao `"GK" | "CB" | "LB" | ...` itd.

# --hints--

Trebaće vam da kreirate naziv vrste koji se zove `Position`.
...
Vaš tip `Position` treba da bude postavljen na `typeof POSITIONS[number]`.
