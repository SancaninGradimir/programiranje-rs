...
Napravi drugi element za `a` unutar drugog elementa od `li`. Dodeli atribut elementu `href` sa vrednošću `#` i tekstom/sadržajem `Widgets`.

# --hints--

Trebali biste kreirati element `a` unutar drugog elementa liste.

```js
assert.lengthOf(document.querySelectorAll('li a'), 2);
```

Vaš element za `a` treba da ima atribut elementa `href` postavljen na `#`.

```js
assert.equal(document.querySelectorAll('a')[1]?.getAttribute('href'), '#');
```

Vaše polje za `a` treba da sadrži tekst od `Widgets`.
