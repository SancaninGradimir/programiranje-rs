...
Ne morate uvek da koristite piksele kada merite veličinu elementa.

Kreiraj nove pravila, `.three`, na weka `width` da budu `91%`.

# --hints--

Trebalo bi da koristiš alat `.three`.
...
Treba da postaviš atribut `width` na `91%`.
...
Vaše polje za `.three` trebalo bi da ima vrednost od `width` za `91%`.
