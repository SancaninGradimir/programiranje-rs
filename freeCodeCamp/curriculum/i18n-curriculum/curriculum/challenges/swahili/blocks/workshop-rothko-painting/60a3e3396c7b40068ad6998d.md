...
Kreiraj pravilo koje cilja `.one` i `.two` na povećanje njihovog efekta od `blur` za piksel 1.

# --hints--

Trebalo bi da imaš listu birača za `.one, .two`.
...
Trebalo bi da postaviš vrednost za `filter` na `blur(1px)`.
...
Vaše polje `.one` mora imati vrednost `filter` od `blur(1px)`.
...
Vaše polje `.two` trebao bi imati vrednost od `filter` koja je `blur(1px)`.
