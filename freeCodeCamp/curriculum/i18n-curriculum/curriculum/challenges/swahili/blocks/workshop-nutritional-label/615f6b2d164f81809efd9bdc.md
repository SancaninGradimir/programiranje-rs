...
Nakon vašeg poslednjeg `.divider` elementa, kreirajte `p` element i dajte mu tekst `Total Fat 8g 10%`. Zatim, obavijte tekst `Total Fat` u jedan `span` element, tekst `10%` u drugi, i date im klasu od `bold`.
