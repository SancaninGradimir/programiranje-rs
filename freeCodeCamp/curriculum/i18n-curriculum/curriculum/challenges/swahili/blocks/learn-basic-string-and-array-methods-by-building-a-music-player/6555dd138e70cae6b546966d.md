...
Morate dobiti pesmu koja se trenutno reprodukuje ili prvu pesmu u plejlisti. Da biste to učinili, kreirajte konstantu `song` i koristite OR konstruktor (`||`) da je postavite kao trenutnu pesmu za `userData`, **ili** prvu pesmu u redovima podataka `userData?.songs`.
