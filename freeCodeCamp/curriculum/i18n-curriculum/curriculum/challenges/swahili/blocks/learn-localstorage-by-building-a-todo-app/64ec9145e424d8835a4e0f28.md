...
Sada ste sačuvali vežbu u redu podataka `taskData`, trebalo bi da prikažete tu vežbu na stranici iteriranjem kroz taj red.

Koristite `forEach()` u `taskData`, a zatim podelite `id`, `title`, `date`, `description` kao parametre. Ne vraćajte ništa još.
...
Trebalo bi da koristiš `{id, title, date, description}` kao razdeljene parametre funkcije za odgovor `forEach()`.
