...
Sada kada znate kako da pronađete sledeću pesmu za reprodukciju, kreirajte funkciju pod nazivom `playNextSong`. Unutar funkcije `playNextSong`, kreirajte izjavu `if` da proverite da li je `currentSong` od `userData` striktno jednako `null`. Ovo će proveriti da li nema trenutne pesme koja se reprodukuje u objektu `userData`.

Ako je uslov tačan, pozovite funkciju `playSong` sa `id` prve pesme u nizu `userData.songs` kao argument.
