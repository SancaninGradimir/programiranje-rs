...
Opcije padajućeg menija trenutno nemaju sadržaj. Da biste mu dodali sadržaj, unesite sledeći tekst za svaki element koji sledi u `option`:
...
Trebalo bi mu dati četvrti deo teksta od `option`, koji je tekstualni sadržaj `freeCodeCamp Forum`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.textContent, 'freeCodeCamp Forum');
```

Trebalo bi mu dati peto polje sa tekstom `Other` za `option`.
