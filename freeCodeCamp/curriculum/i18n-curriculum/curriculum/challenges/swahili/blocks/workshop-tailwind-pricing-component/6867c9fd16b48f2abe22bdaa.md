...
Sledeći dekorativni element je tekst cene, uključujući `span` unutar njega. Primeni klasu `p` na element cene koji ima klase `text-4xl`, `font-bold` i `text-gray-900`.

# --hints--

Tvoj element za `p` mora imati klasu `text-4xl`.
...
Element za `p` mora imati klasu `font-bold`.
...
Element vašeg `p` mora imati klasu `text-gray-900`.
