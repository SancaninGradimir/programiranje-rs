...
Sada koristi određenu boju na `#years`. Podesi tekst za `#fff` i temu za `#0a0a23`.

# --hints--

`#years` vaše treba da imaju svojstvo `color` postavljeno na `#fff`.

```js
assert(new __helpers.CSSHelp(document).getStyle('#years')?.getPropertyValue('color') === 'rgb(255, 255, 255)');
```

`#years` treba da ima svojstvo `background-color` postavljeno na `#0a0a23`.
