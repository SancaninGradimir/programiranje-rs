...
Kreiraj još jedan element `p` ispod vašeg velikog razdvojnika. Elementu `p` dodaj tekst `Vitamin D 2mcg 10%`.

Element `p` sadrži samo tekst; možete procentualnu vrednost obaviti u elementu `span` tako da se smatra zasebnom entitetom od ostatka teksta, i pomeriti je desno.
