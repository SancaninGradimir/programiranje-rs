...
Dovršite setter štampanjem `'{name}' promoted to '{new_level}'.` u vašoj metodi. Zamenite `{name}` i `{new_level}` sa imenom zaposlenog i novim nivoom, po odelima.

# --hints--

Treba da odštampate `'{name}' promoted to '{new_level}'.` gde `{name}` i `{new_level}` treba zameniti sa imenom zaposlenog i novim nivoom, respektivno.
