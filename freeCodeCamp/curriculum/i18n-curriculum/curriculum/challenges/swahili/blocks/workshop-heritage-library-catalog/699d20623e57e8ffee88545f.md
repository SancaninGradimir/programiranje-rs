...
Log `byDecade` je ispunio svoju svrhu. Uklonite `console.log(byDecade)` iz vašeg koda.

Zatim, kako biste olakšali skeniranje unosa kataloga, kreirajte funkciju pod nazivom `renderEntry` koja prima `entry` kao parametar. Ostavite telo funkcije prazno za sada. Ispod funkcije, logujte `renderEntry(catalog[0])`.
