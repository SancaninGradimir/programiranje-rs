...
Kreiraj izraz/deklaraciju `if` koji proverava da li `self` i `other` nisu u istoj klasi i vraća `NotImplemented`.

# --hints--

Trebalo bi da kreirate izraz `if` koji proverava da li `self` i `other` nisu u istoj klasi objekata.
