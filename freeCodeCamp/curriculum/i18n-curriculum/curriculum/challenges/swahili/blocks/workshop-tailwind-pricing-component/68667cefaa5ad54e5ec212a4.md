...
Premesti ga u stavku liste, dodaj klase elementima `mt-6`, `space-y-2`, `text-sm`, na `text-fuchsia-100` na `ul` kako bi se napravio razmak između stavki liste i učinio da izgleda privlačnije.

# --hints--

Tvoj element za `ul` treba da ima klasu objekata `mt-6`.
...
Tvoj element od `ul` treba da ima klasu objekata `space-y-2`.
...
Vaš element za `ul` mora imati klasu objekata `text-sm`.
...
Vaš element `ul` mora imati klasu objekata `text-fuchsia-100`.
