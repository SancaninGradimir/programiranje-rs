...
Unutar `card-stats` div-a, nakon `stat-col` prvog, potrebno je dodati još dve decice.

Prvo, `div` koji se spaja sa `className` od `stat-divider` da prikaže tanku vertikalnu liniju između dva reda podataka, i `div` drugi sa `className` od `stat-col` koji ima tri elementa `div`, svaki sa `className` od `stat-row`.

# --hints--

Trebalo bi da dodate samostalni kod `div` sa `className` nakon `stat-divider` posle prvog `.stat-col`.

```js
assert.exists(document.querySelector('.stat-col + .stat-divider'));
```

Trebalo bi da dodate `div` drugi element sa `className` iz `stat-col` nakon `.stat-divider`.

```js
assert.lengthOf(document.querySelectorAll('.card-stats > .stat-col'), 2);
```

`.stat-col` drugi treba da ima tri elementa od `div`, svaki sa `className` od `stat-row`.
