...
`.card` selektor bi trebalo da ima svojstvo `border-radius` sa vrednošću `10px`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".card")?.getPropertyValue("border-radius"), "10px");
```

Selektor `.card` bi trebalo da ima svojstvo `box-shadow` sa vrednošću `0 4px 8px gray`.
