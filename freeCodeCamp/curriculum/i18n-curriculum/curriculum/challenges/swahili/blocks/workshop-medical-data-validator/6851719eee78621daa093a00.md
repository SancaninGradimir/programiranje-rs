...
Sada kada je vaš `constraints` rečnik završen, promenićete izjavu `return` u `find_invalid_records` da bi vratila listu nevažećih ključeva.

Korišćenjem sintakse razumevanja lista (list comprehension), vratite listu koja procenjuje `key` za svaki `key, value` u `constraints.items()`.
