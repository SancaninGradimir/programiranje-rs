...
Unutar vašeg `.image-wrapper` elementa, dati drugom `img` elementu `src` od `https://cdn.freecodecamp.org/testable-projects-fcc/images/calc.png`, `alt` od `image of a calculator project`, atribut `loading` postavljen na `lazy`, `class` postavljen na `image-2`, atribut `width` postavljen na `400`, i atribut `height` postavljen na `400`.
