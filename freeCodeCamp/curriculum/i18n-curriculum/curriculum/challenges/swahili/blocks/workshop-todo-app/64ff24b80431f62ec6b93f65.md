...
Uklonite red podataka `myTaskArr` sa celog koda `localStorage` jer više neće biti potreban.

# --hints--

Trebalo bi da uklonite `myTaskArr` i sav kod povezan sa `localStorage` koji ste nedavno naučili.
...
Morate ukloniti sve preostale reference za `myTaskArr` bilo gde u kodu.

```js
assert.notMatch(code, /myTaskArr/);
```

Trebalo bi da uklonite bilo koji reference za `localStorage.getItem` za element `"data"`.
