...
Morate dobiti proizvod koji korisnik dodaje u košaricu. Zapamtite da redovi podataka imaju način za `.find()`. U vašoj funkciji od `addItem`, deklarišite parametar `product` i postavite njegovu vrednost za poziv metode `.find()` u red podataka `products`.

Sa pozivom za vraćanje `.find()`, funkcija koja uzima jedan parametar `item`, i vraća kao svojstvo `id` za `item` je potpuno jednako parametru `id` koji je prosleđen za `addItem`.

# --hints--

Trebalo bi da deklariš parametar `product` u svojoj funkciji `addItem`.
...
Treba ti da pozoveš putanju `.find()` u svoj red podataka `products`.
...
Trebalo bi da prosledite funkciju povratne informacije putem `.find()`.
...
Funkcija odgovora treba da uzme jedan parametar.
...
Funkcija povratnog poziva bi trebalo da vrati jer je svojstvo `id` od `item` identično parametru `id` koji je prosleđen za `addItem`.
...
Trebalo bi da postaviš vrednost puta `.find()` u parametar `product`.
