...
Kako biste stilizovali dugme za slanje, možete koristiti selector svojstva elementa, koji bira element na osnovu vrednosti definisanog svojstva elementa. Evo primera:

```css
input[name="password"]
```

Iznad on selektuje elemente `input` sa svojstvom elemenata `name` sa vrednošću `password`.

Sada, koristi selektor atributa elementa za stilizovanje dugmeta za slanje koje ima `display` od `block`, i `width` od `60%`.

# --hints--

Trebalo bi da koristite elementni predložak sa atributom `input[type="submit"]` za dekorisanje dugmeta za slanje.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('input[type="submit"]'));
```

Trebalo bi da mu date dugme za slanje `display` od `block`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.display, 'block');
```

Treba da mu/jajš dugme za slanje `width` od `60%`.
