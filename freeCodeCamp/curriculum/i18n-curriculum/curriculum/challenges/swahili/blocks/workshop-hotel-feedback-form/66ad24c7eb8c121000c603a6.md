...
Da biste učinili polje za čekiranje podrazumevano označeno, možete dodati atribut `checked`.

Evo primera korišćenja atributa `checked`:

```html
<input checked type="checkbox" id="checked" name="checked">
```

Dodajte atribut `checked` polju za čekiranje sa `id` od `"reputation"` da biste ga učinili podrazumevano označenim.
