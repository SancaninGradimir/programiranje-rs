...
U ranijim modulima, naučili ste kako da iterirate kroz `NodeList` na ovaj način:
...
Koristite metod `forEach` da iterirate kroz `tabs`. Za funkciju povratnog poziva (callback), koristite strelicastu funkciju koja prima jedan parametar, `tab`.
