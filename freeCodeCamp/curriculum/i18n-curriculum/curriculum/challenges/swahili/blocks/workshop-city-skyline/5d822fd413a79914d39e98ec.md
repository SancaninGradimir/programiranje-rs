...
Ponovo spoji objekte dodavanjem dva prazna elementa `div` unutar gornjeg i donjeg dela elementa `.foreground-buildings`, i još jedan između `.fb2` i `.fb3`.

# --hints--

Trebalo bi da dodaš dva elementa `div` kao prva potomka `.foreground-buildings`.
...
Trebalo bi da dodate jedan element `div` između `.fb2` i `.fb3`.

```js
assert.equal(document.querySelector('.fb2')?.nextElementSibling, document.querySelector('.fb3')?.previousElementSibling);
```

Treba da dodaš dva elementa `div` kao poslednja deca od `.foreground-buildings`.
