...
Sada moraš da umetneš `product` u red podataka `items` za vagarić. Zapamti da koristiš glavnu reč `this`.

# --hints--

Trebalo bi pozvati metodu `push` u redu podataka `items`.
...
Zapamti da moraš koristiti glavnu reč `this` da dobiješ pristup nizu podataka `items`.
...
Trebalo bi da postaviš varijablu `product` na red podataka `items` koristeći `push`.
