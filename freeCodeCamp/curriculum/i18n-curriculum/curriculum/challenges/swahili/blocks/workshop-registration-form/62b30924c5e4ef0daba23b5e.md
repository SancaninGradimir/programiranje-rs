...
Granica poslednjeg elementa `fieldset` izgleda malo neuredno.

U prethodnim lekcijama, naučili ste kako da radite sa pseudo-klasom `last-of-type` CSS sledeće:

```css
p:last-of-type { }
```

Koristiće se za odabir poslednjeg elementa `p`.

Kreiraj novi selektor koji cilja na poslednji element od `fieldset` i postavi mu vrednost/vrednosti da bude `none`, koristeći `border-bottom`.

# --hints--

Možete koristiti darasa la bandia la `:last-of-type` da izaberete samo poslednji element određenog tipa.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('fieldset:last-of-type'));
```

`fieldset:last-of-type` vaš treba da ima `border-bottom` postavljen kao `none`.
