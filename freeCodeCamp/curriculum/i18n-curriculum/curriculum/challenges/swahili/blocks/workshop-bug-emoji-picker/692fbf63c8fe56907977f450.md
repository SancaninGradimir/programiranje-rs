...
Sledeće, definiši objekt nazvan `bugMap` tipa `Record<string, Bug<string>>` i postavi njegovu vrednost na null za sada.

# --hints--

Trebalo bi da imaš nešto što se zove `bugMap`.
...
`bugMap` treba da ima detalje tipa `Record<string, Bug<string>>`.
...
`bugMap` Trebalo bi da se inicijalizuje u praznu vrijednost.
