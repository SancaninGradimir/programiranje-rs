...
Kreirajte još jedan `p` element nakon vašeg poslednjeg `.divider`, i dajte mu tekst `Total Sugars 12g`. Dodelite tom `p` elementu `class` vrednosti neophodne za uvlačenje i uklanjanje donje ivice. Zatim kreirajte još jedan `.divider` ispod vašeg novog `p` elementa.
