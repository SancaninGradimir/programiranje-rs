...
Mission Control zahteva još jednu funkciju za snimanje sažetka vašeg odreda. Kreiraj praznu funkciju koja se zove `printCrewSummary` i koja prima parametar `crew`.

# --hints--

Treba da kreiraš funkciju nazvanu `printCrewSummary`.

```js
assert.isFunction(printCrewSummary);
```

Funkcija vašeg `printCrewSummary` mora imati parametar `crew`.
