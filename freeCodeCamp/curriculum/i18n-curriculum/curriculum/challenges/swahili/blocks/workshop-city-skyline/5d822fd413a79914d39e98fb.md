...
Promena gradijenta boje često se menja polako od jedne boje do druge. Kada su potrebne oštre promene, promena može biti napravljena naglim zaustavljanjem kao ovde:
...
Ongeza `linear-gradient` za `.bb2b` koji koristi `--building-color2` od `0%` do `6%` i `--window-color2` od `6%` do `9%`.

# --hints--

Trebalo bi da mu daš atribut `.bb2b` sa vrednošću `background`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(`.bb2b`)?.background);
```

Trebalo bi da koristiš `linear-gradient` na `background`.
...
`.bb2b` treba da ima izmene od `linear-gradient` od `--building-color2` u `0%` do `6%`, i `--window-color2` od `6%` do `9%`.
