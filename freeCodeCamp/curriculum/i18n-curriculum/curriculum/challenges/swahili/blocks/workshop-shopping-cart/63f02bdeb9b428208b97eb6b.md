...
Tvoja funkcija za `calculateTaxes` treba da vrati vrednost od `taxRate` (podeljeno sa 100, konvertovano u procenat) pomnoženu sa parametrom `amount`.

Za jasnoću, obuhvatite izračun `taxRate / 100` u obične zagrade.
...
Trebalo bi da obuhvatite izračun `this.taxRate / 100` u obične zagrade.
...
Treba da pomnožite parametar od ``amount`` i ``taxRate``, podeljen sa 100, u vašem načinu za ``calculateTaxes``.
...
Tvoja funkcija za `calculateTaxes` treba da vrati vrednost `taxRate` (podeljeno sa 100, konvertovano u procenat) pomnoženu sa parametrom `amount`.
