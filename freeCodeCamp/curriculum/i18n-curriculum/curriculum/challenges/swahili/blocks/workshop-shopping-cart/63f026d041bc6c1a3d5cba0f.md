...
Morate proći kroz dugmad sadržana u vašoj varijabli ``addToCartBtns``. Međutim, ``.getElementsByClassName()`` vraća Collection, koja nema metodu za ``forEach``.

Koristi operater za spread na varijabli `addToCartBtns` da je pretvoriš u niz podataka. Zatim, koristi metodu `forEach` da iteriraš kroz niz podataka. Još ne pokrećaj funkciju povratne pozive.

# --hints--

Trebalo bi da koristiš spread operater na varijabli `addToCartBtns`.

```js
assert.match(code, /\.\.\.addToCartBtns/);
```

Treba da proslediš varijablu `addToCartBtns` kao red podataka.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]/);
```

Trebalo bi da koristiš metodu `forEach` na liniji podataka koju si kreirao/kreirala.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(/);
```

Ne treba da proširite funkciju protoka na putanju `forEach`.
