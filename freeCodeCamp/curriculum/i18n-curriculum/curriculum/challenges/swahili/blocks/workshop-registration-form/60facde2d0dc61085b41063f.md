...
Funkcija ``textarea`` radi kao funkcija ``input`` tipa ``text``, ali ima dodatnu prednost pri primanju višelinijskog teksta, zajedno sa početnim brojem linija i redovima teksta.

Korisnici će moći da se registruju i kreiraju profil. Dodajte `label` sa tekstom `Provide a bio:` na kraju `fieldset`. Dodajte element `textarea` unutar elementa `label`. Zapamtite da `textarea` zahteva oznaku za zatvaranje (closing tag).

# --hints--

Treba da dodate element za `label` na kraju trećeg `fieldset`, nakon postojećih komponenti od `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)'));
```

Trebalo bi mu dati tekst od `label` za `Provide a bio:`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label:nth-child(4)')?.innerText, /Provide a bio/);
```

Treba da postaviš unutar elementa `textarea` unutar `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea'));
```

Trebalo bi mu dati atribut `textarea` za otvaranje i zatvaranje.
