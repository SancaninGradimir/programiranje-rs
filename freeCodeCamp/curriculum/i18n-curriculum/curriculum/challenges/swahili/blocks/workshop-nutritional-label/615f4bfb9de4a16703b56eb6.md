...
Sada možete dodati horizontalni razmak koristeći `flex`. U vašem `p` selektoru, dodajte svojstvo `display` postavljeno na `flex` i svojstvo `justify-content` postavljeno na `space-between`.
