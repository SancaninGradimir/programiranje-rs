...
Dodavanje padajućeg menija na formular je lako korišćenjem elementa ``select``. Element ``select`` je kontejner za grupu elemenata ``option``, a element ``option`` funkcioniše kao labela za svaku opciju padajućeg menija. Oba elementa zahtevaju zatvarajuće labeli.

Počnite dodavanjem elementa `select` ispod poslednjeg elementa `label`. Zatim postavite unutar pet elemenata `option` unutar elementa `select`.

# --hints--

Treba da dodate element `select` u treći element od `fieldset`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > select'));
```

Treba da postavite u pet elemenata od `option` unutar elementa sa `select`.
