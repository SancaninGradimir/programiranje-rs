...
Zapamtite da svojstvo `grid-column` određuje od koje kolone element počinje i na kojoj završava. Može biti trenutaka kada niste sigurni koliko će vaš grid imati kolona, ali želite da se element zaustavi na poslednjoj koloni. Da biste to uradili, možete koristiti `-1` za krajnju kolonu.

Kreirajte selektor `.hero` i dodelite mu svojstvo `grid-column`, postavljeno na `1 / -1`. Ovo će reći elementu da se proteže preko cele širine grida.
