...
Povećaj polje `else` koje postavlja vrednost broja `0` za brzinu igrača `x`.

# --hints--

Trebalo bi da dodaš deklaraciju `else` u svoju funkciju `animate`.

```js
assert.match(animate.toString(), /\}\s*else\s*\{/);
```

Vaša izjava za `else` mora da dođe nakon vaše izjave za `else if`.
...
Trebalo bi da uključite deo `else` koji postavlja vrednost broja `0` za brzinu `x` igrača.
