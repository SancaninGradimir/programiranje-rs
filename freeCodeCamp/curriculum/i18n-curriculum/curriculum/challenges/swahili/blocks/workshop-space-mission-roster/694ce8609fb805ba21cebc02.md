...
Kontrolni centar zahteva način za zamenu pozicija dva člana odreda u spisku. Kreiraj praznu funkciju pod nazivom `swapCrewMembers` sa tri parametra:

- `crew`: redovi podataka o objektima za svemirskog čoveka

- `fromIndex`: Model prvog transformativnog čoveka iz prostora

- `toIndex`: Model čoveka iz druge dimenzije transformacije

# --hints--

Treba da kreiraš funkciju koja se zove `swapCrewMembers`.

```js
assert.isFunction(swapCrewMembers);
```

Vaša funkcija za `swapCrewMembers` mora imati parametre `crew`, `fromIndex`, i `toIndex`.
