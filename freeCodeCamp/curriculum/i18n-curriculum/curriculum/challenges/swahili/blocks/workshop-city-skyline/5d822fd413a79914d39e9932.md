...
Nastavi ka sledećoj zgradi! To je zelena koja je ispred. Promeni boju svoje zgrade od `repeating-linear-gradient` na `0%` do `5%`, i `transparent` od `5%` do `10%`.

# --hints--

Trebalo bi mu dati `.fb5` atribut od `background`.
...
Trebalo bi mu dati boju `repeating-linear-gradient` od `--building-color2` do `0%` do `5%`.
...
Trebalo bi mu dati boju sekundarne boje od `5%` do `10%` na `transparent` iz `repeating-linear-gradient`.
