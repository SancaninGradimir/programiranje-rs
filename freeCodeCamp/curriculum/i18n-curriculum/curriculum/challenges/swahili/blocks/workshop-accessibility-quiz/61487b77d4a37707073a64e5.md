...
Kada je širina ekrana mala, `h1` ne prikazuje svoj tekstualni sadržaj kako treba. Centrirajte tekst elementa `h1`.

Zatim, ukloni unutrašnji razmak (padding) za `main` kako bi zaglavlje sekcije `Student Info` bilo potpuno vidljivo.
...
Treba da dodate selektor za `main` ili ciljati na element sa `main`.
...
Treba da promenite samo vrednost `padding-top`.
