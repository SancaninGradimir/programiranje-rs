...
U klasi `Spider`, implementiraj metodu `render`, sa prefiksom `override`, kako bi se ispravio atribut `innerText` od `emojiElement` na vrednost atributa `emoji`.

# --hints--

Tvoja klasa za `Spider` mora imati metodu za `render`.
...
Tvoja putanja za `Spider` `render` treba da koristi prefiks `override`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?override\s+render/);
```

Tvoj put od `Spider` `render` treba da postavi `this.emojiElement.innerText` na `this.emoji`.
