...
Dohvati element sa `select` koji ima identifikator `species` iz DOM i sačuvaj ga u varijablu nazvanu `selectElement`. Koristi non-null `!` operator za validaciju da potvrdi da element nije `null`.

# --hints--

Treba ti konstanta nazvana `selectElement`.
...
`selectElement` mora zahtevati funkcionalnost sa identifikatorom `species` koristeći `document.querySelector` i argument tipa `HTMLSelectElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLSelectElement\s*>\s*\(\s*['"]#species['"]\s*\)/);
```

`selectElement` tvoj mora koristiti ne-null autentikacioni drajver (`!`).
