...
Promeni pozadinsku boju `footer` u `#2a2a40` i koristi _Flexbox_ da centrirate tekst horizontalno.
...
Trebalo bi mu dati `footer` `display` od `flex`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('footer')?.display, 'flex');
```

Trebalo bi mu dati `footer` `justify-content` od `center`.
