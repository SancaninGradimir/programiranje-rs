...
Unutar ``card`` div-a, dodaj ``div`` koji ima ``className`` od ``card-header``. Ovaj deo će sačuvati nivo igrača, poziciju, oznaku rangiranja (tier badge), i ime kluba.

# --hints--

Trebalo bi dodati `div` koji ima `className` od `card-header` unutar `div` koji ima `className` od `.card`.
