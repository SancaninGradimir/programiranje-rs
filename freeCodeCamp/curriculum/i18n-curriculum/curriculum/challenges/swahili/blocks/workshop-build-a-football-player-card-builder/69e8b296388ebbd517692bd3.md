...
Unutar `stats-grid`, dodaj dva elementa od `div` svaki sa `className` od `form-group`.

Prvo, dodaj `label` koji ima `className` od `label`, `htmlFor` od `pac` i tekst od `PAC`, zatim unos koji ima `id` od `pac`, `className` od `input` i `type` od `number`.

Za drugi deo, dodaj `label` koji ima `htmlFor` od `sho` u tekst `SHO`, zatim unos sa `id` od `sho` i `type` od `number`.

# --hints--

Trebalo bi da ima dva elementa od `div`, svaki sa `className` od `form-group` unutar `div` od `className` od `stats-grid`.
...
Tvoj prvi element od `div` koji ima `className` od `form-group` treba da ima `label` sa svojstvom elementa `htmlFor` od `pac`, `className` od `label`, i tekst za `PAC`.
...
Vaš prvi element za `div` sa `className` od `form-group` mora imati `input` sa `className` od `input`, `id` od `pac` i `type` od `number`.
...
Tvoj drugi element od `div` koji ima `className` od `form-group` treba da sadrži `label` sa svojstvom elementa `htmlFor` od `sho`, `className` od `label`, i tekst za `SHO`.
...
Vaš drugi element od `div` koji bi trebalo da ima `className` sa `form-group`, mora imati `input` koji ima `id` od `sho`, `className` od `input` i `type` od `number`.
