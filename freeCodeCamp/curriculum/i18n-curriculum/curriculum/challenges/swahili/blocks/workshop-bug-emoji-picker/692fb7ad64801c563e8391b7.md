...
Kreiraj podklasu objekata `Spider` koja proširuje klasu objekata `Bug` i sa opštim tipom parametrom koji je `string`.

# --hints--

Trebalo bi da imaš klasu objekata nazvanu `Spider`.
...
Vaša klasa objekata `Spider` treba da doda `Bug`.
...
Vaša klasa objekata `Spider` treba da doda `Bug` i zapis tipa `string`.
