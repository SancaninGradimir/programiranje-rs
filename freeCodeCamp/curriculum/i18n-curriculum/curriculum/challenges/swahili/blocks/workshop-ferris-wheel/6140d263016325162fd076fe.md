...
Trebalo bi da imate selektor `.cabin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.cabin'));
```

Vaš `.cabin` selector treba da ima `background-color` svojstvo set do `red`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cabin')?.backgroundColor, 'red');
```

Vaš `.cabin` selektor bi trebalo da ima svojstvo `width` set do `20%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.cabin')?.width, '20%');
```

Vaš `.cabin` selektor bi trebalo da ima `height` svojstvo set do `20%`.
