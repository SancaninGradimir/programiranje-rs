...
Nakon dobijanja stvarnog modela, kreiraj informaciju za if da bi se proverilo/la da li je `actual_index` manje od nule ili veće ili jednako dužini liste `self.emails`. Ako jeste, objavi poruku `Invalid email number.\n` i koristi `return` iz puta.

# --hints--

Treba da kreirate izveštaj za if ili proverite da li `actual_index` je manje od nule, ili veće ili jednako dužini liste `self.emails`.
...
Ako je šablon nevažeći, treba da objavite poruku `Invalid email number.\n` i koristite `return` iz puta.
