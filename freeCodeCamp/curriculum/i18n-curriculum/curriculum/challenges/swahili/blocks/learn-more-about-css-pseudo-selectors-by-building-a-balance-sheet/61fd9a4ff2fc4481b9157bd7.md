...
Za vašu treću tabelu, dodajte `caption` sa tekstom `Net Worth`, i pripremite zaglavlje tabele i telo tabele.

# --hints--

Treći vaš element za `table` mora imati komponentu `caption`.

```js
assert(document.querySelectorAll('table')?.[2]?.children?.[0]?.localName === 'caption');
```

Vaš element za `caption` bi trebalo da ima tekst `Net Worth`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('caption')?.textContent === 'Net Worth');
```

Vaš treći element za `table` mora imati element za `thead`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('thead'));
```

Vaša treća komponenta od `table` treba da ima element `tbody`.

```js
assert(document.querySelectorAll('table')?.[2]?.querySelector('tbody'));
```

Vaš element `thead` treba da bude direktno ispod vašeg elementa `caption`.
