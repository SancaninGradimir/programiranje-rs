...
Menja svojstva veze kada mišem pređe preko nje koristeći <dfn>pseudo-selektori</dfn> koji izgleda kao `a:hover { propertyName: propertyValue; }`.

Promeni boju veze na žičici `Visit our website` u `brown` kada korisnik pređe preko nje.
...
`a:hover` treba da ima `color` od `brown`.
