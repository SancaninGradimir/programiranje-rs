...
Kreiraj još jedan `@media` upit za `only screen` sa `max-width` od `600px`. Unutar toga, kreiraj pravilo `.text-with-images` i dodeli mu svojstvo `grid-template-columns` od `1fr`.

Ovo će svesti vaš donji tekstualni prostor na jednu kolonu na manjim ekranima.
