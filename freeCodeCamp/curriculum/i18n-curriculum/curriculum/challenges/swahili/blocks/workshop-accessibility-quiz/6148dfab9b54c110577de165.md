...
Omogući dugme za predaju u stilu freeCodeCamp, sa sledećim parametrima CSS:
...
Trebalo bi mu dati `button` `padding` od `15px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('button')?.padding, '15px');
```

Trebalo bi mu dati `button` `font-size` od `1.438rem`.
