...
Ukloni atribut sa vrednosti `background-color` iz `.fb3` i stavi ga u `.fb3a` i `.fb3b`.
...
Treba da mu dodeliš atribut `.fb3a` sa vrednošću `background-color`, koja je `--building-color1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

Trebaš mu dati atribut `.fb3b` sa vrednošću `background-color`, koja je `--building-color1`.
