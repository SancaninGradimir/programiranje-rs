...
Sada morate da se pripremite za uređivanje prikaza i novog proizvoda koji je korisnik dodao. Obavestite o promeni `currentProductCount`, i postavite mu vrednost na atribut objekta `totalCountPerProduct` koji odgovara `id` od `product`.

# --hints--

Treba da objaviš promene u `currentProductCount` unutar svoje funkcije `addItem`.
...
Treba ti da koristiš `const` za objavu `currentProductCount`.
...
Trebalo bi da postaviš vrednost `totalCountPerProduct[product.id]` na `currentProductCount`.
