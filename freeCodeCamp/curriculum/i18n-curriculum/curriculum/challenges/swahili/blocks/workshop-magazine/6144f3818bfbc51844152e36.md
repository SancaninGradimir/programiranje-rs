...
Koristite funkciju `minmax` da učinite vaše kolone responsivnim na bilo kom uređaju. Funkcija `minmax` prima dva argumenta: prvi je minimalna vrednost, a drugi maksimalna. Ove vrednosti mogu biti dužina, procenat, `fr` ili čak ključna reč poput `max-content`.

Umotajte svaku od već definisanih vrednosti svojstva `grid-template-columns` u funkciju `minmax`, koristeći svaku vrednost kao drugi argument. Prvi argument treba da bude `2rem`, `min-content` i `2rem` odgovarajuće.
