...
Sada kada imate listu vaših proizvoda, možete koristiti JavaScript da ih unesete u HTML. Na ovaj način, kada prestanete da dodajate proizvode, HTML će se automatski promeniti i prikazati ih.

Počnite pozivanjem metode `.forEach` vašeg reda podataka `products`. Koristite strelicastu sintaksu za kreiranje prazne funkcije povratnog poziva.

# --hints--

Trebalo bi da pozoveš putanju `.forEach` svog niza podataka `products`.

```js
assert.match(code, /products\.forEach\(/);
```

Trebalo bi da koristiš sintaksu strelice za kreiranje prazne funkcije (ili apstraktnog metoda).

```js
assert.match(code, /\(\s*\)\s*=>\s*\{\s*\}/)
```

Morate da prosledite svoj prazan callback funkciju putem `.forEach`.
