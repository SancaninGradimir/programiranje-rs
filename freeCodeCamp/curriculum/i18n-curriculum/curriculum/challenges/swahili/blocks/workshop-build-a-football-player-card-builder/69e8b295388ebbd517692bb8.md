...
Ispod elementa sa `div` koji ima `className` od `card-tier-badge`, dodaj `div` koji ima `className` od `card-club` i tekst od `Santos FC`.

# --hints--

Trebalo bi dodati `div` koji ima `className` od `card-club` ispod `.card-tier-badge`.

```js
assert.exists(document.querySelector('.card-tier-badge + .card-club'));
```

Element za `.card-club` mora imati tekst od `Santos FC`.
