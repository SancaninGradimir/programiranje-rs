...
Unutar funkcije `playNextSong`, kreiraj izjavu `if` koja proverava da li je `currentSong` od `userData` potpuno jednak sa `null`. Ovo će proveriti da li se trenutno ne reprodukuje pesma u objektu `userData`.
