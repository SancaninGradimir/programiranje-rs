...
Unutar petlje, proveriti da li autor trenutnog unosa (konvertovan u mala slova) uključuje `searchTerm`. Metoda `.includes()` vraća `true` ako niz sadrži drugi niz kao podstring. Ako je uslov tačan, dodati unos u `results`. Nakon petlje, vratiti `results`.
