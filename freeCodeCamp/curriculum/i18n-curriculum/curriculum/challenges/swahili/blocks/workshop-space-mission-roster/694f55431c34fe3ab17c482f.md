...
Komunikacioni centar je zatražio novu funkciju za podelu odreda na delove različitih veličina. Kreirajte praznu funkciju pod nazivom `chunkCrew` koja prima dva parametra, `crew` i `size`.

# --hints--

Trebalo bi da kreirate funkciju nazvanu `chunkCrew`.

```js
assert.isFunction(chunkCrew);
```

Tvoja funkcija za `chunkCrew` treba da ima parametre `crew` i `size`.
