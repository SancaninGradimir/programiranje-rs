...
U vašoj novoj funkciji, deklariši parametar `calories` i tip vrednosti `0`. Koristi `let` za deklarisanje, jer ćeš kasnije menjati njegovu vrednost.
...
Vaš parametar za `calories` treba da dobije vrednost `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Trebalo bi da objaviš svoj parametar `calories` za `let`.
