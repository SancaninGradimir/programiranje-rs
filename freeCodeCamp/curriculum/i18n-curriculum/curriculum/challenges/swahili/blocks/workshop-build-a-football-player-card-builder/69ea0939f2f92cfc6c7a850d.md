...
Sledeći, dodaj poziv `useEffect` koji se izvršava svaki put kada se stanje `player` razlikuje. Unutra, koristi strelicastu funkciju i dodaj `try..catch` sa `error` kao parametar za segment `catch`.

# --hints--

Trebalo bi da pozoveš `useEffect` unutar `FootballPlayerCard`.
...
`useEffect` trebalo bi da ima listu zavisnosti `[player]`.

```js
assert.match(__helpers.removeJSComments(code), /useEffect\s*\([\s\S]*,\s*\[\s*player\s*\]\s*\)/);
```

Povratni poziv za `useEffect` mora imati segment `try...catch` koji ima `error` kao parametar za segment `catch`.
