...
Acha `f(k, n)` = $e^\frac{k}{n} - 1$, za sve neujemne celobrojke `k`.

Iznenada, `f(6, 200) + f(75, 200) + f(89, 200) + f(226, 200)` = 3.1415926… ≈ π.

Zapravo, je najbolja procena π od tipa `f(a, 200) + f(b, 200) + f(c, 200) + f(d, 200)`.

Izraz `almostPi(n)` = a<sup>2</sup> + b<sup>2</sup> + c<sup>2</sup> + d<sup>2</sup> za a, b, c, d koji minimizuju greške: $\lvert f(a,n) + f(b,n) + f(c,n) + f(d,n) - \Pi\rvert$

Umepatiwa `almostPi(200)` = 6<sup>2</sup> + 75<sup>2</sup> + 89<sup>2</sup> + 226<sup>2</sup> = 64658.

# --hints--

`almostPi` Trebalo bi da bude funkcija.

```js
assert.isFunction(almostPi);
```

`almostPi` Trebalo bi da vrati broj.

```js
assert.strictEqual(typeof almostPi(10), 'number');
```

`almostPi(29)` mora vratiti `1208`.

```js
assert.strictEqual(almostPi(29), 1208);
```

`almostPi(50)` trebalo bi da vrati `4152`.

```js
assert.strictEqual(almostPi(50), 4152);
```

`almostPi(200)` treba da vrati `64658`.
