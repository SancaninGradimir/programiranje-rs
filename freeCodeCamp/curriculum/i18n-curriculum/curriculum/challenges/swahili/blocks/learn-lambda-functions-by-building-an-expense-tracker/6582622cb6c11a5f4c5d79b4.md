...
Još unutar sekcije `elif`, izvrši rutinu `expenses_from_category` za poziv na `print_expenses`.

# --hints--

Trebalo bi da pozoveš `print_expenses()` kada prosleđuješ `expenses_from_category` kao argument u svoj deo `elif`.
...
Vaš poziv za `print_expenses()` mora da dođe nakon parametra `expenses_from_category`.
