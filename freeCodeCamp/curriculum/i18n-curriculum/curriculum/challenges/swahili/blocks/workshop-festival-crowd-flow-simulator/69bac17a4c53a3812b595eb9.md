...
Trebalo bi da pozovete `initializeThroughput()` sa `gates` i dodelite rezultat promenljivoj nazvanoj `throughputSummary`.
...
Vaša funkcija `simulateFestival` bi trebalo da pozove funkciju `initializeThroughput()` sa `gates`.
