...
title: 'Vežba 12: Broj koji se veoma lako deli sa tri'
challengeType: 1
forumTopicId: 301746
...
Niz trougličnih brojeva stvara se sabiranjem prirodnih brojeva. Stoga, trouglični broj za broj 7 biće 1 + 2 + 3 + 4 + 5 + 6 + 7 = 28. Prvih deset elemenata će biti:

<div style='text-align: center;'>1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...</div>

Ispisujemo vrednosti prvih sedam trokutačnih brojeva:
...
Možemo videti da je 28 prvi trokutasti broj koji ima više od pet delitelja.

Koja je vrednost prvog trougličitog broja koji je veći od `n`?

# --hints--

`divisibleTriangleNumber(5)` Trebalo bi da vrati broj.

```js
assert.isNumber(divisibleTriangleNumber(5));
```

`divisibleTriangleNumber(5)` treba vratiti 28.

```js
assert.strictEqual(divisibleTriangleNumber(5), 28);
```

`divisibleTriangleNumber(23)` treba da vrati 630.

```js
assert.strictEqual(divisibleTriangleNumber(23), 630);
```

`divisibleTriangleNumber(167)` treba vratiti 1385280.

```js
assert.strictEqual(divisibleTriangleNumber(167), 1385280);
```

`divisibleTriangleNumber(374)` trebalo bi da vrati 17907120.

```js
assert.strictEqual(divisibleTriangleNumber(374), 17907120);
```

`divisibleTriangleNumber(500)` trebalo bi da vrati 76576500.
