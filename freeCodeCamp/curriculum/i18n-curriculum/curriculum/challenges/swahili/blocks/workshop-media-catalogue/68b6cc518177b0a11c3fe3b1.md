...
Pre bloka `try`, kreirajte instancu od `MediaCatalogue` i dodelite je promenljivoj pod imenom `catalogue`.

Zatim, koristite metodu `add` za dodavanje `movie1` u katalog. Na kraju ispišite `catalogue` na konzolu.
