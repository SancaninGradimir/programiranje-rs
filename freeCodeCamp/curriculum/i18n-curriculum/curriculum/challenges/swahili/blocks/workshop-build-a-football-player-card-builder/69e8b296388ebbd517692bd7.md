...
Zamenite `input` element elementom `select` sa `id` vrednošću `position`, `className` vrednošću `input` i `value` vrednošću `player.position`.

# --hints--

Treba da zamenite `input` za poziciju elementom `select` sa `id` vrednošću `position` i `className` vrednošću `input`.
