...
Dodajte svojstvo `value` elementu SHO `input` i postavite ga na `player.sho`. Zatim postavite njegovo svojstvo `onChange` na streličastu funkciju koja prima `e` kao argument i poziva `setPlayer` šireći `player` i postavljajući `sho` na `Number(e.target.value)`.

# --hints--

Vaš SHO `input` treba da ima `value` jednako statistici `sho` trenutnog igrača.
...
Vaš SHO `input` treba da ima obrađivač događaja `onChange` koji poziva `setPlayer` šireći `player` i postavljajući `sho` na `Number(e.target.value)`.
