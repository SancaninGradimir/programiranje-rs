...
Za konačni `input` i `label` unutar ovog `fieldset`, dodaćete čekboks `input` sa atributima `id` i `value` podešenim na `"price"`, i atributom `name` podešenim na `"choice"`.

Zatim, element `label` sa tekstom `Price` i atributom `for` podešenim na `"price"`.

Sada možete testirati svoj `form` biranjem različitih čekboksova.
