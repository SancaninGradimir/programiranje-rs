...
Sada ste sačuvali vežbu u podatkovnom nizu za `taskData`, treba da prikaže tu vežbu na stranici prelistavanjem kroz taj niz.

Koristi `forEach()` u `taskData`, a zatim podeli `id`, `title`, `date`, `description` kao parametre. Ne vraćaj ništa još.

# --hints--

Trebalo bi da koristiš `forEach()` na `taskData`.

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

Trebalo bi da koristite `{id, title, date, description}` kao segmentovane parametre za funkciju povratnog poziva `forEach()`.
