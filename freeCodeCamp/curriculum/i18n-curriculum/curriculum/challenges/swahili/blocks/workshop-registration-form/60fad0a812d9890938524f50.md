...
Da bi učenicima dali ideju šta da stave u svoj profil, koristi se atribut elementa `placeholder`. `placeholder` prihvata tekstualnu vrednost, koja se prikazuje dok korisnik ne počne da kuca.

Postavi atribut elementa `placeholder` sa vrednošću `I like coding on the beach...` na `textarea`.

# --hints--

Trebalo bi mu dati osobinu elementa `textarea` `placeholder`.
