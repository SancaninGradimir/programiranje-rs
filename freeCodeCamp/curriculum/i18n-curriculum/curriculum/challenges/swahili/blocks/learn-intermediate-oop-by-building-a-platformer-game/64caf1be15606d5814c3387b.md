...
Dodaj još jednu izjavu sa vrednošću tačno ili netačno koja proverava da li je pozicija igrača za `y`, zajedno sa visinom igrača, veća ili jednaka poziciji platforme za `y`.

Ispod njega, dodajte drugi izraz za stvarnu ili lažnu vrednost koji proverava da li je prostor igrača `y` manji ili jednak ukupnom prostoru platforme `y` zajedno sa dužinom platforme.

# --hints--

Ne možete menjati red `platformDetectionRules` koji postoji i sadržaj unutar njega.
...
Trebalo bi da ima vrednost tačno ili netačno koja proverava da li je prostor igrača `y` i visina igrača veći ili jednaki prostoru platforme `y`.
...
Treba da imaš vrednost zaista ili lažna koju proverava da li je prostor igrača `y` manji ili jednak ukupnom prostoru platforme `y` plus dužina platforme.
