...
Postavite svoju deklaraciju `password` za promenu sledećeg ciklusa `for` unutar ciklusa `while`. Koristite `True` kao uslov za vaš novi ciklus.

# --hints--

Trebalo bi da kreirate ciklus `while True` koji okružuje vašu deklaraciju `password` koja se nalazi sa krugom `for`.
