...
Za vaš stvarni matematički izraz, uklanjanjem `undefined`, morate ispraviti `textContent` od `currentProductCountSpan` ili da bude `currentProductCount` praćen sa `x`. Koristite template literal za to.

# --hints--

Trebalo bi da uklonite `undefined` iz vašeg matematičkog izraza.
...
Trebalo bi da dobiješ kvalifikaciju `textContent` od `currentProductCountSpan`.
...
Trebalo bi da koristite sintaksu *template literal*-a za ispravljanje `textContent` ili ako je `${currentProductCount}x`.
