...
Ako pažljivo proverite, videćete da trenutno puštena pesma nije istaknuta u plejlisti, pa ne znate koju pesmu se zaista pušta. Možete ovo popraviti kreiranjem funkcije koja ističe bilo koju pesmu koja se pušta.

Kreirajte funkciju pod nazivom `highlightCurrentSong`. Unutar funkcije, proverite da li postoji element `.playlist-song` sa atributom `aria-current="true"`. Ako jeste, koristite metod `removeAttribute` da uklonite atribut `aria-current` sa njega.
