...
Postavite elemente `h1` i `p` u sredinu, dajući im `margin` od `1em auto`. Zatim, rasporedite njihov tekst takođe u `center`.

# --hints--

Trebalo bi da koristite selector stavki razdvojenih zarezima kako biste organizovali stavke od `h1` i `p`.
...
Trebalo bi da koristite `text-align` od `center` za centriranje teksta `h1` na `p`.
