...
Unutar `div`, dodaj dva elementa od `p`. Postavi tekst drugog elementa od `p` na vrednost promene `price`.

# --hints--

Treba da dodate dva elementa `p` unutar vašeg elementa `div`.
...
Tvoj drugi element za `p` mora imati opis izmena za `price`.
