...
Posljednje vrijednosti postavljene direktno su šest statistika igrača. Zamijenite prva tri skupa direktno postavljenih brojeva sa odgovarajućim atributima za `player`:
...
Trebalo bi da promeniš vrednost `97` u `PAC` sa `player.pac`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.pac\}/);
```

Trebalo bi da promeniš vrednost `98` u `SHO` na `player.sho`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.sho\}/);
```

Trebalo bi da promeniš vrednost `83` u `PAS` sa `player.pas`.
