...
Kreiraj drugi element za `button` sa klasom stavki `playlist-song-delete` i svojstvom elemenata `aria-label` postavljeno na `Delete` kombinovano sa `song.title`. Za sadržaj ikone za brisanje, zalepite sledeći SVG:
...
Vaš element dugmeta treba da ima atribut elemenata `aria-label` sa vrednošću `Delete ${song.title}`, kao i drugi atribut elementa i njegovu vrednost.
