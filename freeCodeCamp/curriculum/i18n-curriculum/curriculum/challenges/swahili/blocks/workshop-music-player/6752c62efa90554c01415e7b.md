...
Kreiraj funkciju pod nazivom `setPlayerDisplay`. Unutar funkcije, postavi `textContent` od `playingSong` na naslov trenutne pesme, ako je definisan. U suprotnom, postavi ga na praznu string.

Niže, postavi `textContent` od `songArtist` na izvođača trenutne pesme, ako je definisan. U suprotnom, postavi ga na praznu string.
