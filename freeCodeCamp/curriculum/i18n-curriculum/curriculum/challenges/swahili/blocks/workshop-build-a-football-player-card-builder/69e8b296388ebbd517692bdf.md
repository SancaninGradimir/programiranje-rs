...
Dodaj atribute za `value` na `onChange` za oba ulaza DEF i PHY.

Za DEF, postavi `value` da bude `player.def` i `onChange` da bude strelna funkcija koja poziva `setPlayer` prenošenjem `player` i `def` postavljeno na `Number(e.target.value)`.

Za PHY, postavi `value` na `player.phy` i `onChange` kao streličnu funkciju koja poziva `setPlayer` širenjem `player` i `phy` postavljeno na `Number(e.target.value)`.

Na ovom koraku, svi ulazi su sada povezani sa stanjem i živi prikaz će biti ažuriran za svako polje!

# --hints--

`input` tvoj DEF mora imati `value` jednako sa statistikom od `def` igrača.
...
`input` yako ya DEF treba da ima handler za `onChange` koji poziva `setPlayer` za emitovanje `player` i postavljanje `def` na `Number(e.target.value)`.
...
`input` Vaša PHY bi trebalo da ima `value` jednako sa statistikom za `phy` igrača.
...
`input` vaš PHY bi trebalo da ima rukovatelja za `onChange` koji poziva `setPlayer` za širenje `player` i `phy` podešen na `Number(e.target.value)`.
