...
Tvoja funkcija je sada gotova. Pokušaj ponovo upisivanjem `genParentheses(2)` umesto `genParentheses(1)`.

# --hints--

Trebalo bi da napišete `genParentheses(2)`.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*genParentheses\s*\(\s*2\s*\)\s*\)/);
```

Ne bi trebalo da imaš `console.log(genParentheses(1))` u svom kodu.
