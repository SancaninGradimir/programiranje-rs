...
Svaki put kada kliknete na dugme za reprodukciju, prva pesma vaše plejliste će uvek da se reprodukuje. Da biste ovo popravili, unutar strelične funkcije slušaoca događaja (event listener), zamenite `playSong(0)` sa izjavom `if` koja proverava da li je `userData.currentSong` jednako `null`.

Unutar bloka `if`, pozovite funkciju `playSong` sa `id` prve pesme u nizu `userData.songs`.
