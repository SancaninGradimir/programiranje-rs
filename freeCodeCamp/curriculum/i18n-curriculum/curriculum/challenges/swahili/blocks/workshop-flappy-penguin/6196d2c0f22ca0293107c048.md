...
Trebalo bi da koristite selektor `.sun`.

```js
assert.match(code, /\.sun\s*\{/);
```

Trebalo bi da date `.sun` na `width` od `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.width, '200px');
```

Trebalo bi da date `.sun` a `height` od `200px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sun')?.height, '200px');
```

Trebalo bi da date `.sun` a `background-color` od `yellow`.
