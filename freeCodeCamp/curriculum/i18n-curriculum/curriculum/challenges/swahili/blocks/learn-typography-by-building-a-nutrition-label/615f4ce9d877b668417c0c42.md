...
Zatvori sve što je unutar elementa `.label` u novom elementu `header`.

# --hints--

Trebalo bi da kreirate novi element za `header`.

```js
assert(document.querySelector('header'));
```

Tvoj element `header` treba da bude unutar tvog elementa `.label`.

```js
assert(document.querySelector('header')?.parentElement?.classList?.contains('label'));
```

Vaši elementi za `h1`, `div`, i `p` treba da budu unutar vašeg novog elementa za `header`.
