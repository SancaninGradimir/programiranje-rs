...
Unutar prvog elementa od `span`, dodajte tekst za `Question`.

U drugom odeljku `span`, dodaj tekst za `Question`.

# --hints--

Trebalo bi mu dati prvi deo teksta `span` od `Question`.

```js
assert.equal(document.querySelector('h3 > span')?.textContent.trim(), 'Question');
```

Trebalo bi mu dati drugi element od `span` teksta `Question`.
