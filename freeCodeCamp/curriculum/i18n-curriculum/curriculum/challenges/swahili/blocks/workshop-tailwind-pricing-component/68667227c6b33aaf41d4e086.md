...
Da biste postavili elemente na karticu na njenom mestu, ponovo su vam potrebne grid klase. Primenite service klase `grid`, `grid-rows-[1fr_auto]`, i `gap-6` na kontejneru `div`.

Pia, ukloni oštar ugao koristeći klasu `rounded-xl`.

# --hints--

Vaš paket `div` za premium plan treba da ima klasu `grid`.
...
Vaš paket `div` za premium plan treba da ima klasu `grid-rows-[1fr_auto]`.
...
Vaš paket `div` za Premium plan treba da ima klasu `gap-6`.
...
Vaš paket `div` za premium plan mora imati klasu `rounded-xl`.
