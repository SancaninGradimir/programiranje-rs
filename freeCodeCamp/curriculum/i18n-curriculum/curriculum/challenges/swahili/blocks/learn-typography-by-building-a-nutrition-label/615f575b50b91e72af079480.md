...
Kreiraj novi selektor za `.left-container p`, podešavanjem gornje i donje ivice na `-5px`, a levu i desnu ivicu na `-2px`. Takođe postavi `font-size` na `2em` i `font-weight` na `700`.

# --hints--

Trebalo bi da imaš novi birač za `.left-container p`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p'));
```

Tvoj novi selektor za `.left-container p` mora imati svojstvo `margin` postavljeno na `-5px -2px`.
...
Tvoj novi selektor za `.left-container p` mora imati svojstvo `font-size` postavljeno na `2em`.

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

Vaš novi `.left-container p` mora imati karakteristiku `font-weight` postavljenu na `700`.
