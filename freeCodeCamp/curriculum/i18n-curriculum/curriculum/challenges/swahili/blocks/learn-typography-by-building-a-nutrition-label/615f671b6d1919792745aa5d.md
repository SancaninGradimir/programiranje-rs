...
Kreiraj novi element za `div` ispod tvog elementa `.medium`. Jedinstveno svojstvo elementa `class` mora biti jednako sa `daily-value small-text`. Unutar ovog novog `div`, dodaj element za `p` sa tekstom `% Daily Value *`, i postavi svojstvo elementa `class` na `bold right`.

# --hints--

Treba da kreiraš novi element za `div` nakon tvog elementa `.medium`.
...
Vaš novi element za `div` mora imati svojstvo elementa `class` postavljeno na `daily-value small-text`.
...
Tvoj novi element `div` treba da ima element `p`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.localName === 'p');
```

Vaša nova funkcija za `p` mora da ima tekst `% Daily Value *`.

```js
assert(document.querySelector('.label')?.lastElementChild?.firstElementChild?.textContent === '% Daily Value *');
```

Tvoja nova karakteristika `p` treba da ima svojstvo karakteristike `class` postavljeno na `bold right`.
