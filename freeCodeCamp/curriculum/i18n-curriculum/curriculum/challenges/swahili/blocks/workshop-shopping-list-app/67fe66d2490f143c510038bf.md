...
Zatim, u elementu za izbor ``input``, dodajte atribut elementa ``checked`` i postavite ga na ``isChecked``. Iako nije striktno neophodno postavljati atribut elementa ``checked``, dobra praksa je osigurati da svaki padajući meni prikazuje trenutni status redova podataka ``selectedItems``.

# --hints--

Treba da dodate atribut elementa `checked` u polje za izbora `input`.
...
Trebalo bi da postaviš atribut elementu `checked` na `isChecked`.
