...
Sada unutar funkcije `isSelect`, vrati vrednost tačno ili netačno koja ukazuje da je `element` instanca od `HTMLSelectElement`. Koristi operator `instanceof` za izvršavanje ove provere.

# --hints--

Tvoja funkcija za `isSelect` treba da vrati `element instanceof HTMLSelectElement`.
