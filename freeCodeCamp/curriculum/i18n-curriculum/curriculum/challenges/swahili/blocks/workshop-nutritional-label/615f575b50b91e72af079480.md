...
Kreiraj novi `.left-container p` selektor koji postavlja gornji i donji margina na `-5px`, a levi i desni margina na `-2px`. Takođe postavi `font-size` na `2em` i `font-weight` na `700`.
