...
Sledeći, napravite komponentu za klasu `Bug` koja prima jedan parametar nazvan `emojiElement` tipa `HTMLParagraphElement`. Unutar komponente, postavite vrednost parametra `emojiElement` na svojstvo `emojiElement` te klase.

# --hints--

Vaša klasa za `Bug` mora imati komponentu.
...
Tvoja komponenta `Bug` treba da ima parametar nazvan `emojiElement` tipa `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Vaša komponenta za `Bug` treba da postavi vrednost `emojiElement` na `this.emojiElement`.
