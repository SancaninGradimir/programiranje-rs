...
Trebalo bi da imate nešto nazvano `enum`, a to je `DrawingType`.

```js
assert.match(__helpers.removeJSComments(code), /enum\s+DrawingType\s*{/)
```

Vaš enum bi trebalo da ima `Past` svojstvo sa string vrednošću od `past`.

```js
assert.match(__helpers.removeJSComments(code), /Past\s*=\s*['"]past['"]/);
```

Vaš enum bi trebalo da ima atribut `Present` sa string vrednošću od `present`.

```js
assert.match(__helpers.removeJSComments(code), /Present\s*=\s*['"]present['"]/);
```

Vaš enum bi trebalo da ima `Future` svojstvo sa string vrednošću od `future`.
