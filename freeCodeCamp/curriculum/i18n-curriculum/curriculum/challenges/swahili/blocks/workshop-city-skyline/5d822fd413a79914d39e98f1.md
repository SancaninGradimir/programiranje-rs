...
Želite da dodate istu boju u sledeće dve sekcije. Umesto toga, kreirajte novi selektor klase objekta nazvan `bb1-window`, i premestite svojstva i vrednosti `height` i `background` iz `.bb1a` u novi selektor klase objekta.

# --hints--

Moraš kreirati novu klasu objekata koja se zove `bb1-window`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

Morate prebaciti svojstvo i vrednost `height` iz `.bb1a` u `.bb1-window`.
...
Treba da premestiš atribut i vrednost od `background` na `.bb1a` ka `.bb1-window`.
...
Ne smete da prenesete atribut iz `background-color` na `.bb1a`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.bb1a')?.backgroundColor);
```

Ne možeš prebaciti atribut iz `.bb1a` na `width`.
