...
Kreiraj `if` izjavu. Za njen uslov, koristi operater `not` da negira `is_sequence`. Unutar `if` izjave, štampaj `Invalid format: expected a list or tuple.` i vrati `False`.
