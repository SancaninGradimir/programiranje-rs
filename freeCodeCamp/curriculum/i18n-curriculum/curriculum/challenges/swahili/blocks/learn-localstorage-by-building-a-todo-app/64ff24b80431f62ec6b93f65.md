...
Uklonite red podataka `myTaskArr` sa svim kodovima `localStorage` jer više neće biti potreban.

# --hints--

Trebalo bi da uklonite `myTaskArr` i sav kod povezan sa `localStorage` koji je nedavno naučen.
...
Trebalo bi da uklonite sve preostale reference na `myTaskArr` nigde u kodu.

```js
assert.notMatch(code, /myTaskArr/);
```

Trebalo bi da uklonite bilo koji referencu na `localStorage.getItem` za element `"data"`.

```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

Morate ukloniti sve preostale reference za `getTaskArrObj` bilo gde u kodu.
