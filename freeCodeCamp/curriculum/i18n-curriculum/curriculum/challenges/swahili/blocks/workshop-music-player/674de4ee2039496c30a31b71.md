...
Dodajte blok `else` za rukovanje trenutnim vremenom reprodukcije pesme. Ovo vam omogućava da nastavite sa trenutnom pesmom od mesta gde je pauzirana.

Unutar bloka `else`, postavite svojstvo `currentTime` objekta `audio` na vrednost sačuvanu u `userData.songCurrentTime`.
