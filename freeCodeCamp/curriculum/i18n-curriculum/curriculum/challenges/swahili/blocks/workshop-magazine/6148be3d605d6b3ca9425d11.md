...
Ispodnaslov takođe mora biti stilizovan. Kreirajte selektor `.hero-subtitle` i dodelite mu svojstvo `font-size` postavljeno na `2.4rem`, svojstvo `color` postavljeno na `orangered`, i svojstvo `text-align` postavljeno na `center`.
