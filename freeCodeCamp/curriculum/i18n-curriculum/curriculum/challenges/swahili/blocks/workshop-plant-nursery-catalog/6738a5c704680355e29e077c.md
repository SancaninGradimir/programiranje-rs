...
Kao što ste već naučili, metoda `delete` se koristi za uklanjanje unosa iz mape. Setovi imaju metodu `delete` koju takođe možete koristiti za uklanjanje elementa iz njih.

Ponekad je možda potrebno ukloniti sve elemente iz mape ili seta. Metoda `clear` radi upravo taj posao.

Pozovite metodu `clear` na `plantsSet` da uklonite sve elemente iz njega.
