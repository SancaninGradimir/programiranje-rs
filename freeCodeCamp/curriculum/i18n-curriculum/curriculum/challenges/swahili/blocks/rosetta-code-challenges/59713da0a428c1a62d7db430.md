...
<a href="https://rosettacode.org/wiki/Cramer%27s_rule" target="_blank" rel="noopener noreferrer nofollow">Kremerevo pravilo</a> je formula za rešavanje sistema linearnih jednačina korišćenjem determinanti matrica formiranih iz podskupa koeficijenata i vrednosti sa desne strane.

Determinat matrice sa 2 reda i 2 kolone izračunava se po:
...
`cramersRule` Trebalo bi da bude funkcija.

```js
assert(typeof cramersRule === 'function');
```

`cramersRule([[2, -1, 5, 1], [3, 2, 2, -6], [1, 3, 3, -1], [5, -2, -3, 3]], [-3, -32, -47, 49])` treba da vrati `[2, -12, -4, 1]`.

```js
assert.deepEqual(cramersRule(matrices[0], freeTerms[0]), answers[0]);
```

`cramersRule([[3, 1, 1], [2, 2, 5], [1, -3, -4]], [3, -1, 2])` bi trebalo da vrati `[1, 1, -1]`.
