...
Podrazumevano, elementi `label` su u liniji (inline). Da ih učinite vertikalnim stablama (osim oznaka za radijumske dugmad), koristite pseudoklasu `:not()`.

Pseudoklasa `:not()` negira selekciju. Ovde, ona bira sve elemente `label` koji nemaju klasu `contact-method`.

Izaberite `label:not(.contact-method)`, zatim postavite `display` na `block` i `margin` na `10px 0`.
