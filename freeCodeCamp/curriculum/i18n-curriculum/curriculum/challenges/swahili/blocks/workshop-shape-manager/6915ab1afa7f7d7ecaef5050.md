...
Da biste konačno prikazali izračun, ponovite kroz nešto od `propertyInputs` koristeći sekvencu od `for...of` i `Object.entries()`. Možete ignorisati ključ korišćenjem zareza pre `input` u deklaraciji sekvence.

# --hints--

Treba da kreirate krug `for...of` koji razbija svaki element iz `Object.entries(propertyInputs)` tako što će proslediti ključ i sačuvati samo `input`.

```js
assert.match(code, /for\s*\(const\s*\[\s*,\s*input\s*\]\s+of/)
```

Tvoj ciklus ``for...of`` bi trebalo da se ponovi na ``Object.entries(propertyInputs)``.
