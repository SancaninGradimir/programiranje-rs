...
Nakon vašeg poslednjeg `.divider`, kreirajte novi `p` element sa tekstom `Cholesterol 0mg 0%`. Zatim, obavijte tekst `Cholesterol` u `span` elementu, `0%` u drugom, i dodelite im klasu `bold`.

Na kraju, ugnježđujte `span` element koji sadrži tekst `Cholesterol` zajedno sa tekstom `0mg`, u dodatni `span` element radi poravnanja.
