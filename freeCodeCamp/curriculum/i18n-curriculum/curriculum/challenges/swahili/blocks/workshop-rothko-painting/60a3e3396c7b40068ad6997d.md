...
Koristi vizetizer da staviš element `.one` na platno za crtež.

Dodaj kratki atribut od `margin` sa vertikalnom granicom od `20px` i horizontalnom granicom od `auto`.

# --hints--

Treba da postaviš atribut `margin` na `20px auto`.
...
Vaše polje `.one` mora imati vrednost `margin` od `20px auto`.
