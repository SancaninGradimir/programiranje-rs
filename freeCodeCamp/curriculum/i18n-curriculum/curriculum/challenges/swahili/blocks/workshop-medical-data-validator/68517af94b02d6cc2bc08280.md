...
Liste razumevanja takođe prihvata klauzule `if` za filtriranje stavki iz iterabilnog objekta:
...
Pošto želite da vratite listu koja sadrži samo nevažeće ključeve, dodajte klauzulu `if` u vaše razumevanje tako da se svaki `key` doda na listu samo kada je `value` lažno.
