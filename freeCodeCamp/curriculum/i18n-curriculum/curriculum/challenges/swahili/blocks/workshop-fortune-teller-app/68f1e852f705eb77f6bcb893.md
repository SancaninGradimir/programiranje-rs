...
Trebalo bi da deklarišete varijablu pod nazivom `cardId` unutar `showFortune`.
...
Vaša varijabla `cardId` treba da dobije vrednost pozivanjem `cardElement.getAttribute("data-id")`.
...
Trebalo bi da koristite metod `getAttribute` na `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

Trebalo bi da prosledite `'data-id'` na `getAttribute`.
