...
Druga vrednost za svojstvo elementa `scope` je `row`, što ukazuje da je ćelija zaglavlje za ceo njen red.

Unutar vašeg elementa sa `th`, dodajte svojstvo elementa `scope` sa vrednošću `row`.

# --hints--

Vaš element za `th` mora imati svojstvo polja `scope` sa vrednošću `row`.
