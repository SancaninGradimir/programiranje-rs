...
Dodajte novu klasu za `bb1-window` za elemente `.bb1a`, `.bb1b` i `.bb1c`. Ovo će im dati bojni stil.

# --hints--

Ne bi trebalo da uklonite klasu `bb1a`.

```js
assert.exists(document.querySelector('div.bb1a'));
```

Trebalo bi dodati klasu `bb1-window` na element `.bb1a`.

```js
assert.exists(document.querySelector('div.bb1a.bb1-window'));
```

Ne smete da uklonite klasu `bb1b`.

```js
assert.exists(document.querySelector('div.bb1b'));
```

Treba dodati klasu za `bb1-window` za komponentu `.bb1b`.

```js
assert.exists(document.querySelector('div.bb1b.bb1-window'));
```

Ne možete ukloniti klasu od `bb1c`.

```js
assert.exists(document.querySelector('div.bb1c'));
```

Trebalo bi dodati klasu `bb1-window` na element `.bb1c`.

```js
assert.exists(document.querySelector('div.bb1c.bb1-window'));
```

Ne možete menjati element za `.bb1d`.
