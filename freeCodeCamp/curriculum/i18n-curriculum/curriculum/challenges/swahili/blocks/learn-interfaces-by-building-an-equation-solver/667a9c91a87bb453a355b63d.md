...
Slobodno promenite podešavanja za `lin_eq` ili vidite kako se rezultati menjaju.

Zatim, obriši svoj poziv za `print(solver(lin_eq))`, i odštampaj rezultate pozivanja `solver()` sa `quadr_eq` kao argument.

# --hints--

Ne bi trebalo da imaš `print(solver(lin_eq))` u svom kodu.

```js
({ test: () => assert.isFalse(runPython(`_Node(_code).has_call("print(solver(lin_eq))")`)) })
```

Trebalo bi da ispišete `solver(quadr_eq)`.
