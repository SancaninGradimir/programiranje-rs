...
Na kraju, premesti svoju deklaraciju `console.log()` unutar funkcije povratnog poziva `useMemo()`, malo iznad deklaracije `return`.

# --hints--

Morate migrirati vaš zapis za `Filtering items...` pre deklaracije `return` unutar funkcije povratnog poziva (callback) od `useMemo()`.
