...
Da bi funkcionalnost `localStorage` radila ispravno, promenite inicijalizaciju stanja sa `defaultPlayer` na `loadPlayer`. Ovo osigurava da stanje počinje sa bilo kojim dostupnim sačuvanim podacima.

Sada probajte funkciju `localstorage`. Sve što unesete treba da ostane nakon osvežavanja prikaza ili ako ponovo učitate stranicu.

Takođe možete menjati sliku `src` povezanu sa `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/maradona.png` da biste videli kako se slika menja u realnom vremenu. Samo se pobrinite da promenite ime igrača u Maradona.

Dakle, tvoj projekat generatora kartica igrača fudbala je završen!

# --hints--

`useState` treba da se inicijalizuje sa `loadPlayer` umesto `defaultPlayer`.
