...
Napravi promenljivu pod nazivom `audio` i postavi je na `new Audio()`. Time će biti kreiran novi HTML5 element `audio`.
...
Treba da dodeliš novu instancu objekta `Audio` promenljivoj pod nazivom `audio`.
