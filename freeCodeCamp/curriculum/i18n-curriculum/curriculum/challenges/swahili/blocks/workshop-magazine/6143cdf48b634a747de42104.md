...
Unutar vašeg `.image-wrapper` elementa, dajte svom trećem `img` elementu `src` od `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`, atribut `alt` od `four people working on code`, atribut `loading` sa `lazy`, `class` postavljen na `image-3`, atribut `width` postavljen na `600`, i atribut `height` postavljen na `400`.
