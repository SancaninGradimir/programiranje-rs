...
Deklarišeš drugu varijablu pod imenom `foundCard`, i dodeljuješ joj rezultat poziva metode `find()` na `cards`. Prošalji `card` kao parametar povratnog poziva (callback), i uporedi `card.name_short` sa `cardId` kako bi pronašao odgovarajuću karticu koristeći strogu jednakost.

# --hints--

Trebalo bi da deklarišete varijablu pod imenom `foundCard` unutar `showFortune`.
...
Vaš `foundCard` bi trebalo da bude dodeljen pozivanjem `find` na `this.cards` sa ispravnom povratnom funkcijom.
...
``find`` callback bi trebalo da ima parametar pod imenom ``card``.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Unutar callback-a, `card.name_short` treba se uporediti sa `cardId`.
