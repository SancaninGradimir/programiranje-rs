...
Trebalo bi da deklariš funkciju pod imenom `getRandomItem`.
...
Tvoja funkcija `getRandomItem` bi trebalo da koristi generički tipski parametar `T`.
...
Vaša funkcija `getRandomItem` bi trebalo da ima parametar pod imenom `items` sa tipom `T[]`.
...
Your `getRandomItem` function treba da ima return tip od `T`.
...
Trebalo bi da deklarišete varijablu pod imenom `index` unutar funkcije `getRandomItem`.
...
Varijabla `index` bi trebalo da koristi `Math.floor()` i `Math.random()` sa `items.length` kako bi proizvela nasumičan indeks.
...
Vaša `getRandomItem` funkcija bi trebalo da return `items[index]`.
