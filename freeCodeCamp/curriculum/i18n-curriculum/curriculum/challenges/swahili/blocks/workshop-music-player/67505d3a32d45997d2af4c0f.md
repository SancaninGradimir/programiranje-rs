...
Kreiraj funkciju pod nazivom `playPreviousSong`. Unutar svoje nove funkcije, koristi vrednost koju vraća funkcija `getPreviousSong` da pronađe pesmu za reprodukciju. Kada je `getPreviousSong()` jednako `undefined`, pustu prvu pesmu u vašoj plejlisti. U suprotnom, pust pesmu pre trenutno reprodukovane pesme.
