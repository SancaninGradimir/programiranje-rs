...
Deklarišite varijablu nazvanu `isReversed` unutar metode `singleCardSelected`. Koristite metodu `Math.random()` za generisanje nasumične booleanske vrednosti. Da bi se osiguralo da vrednost ostaje u opsegu, uporedite je sa `0.5`.

# --hints--

Trebalo bi da deklarišete varijablu pod imenom `isReversed`.
...
Vaša promenljiva `isReversed` bi trebalo da koristi `Math.random()` da nasumično odluči if da li je kartica obrnuta.
