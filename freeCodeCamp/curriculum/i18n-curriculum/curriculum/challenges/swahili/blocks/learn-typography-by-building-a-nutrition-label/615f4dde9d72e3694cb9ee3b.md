...
Sada ispravite svoju komponentu za `h1` u `header h1` ili ciljajte specifično svoj element od `h1` unutar `header` nov.

# --hints--

Trebalo bi da imaš birač za `header h1`.

```js
assert(new __helpers.CSSHelp(document).getStyle('header h1'));
```

Ne bi trebalo da imaš selektor za `h1`.

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('h1'));
```

Ne smete menjati bilo koje parametre u selektoru.
