...
Na isti način koji ste ranije koristili sa `__eq__`, kreiraj izjavu `if` koja proverava da li `self` i `other` nisu u istoj klasi objekata i vrati `NotImplemented`.

# --hints--

Treba da napravite izraz za `if` koji proverava da li `self` i `other` nisu u istoj klasi objekata.
...
Treba ti da vratiš `NotImplemented` iz tela/sadržaja `if`.
