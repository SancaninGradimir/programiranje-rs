...
Da, vrsta od `input` `file` dozvoljava samo to. Dodajte `label` sa tekstom `Upload a profile picture: `, i polje unutar `input` koje prihvata učitavanje fajlova.

# --hints--

Treba da dodate `label` sa tekstom `Upload a profile picture: `.

```js
assert.match(document.querySelector('fieldset:nth-child(3) > label')?.innerText, /Upload a profile picture:/i);
```

Treba da postaviš unutar elementa `input`, i unutar elementa `label`.
