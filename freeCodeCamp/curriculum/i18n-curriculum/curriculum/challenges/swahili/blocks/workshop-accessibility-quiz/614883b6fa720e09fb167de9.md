...
Povećaj unutrašnji prostor (padding) gore i levo za elemente `.info`, i postavi druge vrednosti na `0`.
...
Morate mu dati `.info` unutrašnji prostor u `padding-top`, najmanje `1px`.

```js
assert.isAtLeast(Number(new __helpers.CSSHelp(document).getStyle('.info')?.paddingTop?.replace(/\D+/, '')), 1);
```

Trebalo bi da mu/jajmo prostor unutar `.info` od `padding-right` u `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingRight, '0px');
```

Treba da mu daš unutrašnji prostor od `.info` za `padding-bottom` u `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.info')?.paddingBottom, '0px');
```

Morate mu dati `.info` unutrašnjeg prostora od `padding-left` najmanje `1px`.
