...
Sada ćete početi da pravite obrazac. Unutar `div` elementa sa `className` vrednošću `form-panel`, dodajte `div` koji će sadržati odeljak „Player Info“. Unutar novog `div` elementa dodajte `p` sa `className` vrednošću `form-section-title` i tekstom `Player Info`.
