...
Zatim postavi `userData.currentSong` na `song`, a da bi na kraju pustio pesmu, pozovi metodu `play()` nad promenljivom `audio`. `play()` je metoda HTML5 Audio API-ja za reprodukciju audio datoteka.
