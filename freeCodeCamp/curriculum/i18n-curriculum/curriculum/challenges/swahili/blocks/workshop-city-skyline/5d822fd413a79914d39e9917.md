...
Možete dodati mešavinu više boja na element razdvajanjem zarezima (`,`) kao što je ovo:
...
Dodaj `repeating-linear-gradient` u `.fb1c` ispod postojećeg; koristi `--building-color4` od `0%` do `10%` i `--window-color4` od `10%` do `90%`. Ovo će popuniti pozadinu poslednje dodate mešavine boja.

# --hints--

Ne smete da promenite `repeating-linear-gradient` prvi.
...
Treba da ima zarez (`,`) nakon prvog `repeating-linear-gradient`.
...
Treba da dodate `repeating-linear-gradient` sa prvom bojom od `--building-color4` od `0%` do `10%`.
...
Trebalo bi da koristite drugu boju od `--window-color4` do `10%` do `90%`.
