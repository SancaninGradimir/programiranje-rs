...
Indikatori stavki liste zahtevaju malo razmaka. Stoga, uklonite klasu `span` i `text-green-700` za svaku `mr-2` u stavkama liste.

Pia, dodajte atribut elementa `aria-hidden` za svaki element `span`.

# --hints--

Vaši elementi za `span` treba da imaju klasu `text-green-700`.
...
Elementi vašeg `span` treba da imaju klasu `mr-2`.
...
Vaši elementi od `span` treba da imaju svojstvo elementa `aria-hidden`.
