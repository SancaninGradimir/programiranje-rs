...
Ako želite dodati više društvenih ikona, ali ih zadržati na istom redu, morali biste ažurirati `grid-template-columns` kako biste kreirali dodatne kolone. Kao alternativa, možete koristiti svojstvo `grid-auto-flow`.

Ovo svojstvo uzima ili `row` ili `column` kao prvu vrednost, sa opcionalnom drugom vrednošću od `dense`. `grid-auto-flow` koristi algoritam automatskog postavljanja za prilagođavanje rasporeda mreže. Postavljanjem na `column`, reći ćete algoritmu da kreira nove kolone za sadržaj po potrebi. Vrednost `dense` omogućava algoritmu da se vrati i popuni rupe u mreži manjim elementima, što može rezultirati pojavljivanjem elemenata van redosleda.

Za vaš selektor `.social-icons`, postavite svojstvo `grid-auto-flow` na `column`.
