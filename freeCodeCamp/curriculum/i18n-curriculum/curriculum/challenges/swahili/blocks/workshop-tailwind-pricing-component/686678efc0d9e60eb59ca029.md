...
Banner se može vizuelno poboljšati. Za to, koristićeš neke kružne klase iz Tailwind-a. Evo kako možeš da koristiš kružna stilizovanja u Tailwindu:
...
Dodajte kurseve `bg-gradient-to-r`, `from-fuchsia-500`, na `to-indigo-500` na baneru `div`. Takođe, dodajte `rounded-full`, `px-3`, `py-1`, `text-xs`, `font-bold`, i `text-white`.

# --hints--

Element vašeg `div` sa tekstom `Most popular` bi trebalo da ima klasu `bg-gradient-to-r`.
...
Vaš element `div` sa tekstom `Most popular` mora imati klasu `from-fuchsia-500`.
...
Tvoj element sa `div` koji ima tekst `Most popular` treba da ima klasu `to-indigo-500`.
...
Vaš element sa `div` koji ima tekst `Most popular` treba da ima klasu `rounded-full`.
...
Vaš element od `div` sa tekstom `Most popular` mora imati klasu `px-3`.
...
Element vašeg `div` sa tekstom `Most popular` bi trebalo da ima klasu `py-1`.
...
Tvoj element za `div` sa tekstom `Most popular` mora imati klasu `text-xs`.
...
Vaš element za `div` sa tekstom `Most popular` treba da ima klasu `font-bold`.
...
Tvoj element za `div` sa tekstom `Most popular` treba da ima klasu `text-white`.
