...
Nakon vašeg polja sa kodom `h2`, kreirajte dva polja sa kodom `p`. Dodajte prvo polje `class` iz `dessert-price`, i podesite tekst na znak dolara `"$"` praćen vrednošću parametra `price`. Dodajte drugo polje `class` iz `product-category`, i tekst `"Category: "` praćen vrednošću parametra `category`.

# --hints--

Moraš da kreiraš dva elementa od `p`.
...
Vaši elementi za `p` bi trebalo da dolaze nakon vašeg elementa za `h2`.
...
Prvi deo vašeg `p` treba da ima `class` od `dessert-price`.

```js
assert.strictEqual(document.querySelector('.dessert-card')?.children[1].className, 'dessert-price');
```

Vaše prvo polje za `p` trebao bi imati dolar znak `"$"` praćen vrednošću parametra `price`.

```js
assert.strictEqual(document.querySelector('.dessert-card')?.children[1].textContent, '$12.99');
```

Vaš drugi element `p` treba da ima `class` od `product-category`.

```js
assert.strictEqual(document.querySelector('.dessert-card')?.children[2].className, 'product-category');
```

Drugi element vašeg `p` mora imati tekst `"Category: "` praćen vrednošću parametra `category`.
