...
Takođe bi trebalo da radiš na funkciji koja pronalazi ulazne delove/polja.

Kreiraj funkciju `clearInputFields`. Unutar nje, koristi `querySelectorAll` da dobije sve ulazne polja i iteriraj kroz njih koristeći `forEach()`. Unutar `forEach()`, postavi vrednost svakog `input` na prazan niz znakova.

# --hints--

Treba da kreiraš funkciju za `clearInputFields`.

```js
assert.isFunction(clearInputFields)
```

Tvoja funkcija za `clearInputFields` treba da postavi svaki deo unosa kao prazan niz znakova.
