...
Kreiraj novi `.calories-info` selektor i dodaj mu svojstvo `display` postavljeno na `flex`. Takođe mu dodaj svojstvo `justify-content` postavljeno na `space-between` i svojstvo `align-items` postavljeno na `flex-end`.
