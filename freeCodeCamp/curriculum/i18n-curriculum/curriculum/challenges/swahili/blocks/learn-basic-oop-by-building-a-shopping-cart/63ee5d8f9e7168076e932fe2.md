...
Nakon vašeg elementa sa `h2`, kreiraj dva elementa od `p`. Dodaj prvi element `class` za `dessert-price`, i postavi tekst sa dolar znakiem `"$"` praćen vrednošću parametra `price`. Dodaj drugi element `class` za `product-category`, i tekst `"Category: "` praćen vrednošću parametra `category`.
