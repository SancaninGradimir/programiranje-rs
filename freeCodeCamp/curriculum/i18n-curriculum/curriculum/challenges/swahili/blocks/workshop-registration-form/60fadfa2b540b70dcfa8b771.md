...
Za drugi `fieldset`, želite da tekst za `input` i `label` bude prikazan na jednoj liniji.

Počnite prosleđivanjem elemenata iz `input` u drugu klasu od `fieldset` za `inline`.

# --hints--

Treba da mu/jaj prvi razred od `input` za razred `inline`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-child(2) input')?.[0]?.classList?.contains('inline'));
```

Trebalo bi da mu/jaj daje `input` za drugi razred od `inline`.
