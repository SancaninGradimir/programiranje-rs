...
Okreni `.three` protiklop za niti od 0.2.

Na ovom zadnjem koraku, tvoja slika Rothka je sada kompletirana.

# --hints--

Trebalo bi da postaviš atribut `transform` na `rotate(-0.2deg)`.
...
Vaše polje `.three` trebalo bi imati vrednost `transform` od `rotate(-0.2deg)`.
