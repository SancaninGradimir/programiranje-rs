...
Unutar vašeg `article` elementa, kreirajte `h3` element sa `class` podešenim na `list-title` i tekstom od `A Brief History`. Ispod toga, kreirajte `p` element sa tekstom `Of the Curriculum`. Zatim kreirajte `ul` element sa klasom `lists`.
