...
Unutar bloka `try`, sačuvajte podatke trenutnog igrača u `localStorage` pozivom `localStorage.setItem` koristeći ključ baze podataka i objekt igrača konvertovan u niz karaktera.

U bloku za `catch`, napiš niz slova `Failed to save player data:` praćen parametrom `error` u konzoli.

# --hints--

Blok vašeg `useEffect` try treba da izvrši poziv ka `localStorage.setItem` sa `STORAGE_KEY` i nečega od `player` što je promenjeno u niz slova.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.setItem\s*\(\s*STORAGE_KEY\s*,\s*JSON\.stringify\s*\(\s*player\s*\)\s*\)/);
```

Tvoj blok za `useEffect` catch mora da napiše `'Failed to save player data:'` i `error` u konzoli.
