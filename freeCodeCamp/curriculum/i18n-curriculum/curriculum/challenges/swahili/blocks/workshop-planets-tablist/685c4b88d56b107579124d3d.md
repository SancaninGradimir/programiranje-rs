...
Ako pogledate HTML, videćete da su drugi i treći panel skriveni. Ovo se radi korišćenjem atributa `hidden`. Prvi panel je vidljiv po podrazumevanju. Kada korisnik klikne na karticu, svi paneli bi trebalo da budu skriveni.

Koristite metod `forEach` da iterirate kroz `panels` i postavite atribut `hidden` na svakom panelu na `true`.
