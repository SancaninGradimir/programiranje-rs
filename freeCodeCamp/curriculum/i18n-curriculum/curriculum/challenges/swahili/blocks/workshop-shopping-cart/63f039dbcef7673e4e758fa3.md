...
Pretraživači imaju ugrađenu funkcionalnost za `confirm()` koja prikazuje upozorenje o autentifikaciji korisniku. `confirm()` prima niz znakova, koji je poruka prikazana korisniku. Vraća `true` ako korisnik potvrdi, i `false` ako korisnik otkaže.

Objavi kriterijum `isCartCleared` za poklopac/poklopac poziva `confirm()` sa nizom slova `"Are you sure you want to clear all items from your shopping cart?"`.
...
Trebalo bi da proslediš niz znakova `Are you sure you want to clear all items from your shopping cart?` funkciji `confirm()`.
