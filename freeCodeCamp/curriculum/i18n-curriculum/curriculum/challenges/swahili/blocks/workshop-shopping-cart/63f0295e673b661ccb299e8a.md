...
Potreban vam je način da dobijete ukupan broj stavki u korpi. Najbolji način da to učinite je dodavanje druge metode u klasu `ShoppingCart` umesto direktnog pristupa redovima podataka iz `items`.

Dodajte metodu za ``getCounts`` u klasu ``ShoppingCart``. Trebalo bi da vrati broj elemenata koji se nalaze u nizu podataka ``items``.

# --hints--

Vaša klasa za `ShoppingCart` mora imati metodu za `getCounts`.
...
Tvoj put za `getCounts` bi trebalo da vrati broj stavki u koloni podataka `items`.
