...
Dodajte dva elementa `span` koja imaju `className` od `stat-value` i `stat-label`, kontrolisana za svaki `div.stat-row`.

# --hints--

Svaki `.stat-row` treba da ima `span` koji ima `className` od `stat-value` na `span` sa `className` od `stat-label`.
