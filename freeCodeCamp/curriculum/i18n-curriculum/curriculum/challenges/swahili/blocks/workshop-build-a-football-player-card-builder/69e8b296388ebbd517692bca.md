...
`FootballPlayerCard` trenutno prikazuje karticu sa postavljenim `defaultPlayer`. Da bi aplikacija bila interaktivna, potrebno je da upravljate podacima o igraču pomoću React stanja kako bi promene izazvale ponovno renderovanje.

Unutar komponente `FootballPlayerCard` napravite promenljivu stanja pod nazivom `player` i funkciju za ažuriranje `setPlayer` koja će čuvati podatke trenutnog igrača. Inicijalizujte stanje pomoću `defaultPlayer` i navedite da stanje koristi tip `PlayerData` kao generički tip kako bi TypeScript znao strukturu objekta igrača.

Zatim ažurirajte ono što komponenta vraća tako da prosledi promenljivu stanja `player` komponenti `PlayerCard`.

# --hints--

Treba da koristite destrukturiranje niza kako biste definisali promenljivu stanja `player` i funkciju za ažuriranje `setPlayer`.
...
Vaš `useState` treba da bude inicijalizovan sa `defaultPlayer` i da koristi `PlayerData` kao generički tip.

```js
assert.match(__helpers.removeJSComments(code), /useState<PlayerData>\(\s*defaultPlayer\s*\)/)
```

Treba da prosledite `player` kao vrednost `player` propa komponenti `PlayerCard`.
