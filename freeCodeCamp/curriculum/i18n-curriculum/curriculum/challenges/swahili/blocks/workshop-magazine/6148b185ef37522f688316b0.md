...
Sada možete stilizovati sadržaj elementa `.heading` pomoću CSS Grida.

CSS funkcija `repeat()` se koristi za ponavljanje vrednosti, umesto da je ručno pišete, i korisna je za grid rasporede. Na primer, podešavanje svojstva `grid-template-columns` na `repeat(20, 200px)` kreiralo bi 20 kolona, svaka široka `200px`.

Dajte svom elementu `.heading` svojstvo `grid-template-columns`, postavljeno na `repeat(2, 1fr)`, da biste kreirali dve kolone jednakih širina.
