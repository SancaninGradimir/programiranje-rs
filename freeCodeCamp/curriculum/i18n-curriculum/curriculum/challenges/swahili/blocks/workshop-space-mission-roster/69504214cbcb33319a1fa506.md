...
Koristi svoj funkcioner `chunkCrew()` da kreira delove veličine `3` iz niza podataka `EVAReadySquad` i skladišti ih u novu parametar nazvan `EVAChunks`.

# --hints--

Morate pozvati `chunkCrew()` na `EVAReadySquad` i `3` da sačuva rezultate u novi parametar nazvan `EVAChunks`.
