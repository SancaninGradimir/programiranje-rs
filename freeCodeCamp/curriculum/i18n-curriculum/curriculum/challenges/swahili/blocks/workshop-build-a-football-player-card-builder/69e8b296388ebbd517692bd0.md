...
Nakon `form-row`, dodaj `div` koji ima `className` od `form-group`. Unutar njega, dodaj element za `label` koji ima `className` od `label`, `htmlFor` od `club`, i tekst od `Club`. Zatim dodaj polje unosa koje ima `id` od `club`, `className` od `input`, i `type` od `text`.
...
`div.form-group` trebalo bi da ima `label` sa karakteristikom elementa `htmlFor` od `club`, `className` od `label`, i tekstualnih podataka iz `Club`.
...
Tvoj klub za `form-group` treba da ima `input` sa `id` od `club`, `className` sa `input`, na `type` od `text`.
