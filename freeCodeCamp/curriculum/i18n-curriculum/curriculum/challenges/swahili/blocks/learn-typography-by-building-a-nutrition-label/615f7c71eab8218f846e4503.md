...
Nakon tvog poslednjeg `.divider`, kreiraj još jedan element tipa `p` sa tekstom `Trans Fat 0g`. Napiši reč `Trans` velikim slovima obuhvativši je unutar elementa tipa `i`. Dodeli novom elementu tipa `p` atribut `class` ako je postavljen na `indent no-divider`. Obuhvati `Trans Fat 0g` unutar elementa tipa `span` za poravnanje.

# --hints--

Treba da napraviš novi element `p` na kraju tvog elementa `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

Tvoja nova karakteristika `p` treba da ima tekst `Trans Fat 0g`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText === 'Trans Fat 0g');
```

Vaša nova karakteristika za `p` mora imati svojstvo `class` ako je postavljena na `indent no-divider`.
...
Tvoja nova komponenta sa `p` mora imati komponentu `i`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelector('i'));
```

Element `i` bi trebalo da obuhvati tekst `Trans`.
...
Vaš element za `span` mora da obuhvati tekst `Trans Fat 0g`.
