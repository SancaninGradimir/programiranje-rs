...
Element `address` ne zahteva fizičku geografsku lokaciju. Može se koristiti za pružanje poveznice/linka za temu.

Umotaj link oko teksta od `freeCodeCamp`, i podesi njegovo područje na `https://freecodecamp.org`.

# --hints--

Treba da dodaš element `a`.

```js
assert.exists(document.querySelector('address > a'));
```

Trebalo bi mu dodeliti atribut elementa `a` sa atributom elementa `href` sa vrednošću `https://freecodecamp.org`.

```js
assert.equal(document.querySelector('address > a')?.getAttribute('href'), 'https://freecodecamp.org');
```

Trebalo bi da zatvoriš deo `a` oko postojećeg teksta `freeCodeCamp`.
