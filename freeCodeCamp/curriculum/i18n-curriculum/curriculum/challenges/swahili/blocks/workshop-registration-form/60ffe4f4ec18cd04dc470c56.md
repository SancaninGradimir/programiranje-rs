...
Želiš da element `select` ostane na belom pozadinjama, ali sada ne odgovara elementima `min-height` niti elementima `input` i `textarea`.

Premesti svojstvo i vrednost `min-height` ili da sva tri tipa komponente dobiju jednaku vrednost od `min-height`, a komponenta `select` i dalje ima belu pozadinu.

# --hints--

Treba da prebacite svojstvo i vrednost od `min-height` na selektor `input, textarea, select`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight);
```

Trebalo bi mu dati birač za `input, textarea, select` `min-height` od `2em`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input, textarea, select')?.minHeight, '2em');
```

Trebaš ukloniti deklaraciju `min-height` iz selektora `input, textarea`.
