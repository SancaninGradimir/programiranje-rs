...
Drugi tekst je izmešten sa mesta. Pomerite ga na mesto tako što ćete selektoru `.first-paragraph::first-letter` dati svojstvo `float` postavljeno na `left` i svojstvo `margin-right` postavljeno na `1rem`.
