...
Zamenite postojeći tekst `cute cats` u anker element koji se povezuje sa:

`https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg`

# --hints--

Nova sidrena funkcionalnost mora da postoji u prvom elementu od `p`.
...
Svojstvo elementa `href` za novi jangkarni element trebalo bi biti `https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg`.

```js
assert.strictEqual(document.querySelector('a')?.href, "https://cdn.freecodecamp.org/curriculum/cat-photo-app/running-cats.jpg");
```

Trebalo bi da postoji samo jedan primer upotrebe `cute cats` u tvom kodu.

```js
assert.strictEqual(code.indexOf("cute cats"),code.lastIndexOf("cute cats")); 
```

Tekst komponente za `p` treba da ostane kao `Everyone loves cute cats online!`.
