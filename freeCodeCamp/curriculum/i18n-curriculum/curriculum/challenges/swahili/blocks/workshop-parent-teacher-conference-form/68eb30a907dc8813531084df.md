...
Podrazumevano, elementi forme poput radijskih dugmadi možda neće biti savršeno poravnati sa svojim etiketama, posebno ako je tekst etikete viši ili ima drugačija podešavanja fonta.

Da biste radijska dugmad vertikalno poravnali sa svojim etiketama, možete koristiti svojstvo `vertical-align`. Ovo svojstvo kontroliše kako se uline ili inline-blok elementi vertikalno poravnaju sa okolnim tekstom.

Sada, postavite `vertical-align` na `bottom` za radijska dugmad. Možete eksperimentisati sa drugim vrednostima kao što su `middle` ili `top` da biste videli kako utiču na poravnanje, ali `bottom` obično dobro radi za radijska dugmad pored etiketama.
