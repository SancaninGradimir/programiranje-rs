...
Mnogi widgeta unose svoje svojstva i vrednosti od CSS po defaultu za različite komponente. Ako pažljivo pogledate, možete primetiti da je fajl `input` manji od drugog teksta za komponente `input`. Po defaultu, `padding` od `1px 2px` je dostupan za komponente `input` u koje možete pisati.

Ako ponovo izaberete koristeći selektor svojstva elementa, pamuk `input` za `type` od `file` ili ima unutrašnji prostor jednak drugim elementima iz `input`.

# --hints--

Trebalo bi da koristite selektor svojstava elementa za stilizovanje elementa `input`.
