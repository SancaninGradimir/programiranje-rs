...
Nakon elementa `card-image-wrap`, dodajte `div` koji ima `className` od `card-name-strip`. Unutar njega, dodajte `span` koji ima `className` od `card-name` i tekst za `PELE`.

# --hints--

Trebalo bi da dodate `div` sa `className` od `card-name-strip`.

```js
assert.exists(document.querySelector('.card-image-wrap + .card-name-strip'));
```

Treba dodati `span` koji sadrži `className` od `card-name` unutar `.card-name-strip`.

```js
assert.exists(document.querySelector('.card-name-strip > span.card-name'));
```

Element za `.card-name` treba da ima tekst od `PELE`.
