...
`Bedtime` se odnosi na vreme kada osoba obično ide na spavanje. Često je povezano sa rutinama koje pomažu ljudima, posebno deci, da se pripreme za san. Na primer:

`Their bedtime is at 8, and they always read for a few minutes before turning off the lights.` - Ovde `bedtime` znači stalno vreme kada oni obično spavaju, a rečenica takođe pokazuje deo njihove večernje rutine.
...
Ova reč se odnosi na decu, ali kao objekat nad kojim se vrši radnja.
