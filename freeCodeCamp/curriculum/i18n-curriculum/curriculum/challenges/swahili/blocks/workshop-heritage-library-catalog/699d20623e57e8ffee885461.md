...
Isti obrazac važi i za ostela tri polja. Dodajte ekvivalentne provere za `author`, `year` i `location`.

Napomena da `isValid` akumulira kroz sve četiri provere: ako bilo koje polje ne uspe, postaje `false` i ostaje `false`, čak i kada provere ostatak prođu. To znači da funkcija uvek proverava svako polje, umesto da zaustavlja na prvom problemu.
