...
Vaš poslednji korak je da učinite vaše dugme za brisanje funkcionalnim. Dodajte slušač događaja za `click` na `clearCartBtn`. Za povratni poziv (callback), možete direktno proslediti `cart.clearCart`.

Međutim, to neće raditi, jer će kontekst od `this` biti atribut za `clearCartBtn`. Morate zatvoriti putanju `clearCart` za objekat od `cart`.

Možete ovo uraditi prosljeđivanjem `cart.clearCart.bind(cart)` kao povratni poziv.

Dakle, radionica za kupovanje automobila je gotova!

# --hints--

Treba da dodaš *event listener* na svoju komponentu za `clearCartBtn`.
...
Vaš slušalac događaja treba da primi `cart.clearCart.bind(cart)` kao povratni poziv.
