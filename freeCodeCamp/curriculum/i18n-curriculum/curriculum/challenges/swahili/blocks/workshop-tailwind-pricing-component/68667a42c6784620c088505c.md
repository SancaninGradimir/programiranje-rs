...
Malo od `Go Premium` CTA, sav sadržaj je smješten unutar `div`. To čini CSS mrežu odličnom opcijom za organizovanje svega.

Dodaj klase `grid`, `grid-rows-[auto_auto_auto_1fr]`, i na `gap-y-2` na `div` koje okružuju elemente `h2`, `p`, i `ul`.

# --hints--

Element `div` koji okružuje elemente `h2`, `p`, i `ul` treba da ima klasu `grid`.
...
Element tvog `div` koji okružuje elemente `h2`, `p` i `ul` treba da ima klasu `grid-rows-[auto_auto_auto_1fr]`.
...
Tvoj komponent za `div` koji okružuje elemente od `h2`, `p`, i `ul` treba da ima klasu od `gap-y-2`.
