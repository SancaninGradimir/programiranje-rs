...
Ispravićete HTML i ovde/na ovaj način. Postavite `textContent` od `cartSubTotal` na vrednost `subTotal` sa postavljenim dve decimalne mesta. Koristite sintaksu sekvence slova šablona da dodate dolar znak na početku vrednosti.
...
Trebalo bi da koristite sintaksu sekvence znakova šablona kako biste dodali dolar znak ($) pre vašeg poziva `.toFixed()`.
...
Treba da postaviš `textContent` elementa `cartSubTotal` u sekvencu tvog šablona.
