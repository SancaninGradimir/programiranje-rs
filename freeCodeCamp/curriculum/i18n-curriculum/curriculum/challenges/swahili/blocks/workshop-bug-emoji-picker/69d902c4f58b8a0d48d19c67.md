...
Sada popuni `bugMap` sa dva unosa. Dodaj `"bee"` kao ključ koji ukazuje na novi primer `Bee`, i `"spider"` kao ključ koji ukazuje na novi primer `Spider`. Izaberi `bugEmojiElement` kao argument za oba.

# --hints--

`bugMap` treba da ima vaše ulaze za `"bee"` postavljene na `new Bee(bugEmojiElement)`.

```js
assert.match(__helpers.removeJSComments(code), /bugMap\s*[^=]*=\s*\{[^}]*["']bee["']\s*:\s*new\s+Bee\s*\(\s*bugEmojiElement\s*\)/s);
```

`bugMap` bi trebalo da ima unose za `"spider"`, postavljene na `new Spider(bugEmojiElement)`.
