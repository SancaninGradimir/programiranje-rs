...
Pošto koristite drugi `div` element, moraćete da učinite ovo pristupačnijim za čitače ekrana.

U prethodnoj lekciji, naučili ste o atributu `aria-live`, koji se koristi za kreiranje živoog regiona na stranici. Ovo će omogućiti korisnicima čitača ekrana da budu automatski obavešteni kada se sadržaj živog regiona promeni, bez potrebe za ručnim fokusiranjem ili interakcijom sa njim.

U vašem `#status` elementu, dodajte atribut `aria-live` i postavite mu vrednost na `"polite"`. 

Vrednost `polite` će reći čitačima ekrana da sačekaju dok se bilo koja trenutna obaveštenja ne završi ili sve dok korisnik prestane da kuca, pre nego što najave ažuriranje.
