...
Kada kliknete na pesmu sa vaše plejliste, želite da pustite tu pesmu. Za to ćete morati dodati slušalicu događaja za događaj `click` na svaki element `button`, koji je dete elementa `song`.

Dopunite povratnu funkciju vašeg `forEach` da to uradite. Unutar povratne funkcije `addEventListener`, pozovite `playSong(n)`, gde je `n` ID pesme koji možete dobiti iz atributa `id` elementa `song`.

Napomena da svaki `song` ima atribut `id` sa vrednošću `song-n`: moraćete da dobijete deo `n` i konvertujete ga u broj pre nego što ga prosledite u `playSong()`.
