...
Čitanje ili skladištenje podataka u `localStorage` smatra se sporednim efektom u React, stoga bi trebalo da koristite `useEffect` za ovu logiku.

Počnite sa implementacijom rešenja strukture za `useEffect` iz React, kao što je urađeno sa `useState`.

# --hints--

Trebalo bi da implementirate rešenje dizajna iz `useEffect` na `React`.
