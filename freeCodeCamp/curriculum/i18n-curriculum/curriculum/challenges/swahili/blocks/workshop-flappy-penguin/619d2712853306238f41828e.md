...
Trebalo bi da date `:root` svojstvo `--penguin-skin`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(':root').getPropertyValue('--penguin-skin'));
```

Trebalo bi da date `--penguin-skin` vrednost `gray`, ali je pronađeno `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(':root').getPropVal('--penguin-skin', true), 'gray');
```

Trebalo bi da date `.penguin-head` `background` od `linear-gradient(45deg, var(--penguin-skin), rgb(239, 240, 228))`.
...
Trebalo bi da date `.penguin-body::before` `background-color` od `var(--penguin-skin)`, ali je pronađeno `--fcc-actual--`.
...
Trebalo bi da date `.arm` `background` od `linear-gradient(90deg, var(--penguin-skin), rgb(209, 210, 199))`.
