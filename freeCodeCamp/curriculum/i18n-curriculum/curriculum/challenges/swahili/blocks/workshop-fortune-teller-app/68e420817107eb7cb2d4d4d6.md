...
Vaš element `img` bi trebalo da ima `onerror` handler-a.
...
Vaš ``onerror`` handler bi trebalo da sadrži ``if`` statement koji proverava da li je `if` ``dataset.failed`` lažno.
...
`dataset.failed` treba da bude set do `'1'` unutar vašeg `if` bloka. Nemojte zaboraviti ključnu reč `this` ovde.
...
Vaš `if` blok bi trebalo da ažurira `this.src` na lokalnu default sliku. Ne zaboravite ključnu reč `this` ovde.
...
Trebalo bi da imate `else` blok koji uklanja `hidden` class. Ne zaboravite na `this` ključnu reč ovde.
...
Vaš `else` blok bi trebalo da set `previousElementSibling.style.display` do `'none'`. Ne zaboravite na `this` ključnu reč ovde.
