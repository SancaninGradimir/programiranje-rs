...
Sada nakon pokretanja provere `if`, koristite `e.target.value` da pretražite odgovarajući kvar u `bugMap` i pozovite njegovu putanju `render`. Nakon toga, trebalo bi da možete izabrati grešku i videti rezultate odmah.

Dakle, vaša radionica za odabir emodžija greške je završena!

# --hints--

Trebalo bi izvršiti poziv metode ``render`` na odgovarajuću grešku iz ``bugMap``.
