...
Za prvi kriterijum funkcije vašeg odgovora, otkrij parametre za `name`, `id`, `price`, i `category` iz poslatog primera `Dessert`.

# --hints--

Trebalo bi da koristite strukturalno rešenje za deklarisanje parametara `name`, `id`, `price`, i `category`. Za ovaj test, redosled je važan.

```js
assert.match(code, /\{\s*name\s*,\s*id\s*,\s*price\s*,\s*category\s*\}/);
```

Vaše rešenje dizajna mora biti prvi funkcionalni parametar odgovora.
