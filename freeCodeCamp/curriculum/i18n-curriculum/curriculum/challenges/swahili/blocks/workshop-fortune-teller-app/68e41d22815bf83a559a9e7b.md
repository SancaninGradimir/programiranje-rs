...
Vaš element `img` bi trebalo da ima hendler za `onload`.
...
Vaš `onload` handler bi trebalo da koristi `classList.remove` da ukloni `hidden` class. Ne zaboravite da vam je potreban `this` ključna reč ovde.
...
Vaš `onload` handler bi trebalo da set `previousElementSibling.style.display` do `'none'`. Ne zaboravite da vam je potreban `this` ključ za do this.
