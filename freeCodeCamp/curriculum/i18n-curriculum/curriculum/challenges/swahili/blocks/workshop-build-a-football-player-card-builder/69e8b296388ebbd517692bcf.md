...
Nastavite dodavanjem `div` od `className` od `form-row`. Unutar njega, dodajte dva elementa `div` koja imaju `className` od `form-group` za svaki.

Prvi element `div` treba da ima `label` od `className` od `label`, atribut elementa `htmlFor` od `position`, tekst od `Position`, i `input` sa `id` od `position`, `className` od `input` i `type` od `text`.

# --hints--

Vaš deo Player Info treba da ima `div` sa `className` od `form-row`.
...
`div.form-row` trebao bi imati dva elementa od `div`, svaki sa `className` iz `form-group`.
...
`form-group` tvoj prvi u redu treba da ima `label` sa `className` od `label` i `htmlFor` od `position`.
...
`label` vaš prostor bi trebalo da sadrži tekst od `Position`.
...
`form-group` Vaš prvi element u redu treba da ima `input` sa `className` od `input`, `id` od `position` i `type` od `text`.
