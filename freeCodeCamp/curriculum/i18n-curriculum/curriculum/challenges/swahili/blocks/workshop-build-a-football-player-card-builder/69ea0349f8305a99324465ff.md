...
U bloku `try`, kreiraj parametar za `saved`. Za vrednost, koristi metodu `getItem()` da bi dobio/dobila `STORAGE_KEY`. Direktno ispod tog parametra, kreiraj izjavu za `if` sa uslovom `saved`. Ostavi blok `if` prazan za sada.

# --hints--

Tvoja funkcija za `loadPlayer` mora da pozove `localStorage.getItem` i `STORAGE_KEY` unutar bloka `try`.

```js
assert.match(__helpers.removeJSComments(code), /localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Rezultati `localStorage.getItem` treba da se sačuvaju u parametar koji se zove `saved`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+saved\s*=\s*localStorage\.getItem\s*\(\s*STORAGE_KEY\s*\)/);
```

Blokki za `try` treba da ima stavku `if` sa uslovom `saved`.
