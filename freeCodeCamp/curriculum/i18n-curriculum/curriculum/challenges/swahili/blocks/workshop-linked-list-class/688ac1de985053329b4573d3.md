...
Zatim, dodeli `current_node.next` na `current_node` u `while` petlji.

Ovo pomera pokazivač `current_node` na sledeći čvor u listi tokom svake iteracije petlje, omogućavajući vam da traversirujete spajačnu listu sve dok se element ne pronađe ili do kraja liste.
