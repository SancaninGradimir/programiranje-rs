...
Kreiraj jedan konačni `@media` upit za `only screen` sa `max-width` od `420px`. Unutar toga, kreiraj `.hero-title` selektor sa svojstvom `font-size`, postavljeno na `4.5rem`.

Čestitam! Vaš magazin je sada završen.
