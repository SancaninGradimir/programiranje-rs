...
Napravi element sidra unutar prvog elementa od `li`. Postavi atribut elementa `a` sa vrednošću `href` za element `#` i tekst od `Dashboard`.

# --hints--

Treba da napraviš element `a` unutar prvog elementa liste.

```js
assert.exists(document.querySelector('li a'));
```

Vaš element od `a` treba da ima svojstvo elementa `href` postavljeno na `#`.

```js
assert.equal(document.querySelector('a')?.getAttribute('href'), '#');
```

Vaš element `a` treba da sadrži tekst od `Dashboard`.
