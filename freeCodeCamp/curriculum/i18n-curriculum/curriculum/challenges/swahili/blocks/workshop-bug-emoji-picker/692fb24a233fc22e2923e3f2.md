...
Nakon komponente, kreiraj metod za `render` bez implementacije koji ima tip povratne vrednosti `void`. Koristićeš ovo u malim klasama za ažuriranje prikazanog emodjija.

# --hints--

Tvoja klasa za `Bug` treba da ima metodu za `render` koja nema implementaciju.

```js
assert.match(__helpers.removeJSComments(code), /abstract\s+render\s*\(\s*\)/);
```

Tvoj put/način za `render` bi trebalo da ima tip vraćanja `void`.
