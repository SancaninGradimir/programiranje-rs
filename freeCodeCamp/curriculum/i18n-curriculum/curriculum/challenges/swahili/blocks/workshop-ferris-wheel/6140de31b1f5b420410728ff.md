...
Sada dodaj pravilo `@keyframes wheel` za selektor `100%`. Unutar toga, postavi/podesi `transform` na `rotate(360deg)`. Time se, vaša animacija će sada izvršiti punu rotaciju.

# --hints--

Vaše pravilo `@keyframes wheel` bi trebalo da ima selektor `100%`.
