...
Elementi mreže su veoma blizu levog i desnog ruba stranice, pa bi trebalo da radiš u prostoru između njih.

Primeni klasu `max-w-6xl` za `div` kako bi se postavila fiksna širina `72rem` i sprečilo da elementi budu previše razdvojeni na stranici.

Pošto će ovo sve potisnuti ulevo, dodaj `mx-auto` na `div` kako bi se komadi postavili u sredinu sa ravnomernim rasporedom.

# --hints--

Vaš element za `div` mora imati klasu `max-w-6xl`.
...
Tvoj element `div` treba da ima klasu `mx-auto`.
