...
Moraćete da prilagodite broj porcija koji se izračunava. Ovo je sjajna prilika za korišćenje druge kurirane funkcije.

Deklarišite funkciju `adjustForServings`, koja prima argument `baseQuantity`. Trebalo bi da vrati drugu funkciju koja prima argument `newServings`.

Unutrašnja funkcija treba da vrati rezultat množenja `baseQuantity` i `newServings`.
