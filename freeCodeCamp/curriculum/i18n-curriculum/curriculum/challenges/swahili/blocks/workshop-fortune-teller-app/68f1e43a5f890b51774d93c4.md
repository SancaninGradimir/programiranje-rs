...
U okviru metode `showFortune`` deklarisati varijablu nazvanu ``target``, dodeliti joj vrednost ``e.target`` i kreirati još jednu praznu `if` izjavu.

# --hints--

Trebalo bi da deklarišete varijablu pod nazivom `target` unutar `showFortune`.
...
Vaša varijabla `target` treba da bude dodeljena `e.target`.
...
Trebalo bi da imate prazan `if` izraz odmah nakon vaše `target` varijable.
