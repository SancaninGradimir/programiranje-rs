...
Unutar elementa sa `preview-panel`, dodaj komponentu za `p` koja je od `className` koja je od `preview-label` i tekst za `Live Preview`, još jedan element sa `p` koji je od `className` koji je od `preview-hint` i tekst za `Updates as you type`, i `div` koji ima `className` postavljen kao template literal `preview-box tier-${getPlayerTier(player.overallRating)}`.

Unutar poslednjeg `div`, prenesi deo `PlayerCard` sa njegovim parametrima.

# --hints--

Vaš element od `div.preview-panel` mora imati `p` sa `className` za `preview-label` i tekstom od `Live Preview`.
...
`div.preview-panel` treba da ima `p` sa `className` od `preview-hint` u tekstu `Updates as you type`.
...
`div.preview-panel` trebalo bi da ima `div` sa `className` od `preview-box` na mostu transformacije `className` koji ima prefiks od `tier-`.
...
`PlayerCard` trebalo bi da se prikaže unutar elementa `div` koji ima/sa `className` od `preview-box`.
