...
Ispod vašeg elementa `p`, dodajte još jedan element `p` sa tekstom `Dietary Fiber 4g`. Izaberite element `p`, svojstvo potrebnog elementa od `class` da unesete i uklonite granicu separatora. Zatim kreirajte separator ispod tog elementa `p`.
...
Tvoja nova funkcija za `p` treba da ima tekst od `Dietary Fiber 4g`.

```js
assert(document.querySelector('.daily-value.small-text p:last-of-type')?.innerText.match(/Dietary Fiber[\s|\n]+4g/));
```

Tvoja nova komponenta `p` mora imati svojstvo komponente `class` postavljeno na `indent no-divider`.
