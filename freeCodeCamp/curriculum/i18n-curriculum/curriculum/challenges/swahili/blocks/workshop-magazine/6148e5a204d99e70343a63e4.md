...
Svojstvo `gap` je skraćeni način za postavljanje vrednosti `column-gap` i `row-gap` istovremeno. Ako se da jedna vrednost, on postavlja i `column-gap` i `row-gap` na tu vrednost. Ako se daju dve vrednosti, on postavlja `row-gap` na prvu vrednost, a `column-gap` na drugu.

Dajte selektoru `.image-wrapper` svojstvo `gap` postavljeno na `2rem`.
