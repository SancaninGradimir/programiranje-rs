...
Ovo pokriva neke osnovne stilske pozicije za karticu elementa, uključujući dugme za CTA. Za preostali interni sadržaj, morate sve organizovati u mrežu (grid) ciljajući interni `div`.

Postavi klasu stavki `grid`, `grid-rows-[auto_auto_auto_1fr]`, na `gap-y-2` za `div` unutra.

# --hints--

Tvoj element ``div`` unutar porodičnog plana treba da ima klasu objekata ``grid``.
...
Komponenta `div` unutar porodičnog plana trebalo bi imati klasu objekata `grid-rows-[auto_auto_auto_1fr]`.
...
Element vaš za porodični plan treba da ima klasu objekata `gap-y-2` `div`.
