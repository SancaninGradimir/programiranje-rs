...
Nakon vašeg poslednjeg `.divider`, kreiraj novi element za `p` sa tekstom `Cholesterol 0mg 0%`. Zatim, obuhvati tekst od `Cholesterol` u elementu `span`, `0%` u drugom, i dodeli svako klasu `bold`.

Konačno, ubaciti u polje `span` sa tekstom `Cholesterol` zajedno sa tekstom `0mg`, u drugo polje `span` za upoređivanje.
...
Tvoj novi element `p` treba da ima tekst `Cholesterol 0mg 0%`.
...
Tvoja nova funkcija za `p` mora apsolutno da ima tri elementa od `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

Tekst za `Cholesterol` mora biti organizovan unutar `span`.
...
`Cholesterol` `span` treba da ima svojstvo od `class` postavljeno na `bold`.
...
Tekst za `0%` treba biti smješten unutar `span`.
...
`0%` `span` treba da ima svojstvo `class` postavljeno na `bold`.
...
`Cholesterol` `span` vaš sadržaj i vaši tekstovi od `0mg` bi trebalo da se obuhvati u `span`.
