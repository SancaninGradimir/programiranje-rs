...
Konačno, dugmad se mogu rasporediti sa boljim razmakom između njih i od vrha. Da biste to uradili, morate ciljati na element `div` koji ih obuhvata oba. Stoga, dodajte klase `flex`, `gap-2` i `mt-6` na taj `div`.

Dakle, tvoj modul za CTA sekciju je završen!

# --hints--

Element za `div` koji okružuje elemente sidra bi trebalo da ima servisnu klasu `flex`.
...
Element `div` koji okružuje komponente sidra treba da ima servisnu klasu od `gap-2`.
...
Komponenta za `div` koja okružuje sidrovske komponente mora imati klasu usluge `mt-6`.
