...
Unutar `thead`, kreiraj element od `tr`. Unutar njega, dodaj `td` i tri elementa od `th`. Unutar svakog elementa od `th`, dodaj element `span` sa atributom elementa `class` postavljenog na `sr-only` i sledeći tekst, u redosledu: `2019`, `2020`, i `2021`.
