...
Kada želite da korisnik izabere više opcija sa liste, možete koristiti čekboksove.

Evo primera kako raditi sa čekboksovima koji se tiču opcija hrane:
...
Atribut `value` koristi se za određivanje vrednosti koja će biti poslata na server kada se formular pošalje.

Ispod vašeg elementa `legend`, dodajte čekboks `input` sa atributima `id` i `value` podešenim na `"ads"`, a atribut `name` podešenim na `"choice"`.

Ispod vašeg čekboksa `input`, dodajte element `label` sa tekstom `Social Media Ads`. Atribut `for` bi trebalo da bude podešen na `"ads"`.
