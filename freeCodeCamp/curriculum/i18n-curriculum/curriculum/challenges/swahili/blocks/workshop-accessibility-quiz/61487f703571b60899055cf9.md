...
Postavi širinu elemenata `section` na `80%` njihovog roditeljskog kontejnera. Zatim, koristi granicu da ih centriruješ elemente `section`, dodajući `10px` na donju ivicu.

Pia, osiguraj da elementi za `section` ne mogu biti veći od `600px` po širini.

# --hints--

Trebalo bi da koristiš alat `section`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('section'));
```

Trebalo bi mu dati `section` `width` od `80%`.
