...
Ispod elementa `h1`, dodaj element `p` sa klasom `description`.

Unutar elementa za `p` unesite tekst `Click on the buttons below to rate your emotions.`

# --hints--

Trebalo bi da kreirate novi element za `p` ispod elementa `h1`.

```js
assert.exists(document.querySelector("h1 + p"));
```

Novi element od `p` treba da ima klasu objekata `description`.

```js
assert.equal(document.querySelector("h1 + p")?.className, "description");
```

Nova funkcija za `p` treba da ima tekst od `Click on the buttons below to rate your emotions.`.
