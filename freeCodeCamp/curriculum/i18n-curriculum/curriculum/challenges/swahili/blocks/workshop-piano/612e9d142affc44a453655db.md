...
Vreme je da stilizujemo same ključeve. Kreirajte selektor ``class`` za elemente ``.key``. Postavite svojstvo ``background-color`` na vrednost ``#ffffff``, svojstvo ``position`` na ``relative``, svojstvo ``width`` na ``41px``, i svojstvo ``height`` na ``175px``.
