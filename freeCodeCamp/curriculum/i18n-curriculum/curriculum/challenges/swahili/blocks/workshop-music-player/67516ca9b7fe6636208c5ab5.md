...
Na kraju, prosledite `false` kao drugi argument u poziv funkciji `playSong(userData.currentSong.id)`. Zatim, testirajte da li sve radi kako je očekivano tako što ćete pustiti pesmu, pauzirati je i zatim pritisnuti ili dugme za prethodnu ili dugme za sledeću.
