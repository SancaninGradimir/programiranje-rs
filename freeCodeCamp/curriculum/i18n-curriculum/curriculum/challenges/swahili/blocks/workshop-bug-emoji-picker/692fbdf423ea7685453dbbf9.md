...
Sada ćete početi da se osvrćate na potrebne komponente DOM.

Pronađi element pasusa sa identifikatorom `bug-emoji` i sačuvaj ga u konstantu nazvanu `bugEmojiElement`. Koristi operater validacije non-null iz `!` da potvrdiš da element nije `null`.

# --hints--

Trebalo bi da imaš nešto što se zove `bugEmojiElement`.
...
`bugEmojiElement` mora da upita element sa identifikatorom `bug-emoji` koristeći `document.querySelector` i parametar tipa `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /querySelector\s*<\s*HTMLParagraphElement\s*>\s*\(\s*['"]#bug-emoji['"]\s*\)/);
```

`bugEmojiElement` vaš treba da koristi ne-autentifikacioni drajver null (`!`).
