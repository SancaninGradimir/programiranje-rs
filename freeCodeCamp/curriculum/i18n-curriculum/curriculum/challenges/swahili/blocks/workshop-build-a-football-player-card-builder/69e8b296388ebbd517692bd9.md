...
Dodajte svojstvo elementa `value` za `player.club` za Klub `input`. Nakon toga, dodajte svojstvo elementa `onChange` koje je postavljeno kao funkcija strelica koja uzima `e` kao parametar i poziva/izvršava poziv `setPlayer` za širenje `player` i `club` postavljeno na `e.target.value`.

# --hints--

Tvoj klub `input` mora imati `value` kao klub igrača.
...
Vaš klub za `input` treba da ima handler za `onChange` koji pravi poziv ka `setPlayer` radi distribucije `player` i `club`, postavljen na `e.target.value`.
