...
Sada, morate dodati atribut `aria-current` trenutno reprodukovanoj pesmi.

Proverite da li postoji `songToHighlight`. Ako jeste, koristite metodu `setAttribute` na `songToHighlight` i prosledite `"aria-current"` i `"true"` kao prvi i drugi argument.
