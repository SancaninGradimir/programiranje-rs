...
Sada, promeni rutinu `for` i deklaraciju `if` koju si dodao u prethodnom koraku, i deklaraciju `if` koja koristi funkciju unutar `any()`.

# --hints--

Uslov za vašu novu deklaraciju za `if` mora biti poziv na `any()`.
...
Treba da prosledite pridevni izraz kao argument za vaš poziv od `any()`.
...
Izjava generatora koja se prenosi za `any()` treba da ponovi preko `args`.
...
Izjava za `if` mora proveriti da li bilo koji argument u `args` nije primer od `int` ili `float`.
...
Trebalo bi da koristiš niz karaktera `TypeError` unutar svog novog izraza `if`.
