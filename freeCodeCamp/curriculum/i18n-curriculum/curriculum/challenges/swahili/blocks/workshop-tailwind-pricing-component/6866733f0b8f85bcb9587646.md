...
Premium karta za cenu treba da bude ona koja se najviše ističe na stranici. Takođe, to je jedina karta koja će imati oznaku `Most popular`. Da bi se ova karta i njena oznaka istakle, možete koristiti Tailwind klase za postavljanje razmaka (spacing) za odnos i povećanje veličine.

Ovo je kako možete učiniti da element raste koristeći povećanje veličine u Tailwind CSS:
...
Vi ćete se baviti pozicioniranjem u sledećim koracima. Trenutno, postavite klasu `scale-105` na poklopac `div` kako bi bio malo veći od ostalih kartica sa cenama.

# --hints--

Element vašeg pokrića za `div`, namenjen premijum paketu, mora imati klasu od `scale-105`.
