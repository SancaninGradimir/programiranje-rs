...
Povećaj ponavljajući gradijent boje na ovoj zgradi; učini ga istim kao onaj koji si nedavno dodao, osim što ne povećavaš smer `90deg` i koristi boju prozora umesto dve boje za `transparent`.

# --hints--

Treba da mu/jadi `.fb6` `repeating-linear-gradient` u kontekstu `background`.
...
Treba da mu/jaj daje (ili: trebaš dati) `repeating-linear-gradient` od prve boje `--building-color3` od `0%` do `10%`.
...
Trebalo bi mu dati `repeating-linear-gradient` od druge boje od `--window-color3` od `10%` do `30%`.
