...
Postavi element `form` u centar, dajući `margin` od `0 auto`. Zatim, prilagodi mu veličinu tako da dobije gornju širinu od `500px` i donju širinu od `300px`. Unutar tog opsega, dozvoli mu da ima `width` od `60vw`.
...
Trebalo bi mu dati `form` `min-width` od `300px`.
