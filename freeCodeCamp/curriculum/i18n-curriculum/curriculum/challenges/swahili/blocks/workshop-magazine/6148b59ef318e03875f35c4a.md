...
Kreiraj selektor `img` i dodeli mu svojstvo `width` postavljeno na `100%`, i svojstvo `object-fit` postavljeno na `cover`.

Svojstvo `object-fit` govori pretraživaču kako da pozicionira element unutar svog kontejnera. U ovom slučaju, `cover` će postaviti sliku tako da popuni kontejner, sekući po potrebi kako bi se izbegle promene aspektnog odnosa.
