...
Sada morate definisati svojstva koja će sve greške imati.

Unutar klase `Bug`, deklarisanje svojstva pod nazivom `emoji` tipa `T` koristeći operator za potvrdu distribucije vrednosti (`!`). Ovo govori TypeScript da će to svojstvo dobiti vrednost kasnije.

# --hints--

Vaša klasa od `Bug` mora imati svojstvo nazvano `emoji`.
...
Tvoja vrijednost za `emoji` mora biti tip `T`.
...
Svojstvo vaše `emoji` treba da koristi operator za validaciju distribucije vrednosti (`!`).
