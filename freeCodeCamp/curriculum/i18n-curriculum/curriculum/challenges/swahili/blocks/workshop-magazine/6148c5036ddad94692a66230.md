...
Sada će algoritam automatskog postavljanja početi da radi kada dodate novi element ikone. Međutim, algoritam postavlja podrazumevanu širinu nove kolone na `auto`, što neće odgovarati vašim trenutnim kolonama.

Možete ovo prepisati sa svojstvom `grid-auto-columns`. Dajte selektoru `.social-icons` svojstvo `grid-auto-columns` postavljeno na `1fr`.
