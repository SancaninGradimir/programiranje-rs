...
`label` prvi treba da ima tekst `Enter Your First Name:`.
...
`label` treći deo bi trebalo da ima tekst `Enter Your Email:`.

```js
assert.match(document.querySelector('fieldset > label:nth-child(3)')?.innerHTML, /Enter Your Email:/i);
```

`label` četvrta treba da ima tekstualni sadržaj `Create a New Password:`.
