...
Trebalo bi da deklariš varijablu pod imenom `el` unutar funkcije `getElement`.
...
Vaša varijabla `el` treba da dobije vrednost `document.querySelector<T>(selector)`.
...
Trebalo bi da imate izjavu if koja proverava da li if `el` nije pronađen.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!el\s*\)/);
```

Treba vam da throw a new Greška sa porukom `Element not found: ${selector}` unutar vaše `if` izjave.

```js
assert.match(__helpers.removeJSComments(code), /throw\s+new\s+Error\(\s*`Element not found:\s*\$\{selector\}`\s*\)/);
```

Trebalo bi da return varijablu `el` unutar funkcije getElement.
