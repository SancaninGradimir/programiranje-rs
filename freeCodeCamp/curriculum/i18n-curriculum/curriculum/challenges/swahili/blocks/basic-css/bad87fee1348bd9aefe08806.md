...
title: Koristi CSS Klasu za Stilizovanje Vise Elemenata
challengeType: 0
...
Klase vam omogucavaju da primenite isti CSS stil na vise HTML elemenata. To mozete videti tako sto cete svoju `red-text` klasu primeniti na prvi `p` element.
...
Vas prvi `p` element treba da bude crven.
...
Vasi drugi i treci `p` elementi ne treba da budu crveni.
...
Vas prvi `p` element treba da ima klasu `red-text`.
