...
Sledeći, dodajte još jedan checkbox `input` sa atributima `id` i `value` podešenim na `"location"`, a atribut `name` podešenim na `"choice"`.

Za element `label`, tekst za `Location` i atribut `for` treba da budu podešeni na `"location"`.

Ispod elementa `label`, dodajte još jedan checkbox `input` sa atributima `id` i `value` podešenim na `"reputation"`, a atribut `name` podešenim na `"choice"`.

Za element `label`, tekst za `Reputation` i atribut `for` treba da budu podešeni na `"reputation"`.
