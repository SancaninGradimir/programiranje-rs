...
Kada se premestite na element `span` sa tekstom `Soundflow`, sadržaj teksta treba da bude velikim slovima.

Ovde je primer koji pokazuje kako da se koristi servisni razred za `uppercase`:

```html
<p class="uppercase">Quincy Larson</p>
```

Dodaj klasu `uppercase` na element `span`.

# --hints--

Tvoja komponenta za `span` treba da ima servisnu klasu `uppercase`.
