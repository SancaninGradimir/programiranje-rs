...
Da biste rasporedili polja za unos na jednoj liniji, kreirajte novo pravilo koje cilja sve elemente od `input` i `label` unutar elementa `.info`, i postavite atribut za `display` na `inline-block`.

Takođe, formatiraj tekst elementa `label` sa desne strane.

# --hints--

Trebalo bi da koristiš alat za `.info > label, .info > input` ili `.info label, .info input`.
...
Treba da mu date atribute za `input` i `label` sa svojstvom `display` sa vrednošću `inline-block`.
...
Treba mu dodeliti element/atribut `label` svojstvo `text-align` sa vrednošću `right`.
