...
Trebalo bi da date tački za prolaz (waypoint) `30%` u rasponu od `transform` do `rotate(110deg) scaleX(-1)`.
...
Trebalo bi da date tački za navigaciju (waypoint) `40%` na `transform` od `rotate(130deg) scaleX(-1)`.
