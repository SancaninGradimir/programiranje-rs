...
Rasporedite šest elemenata `div` unutar `.foreground-buildings` i dodelite im klase od `fb1` do `fb6` u tom redosledu. "fb" znači "prednji objekat". Ovo će biti šest dodatnih objekata napred.

# --hints--

Treba da kreiraš novi `div` sa klasom `fb1`.

```js
assert.exists(document.querySelector('div.fb1'));
```

Treba da kreiraš novi `div` sa klasom `fb2`.

```js
assert.exists(document.querySelector('div.fb2'));
```

Trebalo bi da kreirate novi `div` sa klasom `fb3`.

```js
assert.exists(document.querySelector('div.fb3'));
```

Treba da kreiraš novi `div` sa klasom `fb4`.

```js
assert.exists(document.querySelector('div.fb4'));
```

Trebalo bi da kreirate novi `div` sa klasom `fb5`.

```js
assert.exists(document.querySelector('div.fb5'));
```

Trebalo bi da kreirate novi `div` sa klasom `fb6`.

```js
assert.exists(document.querySelector('div.fb6'));
```

Treba da postaviš ove nove elemente za `div` unutar elementa `.foreground-buildings`.
