...
Nakon izjave `if` u vašoj metodi `__str__`, kreirajte dve varijable pod nazivima `movies` i `series` i dodelite im, respektivno, poziv metode `get_movies` i poziv metode `get_tv_series`.
