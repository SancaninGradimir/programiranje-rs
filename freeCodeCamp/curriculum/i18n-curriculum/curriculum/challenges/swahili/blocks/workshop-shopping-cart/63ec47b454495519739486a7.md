...
Potrebno je prikazati proizvode dostupne u vašem HTML. Počnite korišćenjem operatora za konkatenaciju da dodate praznu sekvencu karaktera na atribut `innerHTML` varijable `dessertCards`.

# --hints--

Treba vam pristup atributu ``innerHTML`` varijable ``dessertCards``.

```js
assert.match(code, /dessertCards\.innerHTML/);
```

Trebalo bi da koristiš operator za sabiranje na atribut `innerHTML`.
