...
Ako `userData.currentSong` nije `null`, moraš pronaći sledeću pesmu za reprodukciju. Zapamti da možeš koristiti `getNextSong` za to.

Popuni svoju funkciju `playNextSong` tako da reprodukuje sledeću pesmu u plejlisti. Ako je trenutna pesma poslednja, postavi `userData.currentSong` na `null`, `userData.songCurrentTime` na nulu i pozovi funkciju `pauseSong`.
