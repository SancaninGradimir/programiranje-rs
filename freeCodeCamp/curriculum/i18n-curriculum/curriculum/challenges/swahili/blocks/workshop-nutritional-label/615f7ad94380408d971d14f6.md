...
Pseudo-selektor ``:not`` se može koristiti za selekciju svih elemenata koji ne odgovaraju datoj CSS pravilu.
...
Prethodno selektuje sve elemente ``div`` bez ``id`` od ``example``.

Izmenite svoj selektor ``.daily-value p`` kako biste isključili elemente ``.no-divider``.
