...
Prvo, kreiraj duboku kopiju niza ulaznih podataka `crew` koristeći `slice()` i postavi njegovu vrednost u parametar nazvan `sorted`.

# --hints--

Trebalo bi da koristiš metodu `slice()` za kopiranje redne podatke unosa `crew` u novo polje nazvano `sorted`.
