...
Klasa objekata ima specijalnu metodu ``constructor``, koja se poziva kada se kreira nova instanca te klase. Metoda ``constructor`` je dobro mesto za inicijalizovanje parametara klase. Evo primera klase sa metodom ``constructor``:
...
Dodaj praznu rutu za `constructor` u klasi `ShoppingCart`.

# --hints--

Trebalo bi da dodaš putanju `constructor` u klasu `ShoppingCart`.

```js
assert.match(code,  /class\s+ShoppingCart\s*\{\s*constructor\s*\(\s*\)\s*/)
```

Tvoj put/putanja za `constructor` mora biti prazan.
