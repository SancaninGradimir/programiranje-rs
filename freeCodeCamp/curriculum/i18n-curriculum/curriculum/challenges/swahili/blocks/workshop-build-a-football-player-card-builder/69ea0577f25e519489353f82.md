...
U bloku `catch`, napišite sekvencu karaktera `Failed to load player data, using defaults:` praćenu parametrom `error` na konzoli.

Nakon izlaska iz `catch`, ali i dalje unutar funkcije `loadPlayer`, vrati `defaultPlayer`.

Sve ovo što učita podatke igrača, popunjavanje u UI-u i učitavanje `defaultPlayer` ako proces ne uspe.

# --hints--

Tvoj blok za `catch` treba da ispisa `'Failed to load player data, using defaults:', error` u konzolu.

```js
assert.match(__helpers.removeJSComments(code), /console\.log\s*\(\s*['"`]Failed to load player data, using defaults:\s*['"`],\s*error\)/);
```

Vaša funkcija `loadPlayer` treba da vrati `defaultPlayer` kao alternativnu vrednost.
