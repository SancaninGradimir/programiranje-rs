...
Kada želite da korisnici izvrše odabir iz padajućeg menija, možete koristiti elemente `select` i `option`. 

Evo primera korišćenja elemenata `select` i `option` za kreiranje padajućeg menija za različite gradove:
...
Počnite dodavanjem elementa `select` sa atributima `name` i `id` podešenim na `"service"`.
