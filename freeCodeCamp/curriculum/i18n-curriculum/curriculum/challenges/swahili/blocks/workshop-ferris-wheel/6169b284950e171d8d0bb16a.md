...
Trebalo bi da kreirate selektor new `75%` u vašoj `@keyframes cabins` pravilu.
...
Vaš selektor `75%` treba da bude između vašeg `50%` i `100%` selektora.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[3]?.keyText, '75%');
```

Vaš `75%` selektor bi trebalo da ima `background-color` svojstvo set do `yellow`.
