...
Znak za kvačicu u svakom `span` na početku stavke liste je dekorativan. Dodajte `aria-hidden` za ove stavke od `span` kako biste ih sakrili od čitača ekrana.

Pia, dodeli klase `text-green-700` na `mr-2` za svaki element od `span`.

# --hints--

Vaša dva elementa za `span` treba da imaju svojstvo od `aria-hidden`.
...
Vaša dva elementa od `span` moraju imati klasu `text-green-700`.
...
Dva vaša elementa sa `span` treba da imaju klasu `mr-2`.
