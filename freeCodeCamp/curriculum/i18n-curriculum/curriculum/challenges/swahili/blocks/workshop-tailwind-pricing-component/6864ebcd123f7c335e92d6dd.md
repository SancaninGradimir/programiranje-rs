...
Sada moraš da radiš na poziciji objekata unutar tog plana.

Dodajte servisnu klasu ``rounded-xl`` za uklanjanje oštrih uglova podnožja, ``p-8`` za dodavanje unutrašnjeg razmaka (paddinga), i ``gap-6`` za kreiranje razmaka između sadržaja i dugmeta poziva na akciju (CTA).

# --hints--

Vaš element `div` mora imati klasu `rounded-xl`.
...
Tvoj element za `div` mora imati klasu `p-8`.
...
Element vašeg `div` mora imati klasu `gap-6`.
