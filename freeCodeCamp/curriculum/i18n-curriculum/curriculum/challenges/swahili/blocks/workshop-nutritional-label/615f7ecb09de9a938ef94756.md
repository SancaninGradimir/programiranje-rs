...
Ispod vašeg poslednjeg `p` elementa, dodajte još jedan `p` element sa tekstom `Total Carbohydrate 37g 13%`. Kao i pre, koristite `span` elemente da učinite tekst `Total Carbohydrate` i `13%` podebljanim. Zatim, obuhvatite hranljivostinost i količinu u `span` za ponovno poravnanje.
