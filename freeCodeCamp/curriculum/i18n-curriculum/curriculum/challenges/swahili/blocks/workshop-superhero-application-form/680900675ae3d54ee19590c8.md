...
Sledeće, kreiraj `label` sa `className` od `section` na `column`, uz sadržaj `How did you get your powers?`. Unutar `label`, kreiraj komponentu za `select`.
...
Vaš element za `label` mora da sadrži tekst od `How did you get your powers?`.
...
Trebaće ti da napraviš komponentu `select` unutar svoje komponente `label`.
