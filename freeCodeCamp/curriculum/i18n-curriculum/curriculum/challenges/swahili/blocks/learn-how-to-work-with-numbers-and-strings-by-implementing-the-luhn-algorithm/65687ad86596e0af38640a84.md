...
Koristi petlju `for` da ponovi preko svakog broja u listi `odd_digits`. Premesti svoj poziv `print` iz prethodnog koraka unutar petlje `for` i modifikuj ga da ispiše svaki broj.

# --hints--

Trebalo bi da koristiš krug `for` preko `odd_digits`.
...
Trebalo bi da imaš `--fcc-expected--` unutar kruga `for`.
