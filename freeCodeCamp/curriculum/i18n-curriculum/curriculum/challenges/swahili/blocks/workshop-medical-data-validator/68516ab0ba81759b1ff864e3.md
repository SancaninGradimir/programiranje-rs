...
Funkcija `all` vraća `True` ako su svi elementi iz iterabilnog objekta prosleđenog joj istiniti (truthy), a `False` u suprotnom:
...
Prosledite listu `[isinstance(i, str) for i in medications]` funkciji `all` kako biste osigurali da je svaki njen element string.
