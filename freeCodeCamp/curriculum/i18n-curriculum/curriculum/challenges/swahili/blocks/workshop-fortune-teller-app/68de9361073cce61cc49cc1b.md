...
Trebalo bi da imate strelicastu funkciju pod imenom `getElement`.
...
Vaša funkcija `getElement` treba da primi parametar pod imenom `selector` tipa `string`.
...
Generički tipski parametar `T` treba da nasledi tip `HTMLElement`.
...
Vaša `getElement` funkcija bi trebalo da ima return tip od `T`.
