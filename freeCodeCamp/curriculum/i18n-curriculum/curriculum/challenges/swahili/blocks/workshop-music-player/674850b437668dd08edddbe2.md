...
U prethodnim koracima ste izgradili funkcionalnost za puštanje pesme. Sada morate dodati tu funkcionalnost na dugme za reprodukciju.

Koristite metod `addEventListener()` na `playButton` i prosledite događaj `"click"` kao prvi argument. Za sada, koristite povratni poziv (callback) koji poziva `playSong(0)` kao drugi argument.

Zatim ga testirajte klikom na dugme za reprodukciju.
