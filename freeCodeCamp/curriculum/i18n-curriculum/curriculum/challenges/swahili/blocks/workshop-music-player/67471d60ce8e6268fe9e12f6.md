...
U ovoj radionici ćete kreirati aplikaciju za reprodukciju muzike koja je sposobna da pušta, pauzira i preskače pesme sa plejliste.

HTML i CSS ovog projekta obezbeđeni su za vas, tako da se možete fokusirati na JavaScript.

Počnite pristupanjem elementima `#playlist-songs`, `#play` i `#pause` i dodeljivanjem istih promenljivama `playlistSongs`, `playButton` i `pauseButton`, respektivno.
