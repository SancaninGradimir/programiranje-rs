...
Ne menja karakteristike konektora kada se konektor klikne koristeći <dfn>lažni izbori</dfn> koji izgleda kao `a:active { propertyName: propertyValue; }`.

Promeni boju konektora u ćelijskom `Visit our website` kada se klikne na `white`.

# --hints--

Trebalo bi da koristiš lažni detektor `a:active`.
...
Trebalo bi da postaviš atribut `color` na `white`.
...
`a:active` treba da ima `color` od `white`.
