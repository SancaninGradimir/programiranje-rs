...
Dodajte svojstvo `value` elementu PAS `input` i postavite ga na `player.pas`. Zatim dodajte svojstvo `onChange` postavljeno na streličastu funkciju koja prima `e` kao argument i poziva `setPlayer` šireći `player` i postavljajući `pas` na `Number(e.target.value)`.
