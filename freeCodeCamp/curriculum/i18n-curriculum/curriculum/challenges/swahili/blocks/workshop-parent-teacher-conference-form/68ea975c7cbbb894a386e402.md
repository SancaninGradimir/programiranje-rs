...
Sledeći korak je dodavanje elemenata `label` i `input` za metodu kontakta putem e-pošte.

Počnite dodavanjem etikete sa klasom `"contact-method"` i atributom `for` postavljenim na `"email"`. Tekst etikete treba da bude `Email: `.

Ispod vašeg elementa `label`, dodajte radio dugme sa atributima `id` i `value` postavljenim na `"email"`. Atribut `name` treba da bude postavljen na `"contact-method"`, a klasa na `"contact-method-radio-btn"`.

Napokon, uverite se da je ovo radio dugme označeno po podrazumevanju.
