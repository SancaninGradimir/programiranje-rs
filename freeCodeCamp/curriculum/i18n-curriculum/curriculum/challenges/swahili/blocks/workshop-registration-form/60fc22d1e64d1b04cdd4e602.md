...
Dodaj razmak između elemenata `.inline` i teksta `label`, dajući im desajno poravnanje od `margin` za `0.5em`. Takođe, postavi sve ostale ivice na `0`.
...
Trebalo bi da im date elemente `.inline` `margin-right` od `0.5em`.
...
Trebalo bi da im date elemente `.inline` `margin-left` iz `0`.
