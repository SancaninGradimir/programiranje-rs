...
Predaja obrasca kada je izabran izbor ne šalje smislenu vrednost na server. Stoga, svaki element od `option` zahteva atribut `value`. Bez ovog atributa elementa, tekstualni sadržaj od `option` će biti poslat na server.

Omogući `option` od `value` za prvi `""`, i svojstva elementa `option` za komponente `value` koje slede od `1` do `4`.

# --hints--

Trebalo bi da napišete `option` za `value` za prvi `""`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.value, '');
```

Treba da dostavite `option` od `value` za `1` drugi.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.value, '1');
```

Trebalo bi da izvučeš `option` od `value` za treći `2`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[2]?.value, '2');
```

Trebalo bi da izvučeš `option` od `value` za `3`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[3]?.value, '3');
```

Trebalo bi da izvučeš `option` od `value` za peto `4`.
