...
Ako malo eksperimentiš sa svojim plejerom, primetićeš nekoliko grešaka. Prvo, kada pustiš pesmu, pauziraš je, a zatim pritisneš ili dugme za prethodnu ili sledeću, pesma se neće pokrenuti od početka.

Da bi počeo da popravljaš to, dodaj drugi parametar nazvan `start` u svoju funkciju `playSong`. Daj mu podrazumevanu vrednost `true`.
