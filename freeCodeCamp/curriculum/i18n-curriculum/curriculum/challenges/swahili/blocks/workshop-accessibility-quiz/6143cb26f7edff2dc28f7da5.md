...
Svaki `fieldset` će biti pitanje istina/laž.

Unutar svakog `fieldset`, uključite jedan element od `legend` i jedan element od `ul` sa dve opcije.

# --hints--

Treba da ga staviš u jedan element od `legend` unutar prvog elementa od `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > legend')?.length, 1);
```

Trebalo bi da smestiš unutar jednog elementa `ul`, unutar prvog elementa `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(1) > fieldset > ul')?.length, 1);
```

Morate postaviti dva elementa `li` unutar prvog elementa `ul`.

```js
assert.equal(document.querySelectorAll('fieldset > ul')?.[0]?.querySelectorAll('li')?.length, 2);
```

Treba da ga postavite u prvi element od `legend` unutar drugog elementa od `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > legend')?.length, 1);
```

Trebalo bi da smestiš unutar jednog elementa `ul` unutar drugog elementa `fieldset`.

```js
assert.equal(document.querySelectorAll('.question-block:nth-of-type(2) > fieldset > ul')?.length, 1);
```

Treba da postavite dva elementa od `li` unutar drugog elementa od `ul`.
