...
Odeljak sa informacijama o igraču je sada završen. Nakon zatvarajuće oznake `div` odeljka „Player Info“, ali i dalje unutar `form-panel`, dodajte novi `div`. Unutar njega dodajte element `p` sa `className` vrednošću `form-section-title` i tekstom `Player Stats`. Zatim dodajte `div` sa `className` vrednošću `stats-grid`. Za sada ostavite mrežu statistike praznom.
...
Odeljak „Player Stats“ treba da sadrži `p` sa `className` vrednošću `form-section-title` i tekstom `Player Stats`.
...
Odeljak „Player Stats“ treba da sadrži `div` sa `className` vrednošću `stats-grid`.
