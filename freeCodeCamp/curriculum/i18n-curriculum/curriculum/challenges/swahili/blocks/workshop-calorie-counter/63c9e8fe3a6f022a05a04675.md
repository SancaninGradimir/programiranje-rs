...
Uzevši u obzir istu sintaksu za unos vrednosti, dodajte drugi element ``p`` sa tekstom ``consumedCalories Calories Consumed`` i treći sa tekstom ``exerciseCalories Calories Burned``. Zapamtite da promenite vaše parametre za ``consumedCalories`` i ``exerciseCalories`` u odgovarajuće vrednosti.

# --hints--

Trebalo bi da dodaš drugi element `p` u svoj template string.
...
Tvoj drugi element za `p` treba da bude na novom redu.
...
Tvoja druga sekcija za `p` mora imati tekst `${consumedCalories} Calories Consumed`.
...
Trebalo bi da dodaš treći element za `p` u vaš tekst šablona.
...
Tvoj treći element od `p` bi trebalo da ima tekst `${exerciseCalories} Calories Burned`.
