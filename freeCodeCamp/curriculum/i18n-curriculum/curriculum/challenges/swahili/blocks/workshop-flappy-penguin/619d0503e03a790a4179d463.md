...
Trebalo bi da date `.chin` svojstvo `top`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.top);
```

Trebalo je da date `.chin` a `top` od `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.top, '25%');
```

Trebalo bi da date `.chin` svojstvo `left`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.chin')?.left);
```

Trebalo bi da date `.chin` a `left` od `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.chin')?.left, '5%');
```

Trebalo bi da date `.chin` a `border-radius` od `70% 70% 100% 100%`.
