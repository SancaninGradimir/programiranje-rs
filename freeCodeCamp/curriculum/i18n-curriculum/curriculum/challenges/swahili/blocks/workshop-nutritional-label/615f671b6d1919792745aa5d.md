...
Kreiraj novi `div` element ispod tvog `.medium` elementa. Daj mu atribut `class` postavljen na `daily-value small-text`. Unutar ovog novog `div`, dodaj `p` element sa tekstom `% Daily Value *`, i postavi atribut `class` na `bold right`.
