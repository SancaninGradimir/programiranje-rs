...
1. Treba da kreiraš `type` pod nazivom `Category` sa sledećim mogućim vrednostima: `'Sport'`, `'Cruiser'`, `'Touring'`, `'Dirt'`, `'Adventure'`, `'Naked'`, `'Electric'`.
2. Treba da kreiraš `interface` pod nazivom `Motorcycle` sa sledećim svojstvima: `id` tipa `string`, `name` tipa `string`, `manufacturer` tipa `string`, `category` tipa `Category`, `price` tipa `number`, `image_url` tipa `string`, `created_at` tipa `Date`, `description` tipa `string` i `year` tipa `number`.
3. Treba da kreiraš funkciju pod nazivom `fetchMotorcycles` koja vraća `Promise` koji se razrešava u niz objekata tipa `Motorcycle`.
4. Treba da učitaš podatke o motociklima sa adrese `https://cdn.freecodecamp.org/curriculum/labs/data/motorcycles.json`.
5. Treba da kreiraš funkciju pod nazivom `renderMotorcycleCard` koja prima jedan objekat tipa `Motorcycle` kao argument i vraća string koji predstavlja HTML za karticu motocikla.
1. Funkcija `renderMotorCycleCard` treba da prima parametar pod nazivom `motorcycle` tipa `Motorcycle`.
2. Funkcija `renderMotorCycleCard` treba da vraća vrednost tipa `string`.
3. Svaka kartica motocikla treba da sadrži element `img` sa ispravnom slikom i klasom `motorcycle-card-image-container`.
4. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-year-badge` koji prikazuje godinu kao običan tekst.
5. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-title` koji prikazuje naziv motocikla kao običan tekst.
6. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-manufacturer` koji prikazuje naziv proizvođača kao običan tekst.
7. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-category` koji prikazuje kategoriju motocikla kao običan tekst.
8. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-description` koji prikazuje opis motocikla kao običan tekst.
9. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-price` koji prikazuje cenu motocikla kao običan tekst.
10. Svaka kartica motocikla treba da sadrži element sa klasom `motorcycle-card-engine` koji prikazuje snagu motora u konjskim snagama.
6. Treba da postoji klasa pod nazivom `MotorcycleGalleryApp`.
1. Ta klasa treba da ima privatni niz pod nazivom `allMotorcycles` tipa niz objekata `Motorcycle`.
2. Ta klasa treba da ima funkciju pod nazivom `renderMotorcycles` koja prikazuje kartice motocikala iz niza `allMotorcycles` unutar elementa sa identifikatorom `motorcycle-grid`.
7. Broj prikazanih elemenata `Motorcycle` treba da bude naveden unutar elementa sa identifikatorom `results-number`.
