...
Kada su svi argumenti u pozivu funkcije ključni argumenti, njihov redosled nije važan.

Da biste ovo potvrdili, pokušajte da zamenite redosled `length=8` sa `nums=1` u vašem pozivu funkcije.

# --hints--

Trebalo bi da promeniš redosled `length=8` i `nums=1` u pozivu `generate_password()`.
