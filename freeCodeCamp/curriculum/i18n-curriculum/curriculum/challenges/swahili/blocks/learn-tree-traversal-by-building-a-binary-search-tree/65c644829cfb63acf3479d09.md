...
Postavite čvor u listu `nodes` unutar primjera `bst` da iterirate kroz listu i pozovete metodu `insert` za svaki čvor u listi. Koristite `node` kao parametar iteracije.

# --hints--

Trebalo bi da imaš krug od `for node in nodes`.
...
Treba ti da imaš `bst.insert(node)` unutar kruga `for`.
