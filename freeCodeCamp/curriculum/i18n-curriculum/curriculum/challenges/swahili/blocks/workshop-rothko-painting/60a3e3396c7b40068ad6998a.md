...
Postavi element `.three` u sredinu crtačkog platna postavljanjem njegovog `margin` na `auto`.

# --hints--

Morate postaviti svojstvo `margin` na `auto`.
...
Vaše polje `.three` treba da ima vrednost `margin` za `auto`.
