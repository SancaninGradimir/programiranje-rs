...
Da bi dugme za slanje izgledalo drugačije, dodajte `background-color` od `#3b3b4f`, i `border-color` od `white`.

# --hints--

Trebalo bi mu dati dugme za slanje `background-color` od `#3b3b4f`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.backgroundColor, 'rgb(59, 59, 79)');
```

Trebalo bi da mu/judi pružite dugme za slanje `border-color` od `white`.
