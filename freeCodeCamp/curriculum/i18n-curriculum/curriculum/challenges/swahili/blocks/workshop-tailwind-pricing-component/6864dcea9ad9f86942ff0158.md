...
Vreme je da radimo na funkciji koja okružuje tri kartice sa cenama `div`. Da biste rasporedili te kartice, možete koristiti Tailwind grid utilitetske klase CSS.

Ovo je kako možeš postaviti vrednost prikaza za `grid` element koristeći Tailwind:
...
Postavite klasu na `grid` za element `div` koji okružuje kartice sa cenama.

# --hints--

Vaš element za `div` treba da ima klasu `grid`.
