...
Sledeći deo obrasca će korisnicima omogućiti da ostave ocenu za hotel.

Počnite dodavanjem novog `fieldset` elementa sa `legend` elementom ugnježdjenim unutra. `legend` treba da ima tekst `Ratings`.

Ispod `legend` elementa, dodajte `label` element sa tekstom `How was the service?`. Atribut `for` treba da bude postavljen na `"service"`.
