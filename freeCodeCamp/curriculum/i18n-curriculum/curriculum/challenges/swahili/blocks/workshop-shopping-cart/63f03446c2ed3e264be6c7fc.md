...
Započnite menjanjem vrednosti `isCartShowing`. Zapamtite da možete koristiti logički operator 'not' `!` da promenite vrednost između istinite i netačne. Postavite promenjenu vrednost za `isCartShowing`.
...
Treba da postaviš promenjenu vrednost za `isCartShowing` na `isCartShowing`.
