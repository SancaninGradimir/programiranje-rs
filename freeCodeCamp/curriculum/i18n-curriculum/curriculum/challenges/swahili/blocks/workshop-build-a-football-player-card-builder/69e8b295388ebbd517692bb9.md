...
Zaglavlje kartice je kompletirano. Sada morate dodati deo slike igrača.

Nakon zatvaranja `div` od `card-header` i i dalje unutar `div` koji ima `className` od `card`, dodajte `div` koji ima `className` od `card-image-wrap`. Ostavite ga prazno za sada. Sliku ćete postaviti u sledećem koraku.

# --hints--

Trebalo bi da dodate `div` sa `className` od `card-image-wrap`.
