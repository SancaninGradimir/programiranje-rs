...
Trenutno, svaki put kada korisnik izmeni belešku i klikne van beleške, promenljiva `currentContent` se ažurira. Ali šta se dešava u situacijama gde korisnik fokusira na belešku i odluči da ode bez pravljenja ikakvih izmena?

Pa, u ovoj situaciji, možete dodati uslov za rukovanje tim slučajem.

Iznad vaše linije `currentContent = newContent;`, dodajte izjavu `if` da proverite da li je `currentContent` jednako `newContent`. Ako jeste, vratite se.
