...
Pseudoselektor `::first-letter` omogućava vam da ciljate prvo slovo u tekstualnom sadržaju elementa.

Kreirajte selektor `.first-paragraph::first-letter` i postavite svojstvo `font-size` na `6rem`. Takođe mu date svojstvo `color`, postavljeno na `orangered`, da bi se istaklo.
