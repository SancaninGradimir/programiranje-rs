...
Su Doku (što na japanskom znači *mesto za brojeve*) je naziv dat poznatom konceptu zagonetke. Njegovo poreklo nije poznato, ali zaslužuje Leonhard Euler koji je kreirao sličnu, pa čak i težu zagonetku nazvanu Latin kvadrati. Cilj Su Doku zagonetki, međutim, je popunjavanje praznih (ili nultih) ćelija u 9x9 mreži na način da svaki red, kolona i 3x3 blok sadrže sve brojeve od 1 do 9. Ispod se nalazi primer početne mrežne zagonetke i njeno rešenje.
...
Dobro konstruisano Sudoku zagonet ima jedinstveno rešenje i može se rešiti logikom, iako je možda korisno koristiti metode „pogađanja i proveravanja“ da bi se eliminisali opcije (postoje mnogi protivrečni mišljenja o tome). Težina pretrage određuje težinu zagonet; primer gore se smatra lakim jer se može rešiti direktnim prepoznavanjem.

Podatkovni red `puzzlesArr` ima niz različitih Sudoku zagonetki sa različitim nivoima težine, ali sve imaju jedinstveno rešenje.

Nakon rešavanja svih zagonetki u `puzzlesArr`, izračunajte ukupan broj cifara 3 koji se nalaze u gornjem levom uglu svake rešene mreže; na primer, 483 je broj cifara 3 koji se nalazi u gornjem levom uglu rešene mreže iznad.
...
`suDoku(testPuzzles1)` Trebalo bi vratiti broj.

```js
assert(typeof suDoku(_testPuzzles1) === 'number');
```

`suDoku(testPuzzles1)` trebao bi da vrati `1190`.

```js
assert.strictEqual(suDoku(_testPuzzles1), 1190);
```

`suDoku(testPuzzles2)` treba da vrati `24702`.
