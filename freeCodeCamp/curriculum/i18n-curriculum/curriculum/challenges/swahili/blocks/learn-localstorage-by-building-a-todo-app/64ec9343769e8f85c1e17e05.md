...
Kreiraj komponentu `div` koja ima klasu od `task`. Koristi niz znakova iz šablona da postavi svojstvo komponente `id` za `div` za `id` koje si izvukao/izvukla iz podataka vežbe.

# --hints--

Morate kreirati element za `div` sa klasom objekata `task`.
...
Vaše svojstvo za `div` mora imati atribut elementa `id` `${id}`.
