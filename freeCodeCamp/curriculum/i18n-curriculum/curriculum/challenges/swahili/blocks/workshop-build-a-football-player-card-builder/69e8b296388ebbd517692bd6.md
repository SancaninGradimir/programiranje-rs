...
Obrazac je sada završen. Da bi se kartica za pregled ažurirala dok kucate, potrebno je da povežete ulazna polja sa stanjem `player`.

Počevši od `input` polja za ime, dodajte mu svojstvo `value` sa vrednošću `player.name`. Zatim dodajte svojstvo `onChange` postavljeno na streličastu funkciju koja prima `e` kao argument i poziva `setPlayer` šireći `player` i postavljajući `name` na `e.target.value`. Ne zaboravite da proširite objekat i postavite vrednost `name` unutar njega.

# --hints--

Vrednost (`value`) vašeg polja za unos imena treba da bude povezana sa poljem stanja `player.name`.
...
Polje za unos imena treba da ima obrađivač događaja `onChange`.

```js
assert.match(__helpers.removeJSComments(code), /onChange=\{/);
```

Obrađivač događaja `onChange` treba da pozove `setPlayer`, pri čemu `setPlayer` dobija objekat koji proširuje `player` i postavlja `name` na `e.target.value`.
