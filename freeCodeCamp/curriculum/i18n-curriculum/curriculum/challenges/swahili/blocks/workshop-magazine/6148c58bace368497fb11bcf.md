...
Slično kao sa Flexbox-om, pomoću CSS Grida možete poravnati sadržaj elemenata grida sa `align-items` i `justify-items`. `align-items` će poravnati elemente grane duž kolonske ose, a `justify-items` će poravnati elemente grane duž redovne ose.

Dajte selektoru `.social-icons` svojstvo `align-items`, postavljeno na `center`.
