...
Sada, pozovi funkciju ``chooseShape()`` za ``val.value`` da prikaže odgovarajuća polja za unos, zatim pozovi ``updateResult()`` da ponovo izračuna i prikaže površinu.

# --hints--

Trebalo bi da pozoveš funkciju `chooseShape` za `val.value`.

```js
assert.match(handleShapeSelect.toString(), /chooseShape\(val\.value\);?/)
```

Trebalo bi da pozoveš funkciju `updateResult()`.
