...
Izaberite vaše elemente iz `td` za parametre `class` sa vrednošću `current`, i učinite fontom kursiv.

# --hints--

Trebalo bi da imaš birač za `td.current`.
...
Tvoj selektor za `td.current` mora imati svojstvo `font-style` postavljeno na `italic`.
