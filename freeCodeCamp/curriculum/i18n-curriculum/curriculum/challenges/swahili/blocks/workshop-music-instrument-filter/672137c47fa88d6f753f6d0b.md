...
Trenutno, vaša funkcija `instrumentCards` vraća niz sa objektima instrumenata, pa će vam trebati još nekoliko koraka pre nego što možete prikazati svoje kartice instrumenata na stranici.

Izmenite svoju funkciju tako da vraća niz stringova koji sadrže HTML kod za prikaz kartica instrumenata, pri čemu svaki string odgovara objektu u nizu `instruments`. Stringovi bi trebalo da imaju ovaj format `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`
