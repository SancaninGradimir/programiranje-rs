...
Ispod vašeg elementa `label`, dodajte dugme `radio` sa `id` podešenim na `"no-option"`, atributom `name` podešenim na `"hotel-stay"` i atributom `value` podešenim na `"no"`.

Ispod vašeg novog dugmeta `radio`, dodajte još jedan element `label` sa atributom `for` podešenim na `"no-option"`. Tekst za `label` treba da bude `No`.

Kada završite, sada možete testirati radio dugmad birajući jednu opciju po jednu.
