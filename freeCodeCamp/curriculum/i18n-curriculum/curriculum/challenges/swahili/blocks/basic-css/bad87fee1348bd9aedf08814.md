...
title: Dodaj Zaobljene Uglove Pomocu Border Radius-a
challengeType: 0
...
Vasa slika macke trenutno ima ostre uglove. Mozemo ih zaobliti koristeci CSS svojstvo `border-radius`.

# --instructions--

Mozete zadati `border-radius` u pikselima. Postavite slici macke `border-radius` na `10px`.

**Napomena:** Ovaj izazov prihvata vise mogucih resenja. Na primer, mozete dodati `border-radius` klasi `.thick-green-border` ili klasi `.smaller-image`.
...
Vasa slika treba da ima `border-radius` od `10px`.
