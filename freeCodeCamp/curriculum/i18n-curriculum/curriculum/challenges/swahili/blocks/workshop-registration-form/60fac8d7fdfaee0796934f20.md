...
Opcije padajućeg menija trenutno nemaju sadržaj. Da biste mu dodali sadržaj, unesite sledeći tekst za svaki element od `option` koji sledi:
...
Treba da mu/jajete prvi deo teksta od `option` sa tekstom `(select one)`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[0]?.textContent, '(select one)');
```

Trebao bi mu dati drugi deo teksta od `option`, tekst od `freeCodeCamp News`.

```js
assert.equal(document.querySelectorAll('fieldset:nth-child(3) > label:nth-child(3) option')?.[1]?.textContent, 'freeCodeCamp News');
```

Trebalo bi mu dati treći deo teksta od `option`, koji je tekst od `freeCodeCamp YouTube Channel`.
...
Trebalo bi mu dati četvrti deo teksta od `option`, tekst od `freeCodeCamp Forum`.
