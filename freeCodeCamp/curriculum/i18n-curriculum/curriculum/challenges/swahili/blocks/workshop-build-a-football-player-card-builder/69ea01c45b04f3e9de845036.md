...
Trenutni problem je što čim se početni prikaz osveži ili kada osvježavate stranicu, sve što dodate na karticu igrača nestaje.

Čuvanje svih ulaza u `localStorage` će sprečiti ovo. Da biste počeli, kreirajte konstantnu vrednost `STORAGE_KEY` sa nizom karaktera `football_player_card` kao njenu vrednost.

# --hints--

Treba da objaviš promene za `STORAGE_KEY`.
...
`STORAGE_KEY` mora imati sekvencu slova `football_player_card` kao svoju vrednost.
