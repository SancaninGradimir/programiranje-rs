...
U tvom prvom elementu od `p`, dodaj element `span`. Dodaj `span` klasu `product-count` i atribut `id` sa vrednošću `product-count-for-id${id}`. Zatim, nakon span-a, dodaj svom elementu `p` tekst promene `name`.

# --hints--

Vaš prvi element od `p` treba da ima element `span`.
...
Vaša funkcija za `span` mora da ima `class` od `product-count`.
...
Vaš element od `span` treba da ima `id` sa `product-count-for-id${id}`.
...
Prvi deo vašeg `p` treba da sadrži tekst promene iz `name`. Ovo bi trebalo da bude van span-a.
