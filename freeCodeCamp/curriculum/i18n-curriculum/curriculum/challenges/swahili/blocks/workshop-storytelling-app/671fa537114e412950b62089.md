...
U vašoj datoteci od JavaScript, izaberite element sa `.story-container` i sačuvajte ga u parametar nazvan `storyContainer`.

# --hints--

Trebalo bi da imate kriterijum za `storyContainer`.

```js
assert.isDefined(storyContainer);
```

Treba da izaberete element `.story-container` koristeći metod `querySelector` i sačuvate ga u parametar `storyContainer`.
