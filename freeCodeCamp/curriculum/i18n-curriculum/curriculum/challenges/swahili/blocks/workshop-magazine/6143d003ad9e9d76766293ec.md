...
Unutar vašeg `.image-quote` elementa, ugnezdite `hr` element, `p` element i drugi `hr` element. Dajte `p` elementu `class` podešeno na `quote` i tekst `The millions of people who are learning to code through freeCodeCamp will have an even better resource to help them learn these fundamentals.`.
