...
Sada, zameni `new_char` sa `encrypted_text`. Takođe, promeni poziv od `print()` u `print('char:', char, 'encrypted text:', encrypted_text)` da prikaže ove promene.

# --hints--

Trebalo bi da zameniš `new_char` sa `encrypted_text` unutar tvog ciklusa `for`.
...
Treba da promeniš poziv `print()` u `print('char:', char, 'encrypted text:', encrypted_text)` unutar svog ciklusa `for`.
