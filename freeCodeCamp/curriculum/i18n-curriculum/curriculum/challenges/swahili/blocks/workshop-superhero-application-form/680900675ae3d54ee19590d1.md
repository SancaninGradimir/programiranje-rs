...
Sledeće, recite formularu kako da se pošalje. Dodajte atribut elementa `method` sa vrednošću `post` i atribut elementa `action` sa vrednošću `https://superhero-application-form.freecodecamp.org` na element forme.

Sada, predaj obrazac za URL tu. To ćete učiniti u sledećem koraku.

# --hints--

Atribut tvog elementa `form` treba da ima svojstvo atributa `method` postavljeno na `post`.
...
Vaš deo `form` mora imati atribut elementa `action` postavljen na `https://superhero-application-form.freecodecamp.org`.
