...
Koristite putanju `setAttribute` na elementu `playButton` da postavite svojstvo elementa nazvano `"aria-label"`. Koristeći tridimensionalni kalkulator uslova, postavite vrednost svojstva elementa na `Play ${song.title}` ili `"Play"` ako `song?.title` nije dostupan.
