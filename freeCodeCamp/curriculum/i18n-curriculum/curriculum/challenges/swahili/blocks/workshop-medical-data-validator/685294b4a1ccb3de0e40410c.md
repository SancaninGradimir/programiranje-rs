...
Vraćajući se na funkciju `validate`, nakon dve `if` naredbe i i dalje unutar petlje `for`, kreiraj varijablu pod imenom `invalid_records`.

Zatim, dodeli mu poziv na `find_invalid_records` korišćenjem operatera `**` za raspakivanje `dictionary`.
