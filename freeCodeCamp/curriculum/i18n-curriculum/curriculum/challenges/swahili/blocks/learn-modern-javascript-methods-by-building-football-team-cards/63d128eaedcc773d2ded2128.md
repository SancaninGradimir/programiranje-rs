...
Ispod tog objekta, napravite novi objekat sa sledećim karakteristikama:
...
Vaš red za `myFavoriteFootballTeam.players` treba da ima dve vrednosti.
...
Vaš red za `myFavoriteFootballTeam.players` treba da ima nešto kao druga vrednost.

```js
assert.isObject(myFavoriteFootballTeam.players[1])
```

Tvoj novi predmet treba da ima karakteristiku `name`.

```js
assert.property(myFavoriteFootballTeam.players[1], 'name');
```

Tvoja vrednost za `name` treba da bude postavljena na `"Sergio Batista"`.
...
Tvoja vrednost za `position` treba da bude postavljena na `"midfielder"`.
...
Tvoja vrednost za `number` mora biti broj.

```js
assert.isNumber(myFavoriteFootballTeam.players[1]?.number);
```

Tvoja vrednost za `number` treba da bude postavljena na `2`.

```js
assert.equal(myFavoriteFootballTeam.players[1]?.number, 2);
```

Tvoj novi uređaj mora imati karakteristiku `isCaptain`.
...
Tvoja vrijednost za `nickname` mora biti postavljena na `null`.
