...
Traka nivoa i dalje prikazuje tekst postavljen direktno `GOLD`. Morate ga promeniti dinamičkim matematičkim izrazom kako bi se prikazao ispravan nivo.

Izvršite poziv `getPlayerTier()` za `player.overallRating` da dobijete niz slova nivoa, zatim izvršite poziv `.toUpperCase()` na rezultat kako biste ga prikazali velikim slovima. Ne zaboravite da obuhvatite ceo matematički izraz uglastim zagradama.

# --hints--

Trebalo bi da koristiš `getPlayerTier(player.overallRating).toUpperCase()` unutar `.card-tier-badge`.

```js
assert.match(__helpers.removeJSComments(code), /getPlayerTier\s*\(\s*player\.overallRating\s*\)\s*\.toUpperCase\s*\(\s*\)/);
```

Funkcija `.card-tier-badge` treba da prikaže `ELITE` za igrača sa nivoom 98.
