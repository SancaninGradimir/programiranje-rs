...
Svaki `tab` predstavlja jedan od dugmadi u spisku kartica. Korisnik bi trebalo da može kliknuti na karticu kako bi prikazao odgovarajući panel i sakrio ostale.

U ranijim modulima, naučili ste kako da priložite slušalicu događaja (event listener) `"click"` svakom dugmetu u `NodeList` na ovaj način:
...
Koristite metodu `addEventListener` da priložite slušalicu događaja (event listener) `"click"` svakom dugmetu `tab`. Drugi argument metode `addEventListener` bi trebalo da bude prazna strelica funkcija (empty arrow function) `() => {}` za sada.
