...
title: 'Izazov 77: Zbroj osnovnog broja'
challengeType: 1
forumTopicId: 302190
...
Moguće je napisati broj deset kao zbir osnovnih brojeva na pet različitih načina:
...
Koja je prva vrednost koja se može napisati kao zbir prim brojeva na više od jednog načina? `n`

# --hints--

`primeSummations(5)` Trebalo bi da vrati broj.

```js
assert(typeof primeSummations(5) === 'number');
```

`primeSummations(5)` treba da vrati `11`.

```js
assert.strictEqual(primeSummations(5), 11);
```

`primeSummations(100)` trebalo bi da vrati `31`.

```js
assert.strictEqual(primeSummations(100), 31);
```

`primeSummations(1000)` treba da vrati `53`.

```js
assert.strictEqual(primeSummations(1000), 53);
```

`primeSummations(5000)` treba da vrati `71`.
