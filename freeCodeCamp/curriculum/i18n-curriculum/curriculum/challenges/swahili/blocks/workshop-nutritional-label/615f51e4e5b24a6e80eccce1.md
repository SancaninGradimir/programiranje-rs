...
Unutar vašeg `.calories-info` elementa, kreirajte `div` element. Dajte tom `div` elementu atribut `class` postavljen na `left-container`. Unutar novokreiranog `div` elementa, kreirajte `h2` element sa tekstom `Amount per serving`. Dajte `h2` elementu atribut `class` postavljen na `bold small-text`.
