...
Ubaciti unutar elementa `select` (zajedno sa njegovim elementima od `option`) unutar elementa `label` koji ima tekst `How did you hear about us?`. Tekst bi trebalo da stoji pre elementa `select`.

# --hints--

Treba da staviš unutar elementa `select` samo unutar elementa `label`.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

Trebalo bi mu dati atribut sa tekstom `label` i `How did you hear about us?`.

```js
assert.equal(document.querySelector('fieldset:nth-child(3) > label:nth-child(3)')?.innerText?.trim(), 'How did you hear about us?');
```

Treba da umetnete tekst pre elementa `select`.
