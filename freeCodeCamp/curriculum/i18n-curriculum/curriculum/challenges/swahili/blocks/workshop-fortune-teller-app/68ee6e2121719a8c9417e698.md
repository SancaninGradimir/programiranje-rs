...
Trebalo bi da pozovete funkciju `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(/);
```

Trebalo bi da prosledite `elements.singleCard` as kao prvi argument.

```js
assert.match(__helpers.removeJSComments(code), /showElements\s*\(\s*this\.elements\.singleCard/);
```

Trebalo bi da prosledite `elements.fortuneContainer` as drugi argument.
