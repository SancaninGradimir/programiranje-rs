...
Kao što možete da vidite, `series1` je prikazan u istom formatu kao objekat `Movie`, jer nasleđuje metod svog roditeljskog `__str__`. U sledećem koraku, nadjačaćete metod `__str__` kako biste pružili prilagođenu string reprezentaciju.

Za sada, obrišite liniju `print(series1)` iz svog koda.
