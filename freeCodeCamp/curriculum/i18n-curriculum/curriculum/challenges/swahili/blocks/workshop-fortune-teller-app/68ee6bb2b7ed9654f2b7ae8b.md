...
Trebalo bi da dodelite ``click`` slušalicu događaja na globalni ``document``.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Vaš event listener bi trebalo da ima parametar povratnog poziva (callback parameter) nazvan `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Vaš parametar za callback bi trebalo da ima tip `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Unutar *callback*-a, trebalo bi da pozovete funkciju `showFortune`

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Unutar funkcije `showFortune`, treba da prosledite argument `e`.
