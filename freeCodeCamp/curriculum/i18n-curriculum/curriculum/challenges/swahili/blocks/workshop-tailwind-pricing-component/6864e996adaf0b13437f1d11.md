...
Sad, treba da pređete na dekorisanje elemenata **mpango wa msikilizaji**.

Primeni klasu `div`, `bg-gray-100` za `ring-1` izvan programa kako bi se kreirala debela linija oko `div`, i `ring-gray-300` da se postavi tamna boja za tu liniju.

# --hints--

Element vašeg `div` mora imati klasu `bg-gray-100`.
...
Tvoj element za `div` mora imati klasu `ring-1`.
...
Vaš element za `div` mora imati klasu `ring-gray-300`.
