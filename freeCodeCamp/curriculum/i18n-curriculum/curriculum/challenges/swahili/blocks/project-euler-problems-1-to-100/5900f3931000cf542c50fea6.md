...
title: 'Vežba 39: Trokuti u krugovima sa celim brojevima'
challengeType: 1
forumTopicId: 302054
...
Ako `p` je krug trougla sa pravim uglom i stranama čija su dužina celih brojeva, {a,b,c}, postoji samo tri rešenja za p = 120.

{20,48,52}, {24,45,51}, {30,40,50}

Za koju vrednost `p` ≤ `n`, broj rešenja postaje veći?

# --hints--

`intRightTriangles(500)` Trebalo bi da vrati broj.

```js
assert(typeof intRightTriangles(500) === 'number');
```

`intRightTriangles(500)` trebalo bi da vrati 420.

```js
assert(intRightTriangles(500) == 420);
```

`intRightTriangles(800)` Trebalo bi da vrati 720.

```js
assert(intRightTriangles(800) == 720);
```

`intRightTriangles(900)` Trebalo bi da vrati 840.

```js
assert(intRightTriangles(900) == 840);
```

`intRightTriangles(1000)` trebalo bi da vrati 840.
