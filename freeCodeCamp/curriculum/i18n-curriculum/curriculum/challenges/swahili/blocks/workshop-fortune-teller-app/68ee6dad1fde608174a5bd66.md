...
Trebalo bi da ažurirate `this.elements.singleCard.innerHTML` pozivom funkcije `renderCard`.
...
Vaš poziv `renderCard` bi trebalo da ima `"Click the card and reveal the fortune"` as kao prvi argument.
...
Trebalo bi da prosledite `isReversed` as drugom argumentu za `renderCard`.
