...
Trebalo bi da imate element `label` unutar elementa `form`.
...
Element `label` treba da ima atribut `htmlFor` sa vrednošću `"search-input"`.
...
Vaš element `label` bi trebalo da ima tekst `Search for fruits:`.
