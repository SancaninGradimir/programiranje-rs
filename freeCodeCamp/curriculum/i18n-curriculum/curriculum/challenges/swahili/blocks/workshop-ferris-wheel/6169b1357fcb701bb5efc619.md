...
Trebalo bi da kreirate selektor new `25%` u vašoj `@keyframes cabins` pravilima.
...
Vaš `25%` selektor treba da bude između vašeg `0%` i `50%` selektora.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText , '25%');
```

Tvoj `25%` selektor bi trebalo da ima `background-color` svojstvo set do `yellow`.
