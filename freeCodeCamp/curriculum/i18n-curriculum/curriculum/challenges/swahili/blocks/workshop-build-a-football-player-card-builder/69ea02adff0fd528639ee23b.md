...
Nakon toga, kreiraj funkciju ``loadPlayer`` sa ``PlayerData`` kao tipom povratne vrednosti. Unutar nje, dodaj sintaksu ``try...catch`` sa ``error`` kao parametar za deo ``catch``. Sve ostalo ostavi prazno na taj trenutak.

# --hints--

Trebalo bi da kreiraš funkciju koja se zove `loadPlayer`.
...
Vaša funkcija za `loadPlayer` mora imati tip povratne vrednosti `PlayerData`.
...
Tvoja funkcija za `loadPlayer` mora imati deo `try...catch`.
