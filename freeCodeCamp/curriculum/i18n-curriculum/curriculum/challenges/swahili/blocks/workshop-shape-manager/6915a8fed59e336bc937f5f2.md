...
Poslednji set izabranih elemenata su elementi rezultata. Koristite vašu funkciju `getElement` da dobijete element sa identifikatorima `result-text` i `result-card`. Svi moraju koristiti `HTMLElement` kao tip.

Zapamti da si već objavio parametre za `resultText` i `resultCard`, pa samo trebaš postaviti vrednosti.

# --hints--

Treba da postaviš vrednost za parametar `resultText`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*/)
```

`resultText` treba da odabere element `result-text`.

```js
assert.match(initializeApp.toString(), /resultText\s*=\s*getElement\(["']result-text["']\);?/)
```

Trebalo bi da koristite `HTMLElement` kao tip `getElement` koji koristite za odabir elementa `result-text`.

```js
assert.match(code, /resultText\s*=\s*getElement\(["']result-text["']\)\s+as\s+HTMLElement;?/)
```

Morate postaviti vrednost za parametar `resultCard`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*/)
```

`resultCard` treba da izabere element `result-card`.

```js
assert.match(initializeApp.toString(), /resultCard\s*=\s*getElement\(["']result-card["']\);?/)
```

Trebaš koristiti `HTMLElement` kao tip `getElement` koji koristiš za odabir elementa `result-card`.
