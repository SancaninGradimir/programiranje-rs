...
Zatim, želite da proverite da `age` je ceo broj. Dakle, dodajte još jedan ključ `age` u rečnik `constraints`. Za njegovu vrednost, pozovite `isinstance`, prosleđujući `age` i `int` kao njegove argumente.
