...
Nakala za `.sr-only` je i dalje vidljiva. Postoji uobičajena metoda skrivanja teksta za oči, tako da ga čitaju samo čitači ekrana.

Ovaj stil je postavljanje sledećih svojstava za CSS:
...
Koristite gore da definišete pravilo CSS za `.sr-only`.
...
Treba da mu daš `.sr-only` `padding` od `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sr-only')?.padding, '0px');
```

Trebalo bi mu dati `.sr-only` `margin` od `-1px`.
...
Treba da mu daš `.sr-only` `white-space` od `nowrap`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.sr-only')?.whiteSpace, 'nowrap');
```

Treba da mu daš `.sr-only` `border` od `0`.
