...
`initialFetch` trebalo bi da bude funkcija.

```js
assert.isFunction(initialFetch)
```

Trebalo bi da koristite `const` za kreiranje funkcije `initialFetch`.
...
Vaša funkcija `initialFetch` ne bi trebalo da prima nikakve parametre.

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(\s*\)\s*/)
```

Vaša funkcija `initialFetch` bi trebalo da koristi strelični sintaksa.

```js
assert.match(code, /const\s+initialFetch\s*=\s*.*\s*\(.*\)\s*=>\s*/)
```

Vaša funkcija `initialFetch` ne sme biti prazna.
...
Vaša funkcija `initialFetch` bi trebalo da sadrži izjavu `fetch` i lančane metode.
