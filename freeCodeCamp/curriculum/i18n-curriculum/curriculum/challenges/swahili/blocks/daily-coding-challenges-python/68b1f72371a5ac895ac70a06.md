...
Ako su dati veličina video zapisa, jedinica veličine video zapisa, kapacitet diska i jedinica kapaciteta diska, vrati broj video zapisa koji mogu stati na disk prema sledećim pravilima.

- Kitengo cha ukubwa wa video kinaweza kuwa bytes (`"B"`), kilobytes (`"KB"`), megabytes (`"MB"`), au gigabytes (`"GB"`).
- Ako jedinica za veličinu video zapisa nije jedna od podržanih, vrati `"Invalid video unit"`.
- Kitengo cha uwezo wa diski kuu kinaweza kuwa gigabytes (`"GB"`) au terabytes (`"TB"`).
- Ako jedinica za kapacitet diska nije jedna od podržanih, vrati `"Invalid drive unit"`.
- Rudisha idadi ya video kamili ambazo diski inaweza kuhifadhi.
- Koristi sledeće konverzije:
...
Na primer, ako su argumenti `500`, `"MB"`, `100` i `"GB"`, odredi koliko video zapisa od 500 MB može da stane na disk od 100 GB.
