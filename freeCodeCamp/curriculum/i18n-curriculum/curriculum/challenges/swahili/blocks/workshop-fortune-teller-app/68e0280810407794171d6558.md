...
Trebalo bi da imate `div` element unutar vašeg `figure` elementa.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<div/);
```

Vaš `div` treba da ima class `img-loader`.

```js
assert.match(__helpers.removeJSComments(code), /<div[^>]*class\s*=\s*"img-loader"[^>]*>\s*<\/div>/);
```

Trebalo bi da imate element `img` unutar vašeg elementa `figure`.

```js
assert.match(__helpers.removeJSComments(code), /<figure[\s\S]*?<img/);
```

Element `img` bi trebalo da koristi `CDN_URL` i `img` for njegovo `src` kada je `img` dostupan, i da se vrati na `LOCAL_DEFAULT_IMG` u suprotnom slučaju.
...
Element `img` bi trebalo da ima klase `card-img` i `hidden`.
