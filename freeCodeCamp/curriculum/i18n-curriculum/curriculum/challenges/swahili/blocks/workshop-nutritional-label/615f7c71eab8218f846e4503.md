...
Nakon vašeg poslednjeg `.divider`, kreirajte još jedan `p` element sa tekstom `Trans Fat 0g`. Italizujte reč `Trans` obavijanjem je u `i` element. Dajte novom `p` elementu atribut `class` postavljen na `indent no-divider`. Obavijte `Trans Fat 0g` u `span` element za poravnanje.
