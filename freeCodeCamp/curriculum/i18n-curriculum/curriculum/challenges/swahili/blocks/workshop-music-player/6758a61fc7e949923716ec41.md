...
Sada, koristi metodu `querySelectorAll` da cilja elemente `.playlist-song` i sačuva ih u varijablu nazvanu `songs`.

Zatim, pozovi metodu `forEach` na `songs`. Prosledi funkciju povratne pozive koja koristi `song` kao parametar metode. Ostavi vitičaste zagrade prazne za sada.
