...
Na malom ekranu, neuređena lista u navigacionoj traci prelazi preko desne strane ekrana.

Ispravite ovo koristeći _Flexbox_ za obuhvaćanje sadržaja `ul`. Zatim, postavite svojstva CSS sledeća kako biste pravilno rasporedili tekst:
...
Treba da mu/jajš (ili: treba da joj) da `ul` `align-items` od `center`.
...
Treba da mu/jajš (ili: treba da joj) daš `ul` `margin-block` od `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('nav > ul')?.marginBlock, '0px');
```

Trebalo bi mu dati `ul` `height` od `100%`.
