...
Da biste organizovali elemente vya `.question-block` za dobar redosled, postavite sledeće svojstva od CSS:
...
Trebalo bi mu dati atribut `.question-block` `margin-top` od `20px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.question-block')?.marginTop, '20px');
```

Trebalo bi da mu date atribut `.question-block` `padding-top` od `5px`.
