...
Trebalo bi da deklariš interfejs pod imenom `Card`.
...
Vaš `Card` interfejs bi trebalo da ima `name` svojstvo tipa `string`.
...
Tvoj `Card` interfejs bi trebalo da ima svojstvo `name_short` tipa `string`.
...
Vaš `Card` interfejs bi trebalo da ima `value` svojstvo sa tipom unije od `string` i `number`.
