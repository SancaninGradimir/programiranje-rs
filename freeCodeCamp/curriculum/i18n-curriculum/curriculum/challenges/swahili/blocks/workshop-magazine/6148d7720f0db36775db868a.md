...
Biće vam potrebno da imate kolonu za tekst i kolonu za slike. Dajte selektoru `.text-with-images` svojstvo `grid-template-columns` postavljeno na `1fr 2fr`. Takođe postavite svojstvo `column-gap` na `3rem` kako biste obezbedili veći razmak između kolona.
