...
Ne brinite za prostor ispod; sve će biti postavljeno kasnije kada povećate visinu elementa na vrhu zgrade.

Povećaj `repeating-linear-gradient` za `.fb1c` za ugao `90deg`, `--building-color4` od `0%` do `10%` i `transparent` od `10%` do `15%`.
...
Trebalo bi da koristite drugu boju od `transparent` od `10%` do `15%`.
