...
Vreme je da koristite još jedan regularni izraz. Slično onome što ste već uradili, koristite operater `and` da dodate izraz trenutnoj vrednosti `constraints['last_visit_id']`.

Na desnoj strani operatera `and`, koristite funkciju `fullmatch` iz modula `re` kako biste osigurali da `last_visit_id` počinje slovom `v` (bilo malim ili velikim slovom) praćenim jednim ili više cifara.
