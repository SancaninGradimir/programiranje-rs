...
Sada možete ispraviti ukupan broj stavki na web stranici. Postavite vrednost putanje `cart` tvog objekta `.getCounts()` u svojstvo `textContent` parametra `totalNumberOfItems`.

# --hints--

Trebalo bi da dobijete pristup atributu `textContent` parametra `totalNumberOfItems`.

```js
assert.match(code, /totalNumberOfItems\s*\.\s*textContent/)
```

Morate postaviti vrednost puta za `cart` vašeg objekta `.getCounts()` na svojstvo `textContent` parametra `totalNumberOfItems`.
