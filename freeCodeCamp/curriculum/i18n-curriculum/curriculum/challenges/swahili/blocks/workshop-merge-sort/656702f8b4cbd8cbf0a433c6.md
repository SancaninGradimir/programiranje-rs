...
Zatim, morate kreirati petlju koja se izvršava sve dok postoje elementi u oba `left_part` i `right_part`.

Za to, kreirajte petlju `while` sa dve uslova: jedan koji proverava da li je `left_array_index` manji od dužine `left_part`, i drugi uslov koji proverava da li je `right_array_index` manji od dužine `right_part`.
