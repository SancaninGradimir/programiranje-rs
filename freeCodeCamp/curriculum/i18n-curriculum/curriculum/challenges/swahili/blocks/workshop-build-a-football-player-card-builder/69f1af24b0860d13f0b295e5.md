...
Sada promenite preostale hard brojčane podatke i sledeće:
...
Trebalo bi da promeniš hardkodiranu vrednost `99` u `DRI` i `player.dri`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.dri\}/);
```

Trebalo bi da promeniš statičnu vrednost `41` u `DEF` sa `player.def`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.def\}/);
```

Trebalo bi da promeniš fiksnu vrednost `99` u `PHY` na `player.phy`.
