...
Umesto da samo vrati `prev.includes(item)`, vrati ternarni operator sa `prev.includes(item)` kao uslov. Trenutno vrati `null` na obje strane.

# --hints--

Vrati ternarni operator koji proverava da li `prev.includes(item)` je tačno. Vrati `null` za obe strane tačno i netačno.
