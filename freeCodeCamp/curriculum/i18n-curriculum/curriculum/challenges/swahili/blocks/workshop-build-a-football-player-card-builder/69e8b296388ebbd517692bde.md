...
Dodajte svojstvo elementa `value` sa vrednošću `player.dri` na ulaz DRI-ja. Zatim dodajte svojstvo elementa `onChange` postavljeno kao strelica funkcija koja prima `e` kao parametar i poziva `setPlayer` za proširenje (spread) `player` i `dri` postavljeno na `Number(e.target.value)`.

# --hints--

`input` vaša DRI mora imati `value` jednako kao statistika za `dri` igrača.
...
`input` vaš DRI bi trebalo da ima hendlera za `onChange` koji poziva `setPlayer` za širenje `player` i `dri` podešen na `Number(e.target.value)`.
