...
Objavi kriterijum `tax` na svoju temu za poziv vašeg novog puta `.calculateTaxes()`, prosleđujući `subTotal` kao argument.

# --hints--

Koristi `const` da objaviš parametar nazvan `tax`.
...
Postavite vrednost poziva za vašu novu putanju/metodu `.calculateTaxes()`, prosljeđujući `subTotal` kao argument, za parametar `tax`. Zapamtite da koristite ključnu reč `this`.
