...
U povratnom pozivu `forEach`, obuhvatite opseg vrednosti sa desne strane `totalCountPerProduct[dessert.id] || 0` unutar običnih zagrada `()` kako biste osigurali tačnu evaluaciju, zatim dodajte vrednost za jedan.

# --hints--

Morate koristiti dot notaciju da biste dobili svojstvo `id` za `dessert`.
...
Trebalo bi da koristite zapisivanje u zagradi (parentheses) da biste dobili atribut `totalCountPerProduct` koji odgovara `dessert.id`.
...
Treba vam da koristite distributivni operator da pojednostavite vrednost atributa `totalCountPerProduct` koji odgovara `dessert.id`.
...
Treba da obavijete `totalCountPerProduct[dessert.id] || 0` unutar običnih zagrada `()` prije nego što ga dodate pojedinačno.
