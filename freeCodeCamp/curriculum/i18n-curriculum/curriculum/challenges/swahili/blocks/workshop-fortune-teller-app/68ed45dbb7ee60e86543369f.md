...
Trebalo bi da deklariš varijablu pod imenom `data` unutar `fetchCardsData`.
...
Vaša varijabla `data` treba da ima tip `Deck`.

```js
assert.match(__helpers.removeJSComments(code), /data\s*:\s*Deck\s*=/);
```

Vaša varijabla `data` bi trebalo da bude dodeljena parsiranjem odgovora as JSON.
...
Trebalo bi da sačuvate `data.cards` u `this.cards`.

```js
assert.match(__helpers.removeJSComments(code), /this\.cards\s*=\s*data\.cards/);
```

Vaš `catch` blok bi trebalo da koristi `console.error()` za logovanje greške sa porukom `"Error fetching cards:", error`.
