...
I kao treći argument boje, poziva funkciju `hsl` sa vrednostima `240` za nijansu (hue), `56%` za zasićenost (saturation) i `42%` za svetlinu.

# --hints--

Vaše pravilo za `.blue` od CSS trebalo bi da ima svojstvo `background` postavljeno na `linear-gradient(hsl(186, 76%, 16%), hsl(223, 90%, 60%), hsl(240, 56%, 42%))`.
