...
Trebalo bi da set `cardTitle` `textContent` do `foundCard.name`. Ne zaboravite da koristite ključnu reč `this`.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

Trebalo bi da set `description` `textContent` do `foundCard.desc`. Ne zaboravite da koristite ključnu reč `this`.
