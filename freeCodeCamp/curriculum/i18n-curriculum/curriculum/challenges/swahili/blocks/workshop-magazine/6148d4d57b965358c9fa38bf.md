...
Sada je vreme da stilizujete svoj treći `section`. Napomenite da ima vrednosti `text` i `text-with-images` za atribut `class`, što znači da već nasleđuje stilove iz vašeg pravila `.text`.

Kreirajte selektor `.text-with-images` i postavite svojstvo `display` na `grid`.
