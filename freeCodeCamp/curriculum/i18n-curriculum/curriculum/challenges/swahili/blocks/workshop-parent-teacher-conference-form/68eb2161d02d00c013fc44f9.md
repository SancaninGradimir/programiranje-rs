...
Da biste učinili tekst `legend` uočljivijim, morate ga stilizovati.

Izaberite element `fieldset legend`. Podelite njegovu `font-size` na `1.3rem` i njegovu `font-weight` na `600`.

`font-weight` kontroliše podebljanje teksta. 

Uobičajene vrednosti za `font-weight` su:

- `normal` → Redovna težina (podrazumevano)
- `bold` → Podebljan tekst
- `lighter` → Lakše od roditeljskog elementa
- `bolder` → Podebljanije od roditeljskog elementa
- Numeričke vrednosti kao što su:
  - `100` (tanak)
  - `400` (normalan)
  - `700` (podebljan)
  - `900` (ekstra podebljan)
