...
Konačno, ažuriraj svojstvo `display` objekta/stav `style` promena `cartContainer` sa drugim uslovnim izrazom od tri dela koji proverava da li `isCartShowing` je tačno. Ako je tačno, postavi svojstvo `display` na `"block"`, inače postavi na `"none"`.

Sada bi trebalo da možete da vidite svoje vozilo i da dodate stvari unutar njega.

# --hints--

Trebalo bi da dobijete pristup atributu `display` atributa `style` izmene `cartContainer`.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display/)
```

Trebalo bi koristiti distributivni operator na svojstvu `display`, svojstvo `style`, promena `cartContainer`.
...
Trebalo bi postaviti atribut `display`, atributa `style` promene `cartContainer` na `"block"` ako je `isCartShowing` tačno.

```js
assert.match(code, /cartContainer\s*\.\s*style\s*\.\s*display\s*=\s*isCartShowing\s*\?\s*('|"|`)block\1\s*:\s*/)
```

Trebalo bi postaviti svojstvo `display` od svojstva `style` promene `cartContainer` na `"none"` ako je `isCartShowing` netačno.
