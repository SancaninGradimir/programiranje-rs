...
Nakon tri `p` elementa unutar vašeg `.text` elementa, kreiraj `blockquote` element. Unutar toga, dodaj `hr` element, `p` element sa `class` podešenim na `quote`, i drugi `hr` element.

Daj `.quote` elementu tekst `The entire curriculum should be a series of projects`.
