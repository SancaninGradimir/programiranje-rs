...
Trebalo bi da pozovete `parseCard` sa `rawCatalogCards[2]` as argument.

```js
assert.match(__helpers.removeJSComments(code), /parseCard\s*\(\s*rawCatalogCards\s*\[\s*2\s*\]\s*\)/);
```

Trebalo bi da dodelite rezultat pozivanja `parseCard` sa `rawCatalogCards[2]` promenljivoj imena `cardResult`.

```js
assert.match(__helpers.removeJSComments(code), /(const|let|var)\s+cardResult\s*=\s*parseCard\s*\(\s*rawCatalogCards\s*\[\s*2\s*\]\s*\)\s*;?/);
```

Trebalo bi da logujete `cardResult` u console.
