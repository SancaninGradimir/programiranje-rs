...
Za početak vašeg CSS-a, normalizujte CSS pravila ciljanjem na sve elemente sa `*`, uključujući pseudoselektore `::before` i `::after`.

Postavite svojstva `padding` i `margin` oba na `0` i postavite svojstvo `box-sizing` na `border-box`.
