...
Povežite odgovarajuće elemente obrasca sa elementima `label`.

Koristi `profile-picture`, `age`, `referrer`, na `bio` kao vrednosti za atribute `id` koji su relevantni.
...
Treći element od `label` mora imati svojstvo `for` sa vrednošću `referrer`.

```js
assert.isTrue(document.querySelectorAll('fieldset:nth-of-type(3) label')?.[2]?.matches('label[for="referrer"]'))
```

Četvrti atribut od `label` mora imati svojstvo `for` sa vrednošću `bio`.
