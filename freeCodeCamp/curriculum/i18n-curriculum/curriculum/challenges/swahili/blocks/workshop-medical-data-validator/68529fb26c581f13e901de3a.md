...
Neposredno nakon varijable `invalid_records`, kreirajte petlju `for` za iteraciju preko nje. Za svaki nevažeći zapis, ispišite `Unexpected format '<key>: <val>' at position <index>.`. Zamenite `<key>`, `<val>` i `<index>` trenutnim ključem, vrednošću i indeksom.

Zapamtite da je `invalid_records` lista ključeva koji se odnose na nevažeće zapise u trenutnom `dictionary`. Biće vam potrebno uzeti ključ iz `invalid_records` i pronaći vrednost u `dictionary`.

Pozicija ili `index` odnosi se na trenutni rečnik (dictionary) u `medical_records`, definisan spoljašnjom petljom `for` u funkciji.

Pregledajte svoj kod do sada ako vam je potrebno da se podsetite na petlje i varijable koje su već kreirane.

Zatim, postavite `is_invalid` na `True`.

Slobodno testirajte funkciju `validate` sa nevažećim podacima kako biste videli poruke validacije. 

Time je radionica za medicinskog validatora završena.
