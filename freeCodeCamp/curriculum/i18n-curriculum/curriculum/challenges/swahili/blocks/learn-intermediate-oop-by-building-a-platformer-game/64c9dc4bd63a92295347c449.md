...
Sledeće, dodajte izjavu za `else if` ili proverite da li je levi taster pritisnut i da li je `isCheckpointCollisionDetectionActive` tačno.

U tom stanju, dodaj petlju za `forEach` koja iterira kroz niz podataka `platforms`.

Unutar tog ciklusa, koristi operaciju sabiranja da dodaš 5 na mestu `x` platforme.

# --hints--

Trebalo bi da ima stanje koje proverava da li je kirični taster pritisnut i da li je `isCheckpointCollisionDetectionActive` tačno.
...
Morate dodati petlju `forEach` koja iterira kroz niz podataka `platforms`.

```js
assert.match(code, /else\s+if\s*\(.*\)\s*{\s*platforms\.forEach\(\s*(\(\s*platform\s*\)|platform)\s*=>\s*{?\s*(.*?)\s*}?\s*\)\s*;?/);
```

Treba vam da koristite operator za dodavanje pet u polje `x` na platformi.
