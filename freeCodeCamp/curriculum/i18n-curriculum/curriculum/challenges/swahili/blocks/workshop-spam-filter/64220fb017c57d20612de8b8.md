...
Sada kada imate svoju grupu za hvatanje, možete označiti ceo obrazac kao opcionalno poklapanje. Kvantifikator `?` odgovara nultom ili jednom pojavljivanju prethodnog karaktera ili grupe. Na primer, regularni izraz `/colou?r/` odgovara i `color` i `colour`, jer je `u` opcionalan.

Označite svoju grupu za hvatanje kao opcionalnu.
