...
Deklariši varijablu pod imenom `cardElement` i koristi klasu `card_container` da dohvatiš element. Koristi selektor `closest` da ga dohvatiš iz `target`.

# --hints--

Trebalo bi da deklarišete varijablu pod nazivom `cardElement` unutar `showFortune`.
...
Vaš `cardElement` treba da koristi `target?.closest` kako bi pronašao `.card_container`.
...
Trebalo bi da prosledite `.card_container` as argument ka `closest`.
