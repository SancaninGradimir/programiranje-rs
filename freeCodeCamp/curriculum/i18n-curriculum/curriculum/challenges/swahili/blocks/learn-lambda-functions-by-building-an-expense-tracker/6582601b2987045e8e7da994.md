...
Nakon vašeg poziva ``print()``, potrebno je filtrirati upotrebu i ispisati filtriranu listu. Deklarišite parametar ``expenses_from_category`` i postavite mu vrednost za poziv ``filter_expenses_by_category``, prosleđujući ``expenses`` i ``category`` kao argumente.

# --hints--

Treba da deklariš parametar nazvan `expenses_from_category` u vašoj sekciji od `elif`.
...
Moraš da pozoveš `filter_expenses_by_category()` prosleđujući `expenses` i `category` kao argumente, i postavi vrednost u svoje polje `expenses_from_category`.
...
Tvoj parametar za `expenses_from_category` mora doći nakon poziva `print()`.
