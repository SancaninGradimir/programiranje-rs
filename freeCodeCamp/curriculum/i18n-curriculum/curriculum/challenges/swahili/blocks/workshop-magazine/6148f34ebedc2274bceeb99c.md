...
Sada kada je raspored magazina završen, potrebno je da ga učinite responsivnim.

Počnite sa `@media` upitom za `only screen` sa `max-width` od `720px`. Unutar, kreirajte selektor `.image-wrapper` i dodelite mu svojstvo `grid-template-columns` od `1fr`.

Ovo će kolapsirati tri slike u jednu kolonu na manjim ekranima.
