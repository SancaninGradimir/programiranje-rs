...
Metoda `keys()` vraća objekat prikaza koji sadrži sve ključeve iz rečnika:
...
Unutar vaše petlje `for`, nakon prvog `if` izjave, kreirajte `if` izjavu koja se izvršava kada je skup ključeva trenutnog rečnika drugačiji od `key_set`. Ovo služi da se osigura da u rečniku ne nedostaju niti su nevažeći ključevi.

Unutar nove `if` izjave, ispišite `Invalid format: <dictionary> at position <index> has missing and/or invalid keys.` (gde bi `<dictionary>` i `<index>` trebalo da budu zamenjeni rečnikom i indeksom u trenutnoj iteraciji) i postavite `is_invalid` na `True`.
