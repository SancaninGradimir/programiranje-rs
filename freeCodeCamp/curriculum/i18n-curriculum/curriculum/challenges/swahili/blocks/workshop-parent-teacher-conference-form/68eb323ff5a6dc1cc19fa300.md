...
Sada, trebalo bi da kreirate unutrašnji krug koji će se pojaviti kada je radio dugme označeno. Za ovo ćete koristiti pseudo-element `::before`.

Pseudo-element poput `::before` omogućava vam da umetnete dodatni sadržaj pre stvarnog elementa. Ovo se često koristi za dekorativne svrhe.

Ciljavanjem pseudo-elementa `.contact-method-radio-btn::before`, postavite `display` od `block`, `content` od `" "`, `width` od `10px`, `height` od `10px`, i `border-radius` od `50%`.
