...
Sve izgleda da radi, osim jednog malog problema. Ako uredite bilješku, a zatim napustite tu bilješku, trebali biste vidjeti poruku `"Note saved successfully!"`. Ali ako pokušate ponovo da uredite bilješku, poruka je i dalje prikazana.

Željeno ponašanje je da se poruka sakrije kada korisnik fokusira na element. Za ovo možete koristiti događaj `focus`.
...
Priložite `addEventListener` na varijablu `noteEl`. Prvi argument treba da bude događaj `"focus"`, a drugi argument treba da bude funkcija povratnog poziva. Unutar te funkcije povratnog poziva, postavite `statusEl.textContent` na prazan string. Ovo će resetovati živo područje kada je bilješkoj fokusiran.
