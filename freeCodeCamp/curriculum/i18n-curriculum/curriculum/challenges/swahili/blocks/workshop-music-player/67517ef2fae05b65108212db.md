...
Morate dobiti ``id`` trenutno reprodukovane pesme. Za ovo, možete koristiti ``userData.currentSong?.id``.

Koristite ``getElementById()`` za odabir trenutno reprodukovane pesme: prosledite šablon literale da prefiksiraju ``id`` sa ``song-``. Dodelite ga promenljivoj pod imenom ``songToHighlight``.
