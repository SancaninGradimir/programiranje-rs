...
Promeni gornju ivicu elemenata `h2` i gornji unutrašnji prostor `60px`.

# --hints--

Treba mu dati `h2` unutrašnji prostor `margin-top` od `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('h2')?.marginTop, '0px');
```

Trebalo bi mu dati unutrašnji prostor od `h2` na `padding-top` od `60px`.
