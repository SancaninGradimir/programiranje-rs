...
Unutar elementa `toggle-container`, dodajte element `button` sa `id` koji se zove `toggle-button`. Tekst dugmeta treba da piše `Message`.
...
Element vašeg `button` mora imati `id` nazvan `toggle-button`.
...
Tekst na tasteru vašeg dugmeta treba da bude `Message`.
...
Vaš element za `button` treba da bude unutar elementa `toggle-container`.
