...
Dodajte još jedan ključ `gender` u rečnik `constraints`. Prateći format izraza koji ste napisali u prethodnim koracima, proverite da li je `gender` string. Zatim, koristite operator `and` da proverite da li je mala slova `gender` u `('male', 'female')`.
