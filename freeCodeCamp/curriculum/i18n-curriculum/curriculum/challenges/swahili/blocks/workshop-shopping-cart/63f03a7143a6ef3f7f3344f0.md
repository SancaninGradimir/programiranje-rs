...
Želite da obrišete korpu samo ako korisnik potvrdi upozorenje. Kreirajte izraz za `if` koji potvrđuje da li je korisnik potvrdio upozorenje.

U kontekstu `if`, postavi svojstvo `items` da bude prazan red podataka, zatim postavi svojstvo `total` na `0`.

# --hints--

Treba da kreiraš poruku `if` koja potvrđuje da je korisnik potvrdio upozorenje. Zapamti, možeš proveriti autentičnost `isCartCleared` direktno.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)/);
```

Vaš element `if` treba da postavi atribut `items` kako bi vratio prazan red podataka.

```js
assert.match(cart.clearCart.toString(), /if\s*\(\s*isCartCleared\s*\)\s*{\s*this\s*\.\s*items\s*=\s*\[\s*\]/);
```

Vaš `if` mora da postavi atribut za `total` na `0`.
