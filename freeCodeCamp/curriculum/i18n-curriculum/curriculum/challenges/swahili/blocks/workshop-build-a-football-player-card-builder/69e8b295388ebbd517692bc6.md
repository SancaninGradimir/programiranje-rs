...
Slika igrača se učitava iz URL postavljena direktno. Promeni vrednost atributa elementa `src` `https://cdn.freecodecamp.org/curriculum/typescript/tsx-workshop/pele.jpg` na `player.imageUrl`.

Takođe izmenite tekst `alt` koji je direktno postavljen sa promenljivom vrednošću koristeći izraz za računanje šablona (template literal) i `player.name`, zajedno sa tekstom `headshot`.

Imajte na umu da kada koristite matematički izraz JavaScript kao vrednost svojstva elementa u TSX-u, koristite vitičaste zagrade `{}` umesto navodnika. Na primer `src={player.imageUrl}`, ne `src="{player.imageUrl}"`.

# --hints--

Trebalo bi da promeniš URL slike koja je postavljena direktno sa `player.imageUrl` kao atribut elementa `src`.

```js
assert.match(__helpers.removeJSComments(code), /src=\{player\.imageUrl\}/);
```

Morate promeniti tekst `alt` koji je direktno umetnut sa matematičkim izrazom template literal-a koristeći `player.name` i tekst `headshot`.
