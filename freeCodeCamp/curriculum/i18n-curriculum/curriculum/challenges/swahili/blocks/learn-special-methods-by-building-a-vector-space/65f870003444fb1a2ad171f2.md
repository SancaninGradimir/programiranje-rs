...
Promeni dva poziva za `print` ili pripremi normu svojih vektora i osiguraj da putanja `norm()` radi ispravno.

# --hints--

Trebalo bi da odštampate `v1.norm()`.

```js
({ test: () => assert(runPython(`_Node(_code).has_call("print(v1.norm())")`)) })
```

Trebalo bi da odštampate `v2.norm()`.
