...
Postavi vrednost `total` tako da bude zbir vrednosti od `subTotal` i `tax`.

# --hints--

Moraš da ispraviš vrednost `total`. Seti se da koristiš glavni ključ od `this`.

```js
assert.match(cart.calculateTotal.toString(), /this\.total/);
```

Morate postaviti vrednost `total` da bude zbir vrednosti `subTotal` i `tax`.
