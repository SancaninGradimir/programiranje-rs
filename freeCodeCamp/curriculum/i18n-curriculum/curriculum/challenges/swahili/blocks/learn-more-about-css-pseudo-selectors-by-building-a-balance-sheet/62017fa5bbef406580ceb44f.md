...
Sada ste dodali glavnu reč za `!important`, možete ukloniti `:not(.sr-only)` iz vašeg selektora `span`.

# --hints--

Ne smete imati selektor `span:not(.sr-only)`.

```js
assert(!(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)')));
```

Trebalo bi da imate birač za `span`.

```js
assert(new __helpers.CSSHelp(document).getStyle('span'));
```

Ne možete da promenite `font-weight`.
