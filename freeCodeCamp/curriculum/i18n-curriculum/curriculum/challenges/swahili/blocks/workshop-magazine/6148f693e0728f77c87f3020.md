...
Kreiraj treći `@media` upit za `only screen` sa `max-width` od `550px`. Unutar toga, kreiraj `.hero-title` selektor sa `font-size` podešen na `6rem`, `.hero-subtitle, .author, .quote, .list-title` selektor sa `font-size` podešen na `1.8rem`, `.social-icons` selektor sa `font-size` podešen na `2rem`, i `.text` selektor sa `font-size` podešen na `1.6rem`.
