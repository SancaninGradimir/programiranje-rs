...
Sledeće, učinite tekstualni sadržaj `h1` da se pojavi postavljanjem vrednosti `class` na `text-3xl` ili da bude veći, `text-center` ili da bude centriran, `font-semibold` ili da bude podebljan tekst, `text-gray-900` ili da bude tamniji, i `md:text-5xl` ili da bude veći na prosečnim i većim veličinama ekrana.

# --hints--

Tvoj element za `h1` mora imati klasu `text-3xl`.
...
Tvoj element za `h1` treba da ima klasu `text-center`.
...
Vaš element od `h1` treba da ima klasu `font-semibold`.
...
Element za vaš `h1` mora imati klasu `text-gray-900`.
...
Element vašeg `h1` treba da ima klasu `md:text-5xl`.
