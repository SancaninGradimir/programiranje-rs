...
Na kraju svoje funkcije, vrati izračunate podatke kako bi se mogli koristiti van funkcije.

Vrati listu za `distances` od `start_node` do svih ostalih čvorova, zajedno sa listom `paths` koja uzrokuje svaki čvor

# --hints--

Trebalo bi da imaš deklaraciju `return` na kraju svoje funkcije.
...
Trebalo bi da vratiš oba `distances` i `paths`.
