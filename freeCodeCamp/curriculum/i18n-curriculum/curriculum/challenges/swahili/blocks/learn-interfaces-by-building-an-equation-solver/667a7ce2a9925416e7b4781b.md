...
Sistem za poređenje strukture omogućio vam da potvrdite da predmet ima specifičnu strukturu. Štaviše, on uključuje imena u strukturi i komponente predmeta. Primer:
...
Promenite vašu strukturu od `match`/`case` ili da odgovara `results` umesto `len(results)`. Zatim, promenite svaki `case` koristeći listu sa tačnim brojem elemenata. Koristite `x` za slučaj kada lista ima jedan element, i `x1` i `x2` za slučaj kada lista ima dva elementa.

Konačno, promeni f-stringove koristeći imena parametara korišćena u svakom `case`.

# --hints--

Trebalo bi da promeniš jednostavno izjava `match` ili koristi `results` kao vrednost polja.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("results")`)) })
```

Treba da promeniš svoj prvi `case` tako da koristi format `[]`.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_cases()[0].find_case_pattern().is_equivalent("[]")`)) })
```

Ne smete da promenite telo vašeg prvog `case`.
...
Trebalo bi da promeniš svoj drugi `case` ili koristiš format `[x]`.
...
Trebalo bi da promeniš f-string unutar `result_list` da koristi `x` umesto `result[0]`.
...
Trebalo bi da promeniš svoj treći `case` ili koristi listu sa `x1` i `x2` kao šablon.
...
Trebalo bi da promeniš f-stringove unutar `result_list` kako bi koristili povezane parametre iz tvog modela.
