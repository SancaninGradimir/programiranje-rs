...
Trebali biste koristiti selektor `.back-mountain`.

```js
assert.match(code, /\.back-mountain\s*\{/);
```

Trebalo bi da date `.back-mountain` a `width` od `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.width, '300px');
```

Trebalo bi da date `.back-mountain` a `height` od `300px`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.height, '300px');
```

Trebalo bi da da `.back-mountain` a `background` od `linear-gradient(rgb(203, 241, 228), rgb(47, 170, 255))`.
