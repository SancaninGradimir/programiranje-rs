...
Trebalo bi da pozovete funkciju return `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(/);
```

Trebalo bi da prosledite `type` as kao prvi argument u funkciji `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\(\s*type\s*/);
```

Trebalo bi da prosledite `isReversed` as drugi argument u funkciji `renderCard`.
...
Treba da prosledite `card.img` as argument u funkciji `renderCard`.

```js
assert.match(__helpers.removeJSComments(code), /return\s+renderCard\s*\([\s\S]*?card\.img\s*\)/);
```

Trebalo bi da priložite `join("")` na metod `map`.
