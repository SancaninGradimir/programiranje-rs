...
U vašem objektu `userData`, kreirajte svojstvo `songs` i postavite mu vrednost na niz `allSongs`.

Zatim, za obradu informacija o trenutnoj pesmi i praćenje vremena reprodukcije, kreirajte svojstva `currentSong` i `songCurrentTime`. Postavite vrednosti na `null` i `0`, respektivno.
