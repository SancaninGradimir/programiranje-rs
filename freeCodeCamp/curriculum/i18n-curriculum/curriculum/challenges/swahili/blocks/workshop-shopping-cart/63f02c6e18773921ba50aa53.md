...
Zbog načina na koji računari skladište i obrađuju brojeve, proračuni koji uključuju decimalne brojeve mogu izazvati neobično ponašanje. Na primer, `0.1 + 0.2` nije jednak `0.3`. To je zato što računari skladište decimalne brojeve kao binarni delovi, a neki binarni delovi ne mogu biti tačno predstavljeni kao decimalni delovi.

Želimo da formatiramo rezultate broja iz vaših proračuna. Stavite svoje proračune unutar standardnih zagradi (nemojte uključivati izraz `return`!) i pozovite metodu `.toFixed()` na njemu. Prosledite metodu/funkciji `.toFixed()` broj `2` kao argument. Ovo će zaokružiti broj do dve decimale i vratiti niz znakova (string).

# --hints--

Trebalo bi da postavite vaše izračunavanje unutar standardnih zagrada.
...
Trebalo bi da pozoveš ruku `.toFixed()` na svojim računima.
...
Treba da prođeš kroz putanju `.toFixed()` broj `2` kao argument.
