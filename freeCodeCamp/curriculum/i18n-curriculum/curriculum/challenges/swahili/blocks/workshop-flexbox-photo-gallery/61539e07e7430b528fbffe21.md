...
Vaš `.gallery` selektor bi trebalo da ima svojstvo `padding` set do `20px 10px` as vrednosti.
...
Vaš `.gallery` selektor bi trebalo da ima `max-width` svojstvo set do `1400px` as vrednosti.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.gallery')?.maxWidth, '1400px');
```

Vaš `.gallery` element bi trebalo da se centrirao koristeći `margin` sa `0 auto` as vrednost.
