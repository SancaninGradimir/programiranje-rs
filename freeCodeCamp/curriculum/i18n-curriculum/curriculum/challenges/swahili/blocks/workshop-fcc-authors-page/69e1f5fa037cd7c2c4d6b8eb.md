...
Trebalo bi da kreirate blok `try/catch`

```js
assert.match(code, /try\s*\{[\s\S]*?\}\s*catch\s*\([^)]*\)\s*\{/);
```

The `catch` ne bi trebalo da bude više lančani metod

```js
assert.notMatch(code, /\.catch/);
```

The `try` blok treba da sadrži tri naredbe koje fetch podatke, kreiraju `json` i zatim ga prikažu.
...
Stranica autora fCC bi trebalo da se pravilno renderuje
