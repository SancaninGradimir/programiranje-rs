...
Biće bolje da tekst `label` bude iznad elemenata forme.

Izaberite sve elemente od `input`, `textarea`, i `select`, da uzmu punu širinu svojih roditeljskih elemenata.

Pia, primeni `10px` od `margin` na izabrane elemente. Postavi druge vrednosti/komponente na `0`.

# --hints--

Trebalo bi da koristiš selektor atributa razdvojen sa zarezima za odabir atributa `input`, `textarea`, i `select`.
...
Trebalo bi da postaviš atribut `width` na `100%`.
...
Treba da postaviš vrednost `margin-top` na `10px`.
...
Trebalo bi da postaviš vrednost `margin-bottom` na `0`.
...
Trebalo bi da postaviš vrednost `margin-left` na `0`.
...
Trebalo bi da postaviš vrednost `margin-right` na `0`.
