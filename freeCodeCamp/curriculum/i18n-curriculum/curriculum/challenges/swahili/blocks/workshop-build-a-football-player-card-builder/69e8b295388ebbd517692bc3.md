...
Zapamtite iz JSX da vam uglaste zagrade dozvoljavaju da umetnete bilo koji izraz od JavaScript direktno unutar JSX-a. Ovo je način na koji prikazujete dinamičke podatke iz prosleđenih parametara. Napravljate isto u TSX.

Zaglavlje kartice i dalje prikazuje direktno postavljene vrednosti. Promenite tekst `98` unutar `card-rating` u `player.overallRating`, i promenite `ST` unutar `card-position` u `player.position`.

# --hints--

Trebalo bi da zamenite `98` koji je direktno postavljen sa `player.overallRating` unutar `.card-rating`.

```js
assert.match(__helpers.removeJSComments(code), /\{player\.overallRating\}/);
```

Trebalo bi da promeniš `ST` koji je umetnut sa `player.position` unutar `.card-position`.
