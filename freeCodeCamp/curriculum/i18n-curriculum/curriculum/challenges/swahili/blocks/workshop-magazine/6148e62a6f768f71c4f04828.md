...
Svojstvo `place-items` može se koristiti za postavljanje vrednosti `align-items` i `justify-items` istovremeno. Svojstvo `place-items` prima jednu ili dve vrednosti. Ako se pruži jedna vrednost, koristiće se za oba svojstva, `align-items` i `justify-items`. Ako se pruže dve vrednosti, prva vrednost koristiće se za svojstvo `align-items`, a druga vrednost za svojstvo `justify-items`.

Dajte selektoru `.image-wrapper` svojstvo `place-items`, postavljeno na `center`.
