...
Trebalo bi da koristite selektor `.face`.

```js
assert.match(code, /\.face\s*\{/);
```

Trebalo bi da date `.face` svojstvo `width`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.width);
```

Trebalo bi da date `.face` a `width` od `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.width, '60%');
```

Trebalo bi da date `.face` svojstvo `height`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.height);
```

Trebalo je da date `.face` `height` od `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.face')?.height, '70%');
```

Trebalo bi da date `.face` svojstvo `background-color`.

```js
assert.notEmpty(new __helpers.CSSHelp(document).getStyle('.face')?.backgroundColor);
```

Trebalo bi da date `.face` `background-color` od `--fcc-expected--`, ali je pronađeno `--fcc-actual--`.
