...
Sada obuhvatite deo `PlayerCard` unutar elementa `main` koji pripada `className`, a on je deo `main`.

Unutar elementa `main`, dodaj `div` od `className` od `layout`. Unutar `layout`, dodaj dva različita elementa `div` od `className` od `form-panel` i `preview-panel` respektivno.

# --hints--

Vaš deo od `FootballPlayerCard` treba da prikaže element `main` sa `className` od `main` unutar `div.page`.
...
Tvoj deo `main.main` treba da ima `div` sa `className` od `layout`.
...
Vaš deo `layout` bi trebalo da ima `div` sa `className` od `form-panel`.
...
Deo vašeg `layout` mora imati `div` sa `className` od `preview-panel`.
