...
Ispod vašeg elementa sa tekstom `Total Fat`, kreirajte novi `p` element sa tekstom `Saturated Fat 1g 5%`. Obuhvatite `5%` u `span` sa atributom `class` postavljenim na `bold`. U ovom slučaju, ovo je dovoljno da poravna procenat na `5%`.
