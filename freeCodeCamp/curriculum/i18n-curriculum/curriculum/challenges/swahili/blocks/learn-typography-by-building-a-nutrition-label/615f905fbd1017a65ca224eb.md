...
Dodaj prosečnu liniju nakon tvog elementa od `.daily-value`. Ispod tog novog separatora, kreiraj element `p` sa atributom elementa `class` postavljenim na `note`.

Navedite sljedeći tekst za element `p`:
...
Trebalo bi da kreirate novi `div` nakon vašeg elementa `.daily-value`.

```js
assert(document.querySelector('.daily-value').nextElementSibling?.localName === 'div');
```

`div` vaš novi treba da ima `class` postavljen na `divider medium`.
...
Treba da kreirate element `p` nakon vašeg novog elementa `div`.

```js
assert(document.querySelector('.label')?.lastElementChild?.localName === 'p');
```

Vaša nova funkcionalnost za `p` mora imati `class` postavljeno na `note`.
