...
Trebalo bi da pozovete funkciju `showElements` unutar metode `multipleCardSelected`.
...
Trebalo bi da prosledite `multipleCard` as kao prvi argument funkciji `showElements`.

```js
assert.match(__helpers.removeJSComments(code), /multipleCardSelected\s*\([\s\S]*?showElements\s*\(\s*this\.elements\.multipleCard/);
```

Trebalo bi da prosledite `fortuneContainer` as argument funkciji `showElements`.
...
Trebalo bi da prosledite `text` as argument funkciji `showElements`.
