...
Dodaj atribut elementa `value` za atribut Slike `player.imageUrl` URL `input`. Zatim postavi atribut elementa `onChange` na funkciju strelice koja prima `e` kao svoj parametar i pozovi `setPlayer` za raspršivanje `player` i postavljanje `imageUrl` na `e.target.value`.

# --hints--

URL vaša slika `input` trebalo bi da ima `value` jednako kao URL slike igrača.
...
URL tvoja slika `input` treba da ima handler za `onChange` koji radi poziv `setPlayer` za distribuciju `player` i `imageUrl` postavljen na `e.target.value`.
