...
Na početku vašeg setača, kreirajte izjavu `if` koja baca/izbacuje `TypeError` sa porukom `'salary' must be a number.` kada `new_salary` nije ceo broj niti decimalni broj.

# --hints--

Treba ti da imaš vrednost od `if` u svom seteru za `salary`.

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

Trebalo bi da podignete `TypeError` sa porukom `'salary' must be a number.` kada `new_salary` nije pun broj niti decimalni broj.
...
Ne smete ukloniti nijedan karakter kada `new_salary` je ceo broj ili decimalni broj.
