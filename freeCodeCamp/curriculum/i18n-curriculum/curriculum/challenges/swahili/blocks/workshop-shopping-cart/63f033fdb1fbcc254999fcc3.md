...
Vaša korpa za kupovanjanje trenutno nije vidljiva na web stranici. Da biste je učinili vidljivom, počnite dodavanjem slušaoca događaja za atribut `cartBtn`, koji prati događaj klika. Funkcija za povratni poziv ne zahteva nikakve parametre.
...
Trebalo bi da slušaš događaj `click` na parametru `cartBtn`.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

Treba dodati praznu callback funkciju (koristeći strelicastu sintaksu) za slušaoca događaja. Zapamtite da ne zahteva nikakve parametre.
