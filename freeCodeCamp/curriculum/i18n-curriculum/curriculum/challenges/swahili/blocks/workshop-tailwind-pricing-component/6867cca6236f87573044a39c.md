...
Poslednji zadatak koji ćete obaviti je dizajniranje CTA dugmeta za porodični plan.

Dajte časove `block`, `rounded-md`, `bg-indigo-600`, `px-4`, `py-2`, `text-center`, `font-semibold`, `text-white`, na `hover:bg-indigo-700`.

Dakle, vaša radionica za sekciju cena Tailwind će biti gotova!

# --hints--

Tvoj ankerski element mora da ima klasu `bg-indigo-600`.
...
Tvoj ankorni element treba da ima klasu `px-4`.
...
Tvoj anker element mora da ima klasu `py-2`.
...
Vaš ankerski deo bi trebalo da ima klasu `text-center`.
...
Tvoj anker element treba da ima klasu `text-white`.
...
Tvoj jangorni element treba da ima klasu `font-semibold`.
...
Vaš anker element mora imati klasu `hover:bg-indigo-700`.
...
Vaš anker element mora imati klasu `rounded-md`.
...
Vaš anker element mora imati klasu `block`.
