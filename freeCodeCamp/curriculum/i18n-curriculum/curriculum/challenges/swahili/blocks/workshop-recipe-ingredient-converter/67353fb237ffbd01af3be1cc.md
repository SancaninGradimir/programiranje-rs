...
Konačno možete početi da uključujete svoju logiku u korisnički interfejs. Biće vam potrebno upitati DOM kako biste dobili nekoliko elemenata po ID-u. Dohvatite elemente sa `ingredient`, `quantity`, `unit`, `servings`, `recipe-form`, i `result-list` ID-ovima, i dodelite ih varijablama `ingredientName`, `ingredientQuantity`, `unitToConvert`, `numberOfServings`, `recipeForm`, i `resultList` odgovarajuće.
