...
Dodaj svojstvo elementu `onChange` postavljeno kao funkcija strelice koja prima `e` kao svoj parametar i poziva `setPlayer` za širenje `player` i `position` postavljeno kao `e.target.value`, menjajući ga u `Position`.

# --hints--

Vaš prostor `select` mora imati handler `onChange` koji pravi poziv `setPlayer` za širenje `player` i modifikovanje `position`.
