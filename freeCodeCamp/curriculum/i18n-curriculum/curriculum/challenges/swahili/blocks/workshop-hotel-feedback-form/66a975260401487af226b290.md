...
Da biste učinili da je `option` podrazumevano odabran, možete dodati atribut `selected` na element `option` koji želite da bude odabran.

Evo primera korišćenjem atributa `selected`:

```html
<option selected value="amazing">Amazing</option>
```

Unutar vašeg `select` elementa, dodajte atribut `selected` na element `option` sa vrednošću `"excellent"`.
