...
Prateći istu strukturu od `cartSubTotal`, ispravite `cartTaxes` ili prikažite vrednost od `tax`, na `cartTotal` prikažite svojstvo od `total`.
...
Trebalo bi da pozoveš putanju `.toFixed()` na parametru `tax`, prosleđujući `2` kao argument.

```js
assert.match(cart.calculateTotal.toString(), /tax\.toFixed\(\s*2\s*\)/);
```

Trebalo bi da koristite sintaksu sekvence slova šablona za dodavanje simbola dolara pre vašeg poziva `.toFixed()`.
...
Trebalo bi da staviš sadržaj elementa `textContent` u sekvencu šablona `tax.toFixed`, koristeći `cartTaxes`.
...
Trebalo bi da pozoveš funkciju `.toFixed()` na parametar `total`, prosleđujući `2` kao argument. Zapamti da koristiš ključnu reč `this`.

```js
assert.match(cart.calculateTotal.toString(), /this\.total\.toFixed\(\s*2\s*\)/);
```

Treba koristiti sintaksu šablon-stringa da se doda dolar znak pre vašeg poziva `.toFixed()`.
...
Trebalo bi postaviti `textContent` elementa od/iz `cartTotal` u sekvencu šablona `total.toFixed`.
