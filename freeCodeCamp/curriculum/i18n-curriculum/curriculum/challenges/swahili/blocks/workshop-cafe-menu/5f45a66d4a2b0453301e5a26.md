...
Skrolujte dole do elementa `footer`, postavite da sav tekst ima vrednost `14px` sa veličinom fonta.
...
Vaš selektor za `footer` treba biti ispod vaših komentara.

```js
assert.match(code, /\/\*\s*FOOTER\s*\*\/\s*footer/i);
```

Trebalo bi da postaviš vrednost za `font-size` na `14px`.
...
Vaša karakteristika za `footer` treba da ima `font-size` od `14px`.
