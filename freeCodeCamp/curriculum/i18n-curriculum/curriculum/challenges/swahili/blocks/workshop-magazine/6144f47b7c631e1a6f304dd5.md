...
Vaš magazin će imati tri glavne sekcije. Već ste postavili opšti raspored u pravilu `main`, ali možete podesiti pozicioniranje u pomoćnim pravilima.

Jedna opcija je svojstvo `grid-column`, koje je skraćeni zapis za `grid-column-start` i `grid-column-end`. Svojstvo `grid-column` govori elementu mreže od koje linije mrežne ćelije treba da počne i na kojoj će završiti.

Kreirajte pravilo `.heading` i postavite svojstvo `grid-column` na `2 / 3`. Ovo će reći elementu `.heading` da počne na mrežnoj liniji 2 i završi na mrežnoj liniji 3.
