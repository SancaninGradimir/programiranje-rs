...
Ispod vašeg elementa od `p`, dodajte još jedan element od `p` sa tekstom `Total Carbohydrate 37g 13%`. Kao i pre, koristite elemente od `span` da učinite tekst `Total Carbohydrate` i `13%` podebljanim. Zatim, poravnajte resurse i količinu u `span` za ponovno usklađivanje.

# --hints--

Trebalo bi da kreirate novi element za `p` na kraju vašeg elementa `.daily-value`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.localName === 'p');
```

Tvoj novi element `p` treba da ima tekst `Total Carbohydrate 37g 13%`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.innerText?.match(/Total Carbohydrate[\s|\n]+37g[\s|\n]+13%/));
```

Tvoja nova sekcija/karakteristika `p` treba da ima tri elementa/komponente `span`.

```js
assert(document.querySelector('.daily-value.small-text')?.lastElementChild?.querySelectorAll('span')?.length === 3);
```

Element od `span` bi trebalo da obuhvati tekst `Total Carbohydrate 37g`.
...
Jedna komponenta od `span` treba da obuhvati tekst `Total Carbohydrate`.
...
Element `span` koji okružuje `Total Carbohydrate` mora imati atribut elementa `class` postavljen na `bold`.
