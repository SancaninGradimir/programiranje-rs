...
Pružite gornju ivicu za elemente `.formrow`, i bočno interno razmakovanje. Ostale vrednosti interno razmakovanja treba da budu `0`.

Zatim, povećajte veličinu fonta za sve elemente `input`.

# --hints--

Treba da koristiš selektor `.formrow` za ciljanje elemenata `.formrow`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.formrow'));
```

Treba da pružite `.formrow` ivicu od `margin-top` od najmanje `1px`.
...
Trebalo bi da izvučeš unutrašnji prostor od `.formrow` na `padding-top` u `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingTop, '0px');
```

Treba da ostaviš unutrašnji razmak od najmanje `1px` za `padding-right` u `.formrow`.
...
Morate da pružite `.formrow` unutrašnji prostor od `padding-bottom` za `0`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.formrow')?.paddingBottom, '0px');
```

Treba da ostaviš unutrašnji prostor od najmanje `1px` za `padding-left` u `.formrow`.
...
Trebalo bi da koristiš alat `input` za ciljanje elemenata `input`.
