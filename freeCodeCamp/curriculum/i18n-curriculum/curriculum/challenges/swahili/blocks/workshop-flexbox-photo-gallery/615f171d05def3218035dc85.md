...
Vaš `body` selektor bi trebalo da ima `margin` svojstvo set postavljeno na 0 as vrednost.
...
Vaš `body` selektor bi trebalo da ima `font-family` svojstvo set na `sans-serif` as vrednost.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.fontFamily, 'sans-serif');
```

Vaš `body` selektor bi trebalo da ima `background-color` svojstvo set na `#f5f6f7` as vrednost.
