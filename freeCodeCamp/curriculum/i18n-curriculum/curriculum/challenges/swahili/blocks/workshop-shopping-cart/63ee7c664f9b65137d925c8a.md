...
Vaša klasa za `ShoppingCart` zahteva mogućnost dodavanja stavki. Kreirajte praznu metodu `addItem`, koja uzima dva parametra: `id` i `products`. Kreiranje metode može izgledati ovako:
...
Prvi parametar, `id`, je `id` proizvoda koji je korisnik dodao u svoju korpu za kupovinu. Drugi parametar, `products`, je red događaja `Dessert`. Korišćenjem parametra umesto direktnog referisanja na vaš red u `products`, ovaj način će biti efikasniji ako želite dodati više lista proizvoda u budućnosti.

# --hints--

Vaša klasa `ShoppingCart` treba da ima metodu za `addItem`.
...
Tvoj put/način za `addItem` treba da uzme dva parametra: `id` i `products`.
...
Tvoj put/adresira `addItem` treba da bude prazan.
