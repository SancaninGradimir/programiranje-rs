...
Vaš formular za povratne informacije o hotelu takođe bi trebalo da korisnicima omogući mogućnost ocenjivanja hrane.

Počnite dodavanjem `label` elementa sa tekstom `How was the food?`. Taj `label` element treba da ima atribut `for`, postavljen na `"food"`.

Ispod vašeg `label` elementa, dodajte `select` element sa `id` i `name`, postavljenima na `"food"`.
