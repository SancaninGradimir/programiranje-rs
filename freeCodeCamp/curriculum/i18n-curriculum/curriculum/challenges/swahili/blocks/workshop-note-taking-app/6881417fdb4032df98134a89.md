...
Preostali koraci za radionicu će uključivati dodavanje koda za dinamičko prikazivanje poruke "beleške sačuvane" i logovanje trenutnih beleški u konzolu.

Počnite kreiranjem varijable nazvane `noteEl` i dodeljivanjem joj rezultata upita dokumenta za element sa `id` od `note`.

Zatim kreirajte još jednu varijablu nazvanu `statusEl` i dodeljujte joj rezultat upita dokumenta za element sa `id` od `status`.
