...
U klasi `Bee`, implementujte metodu `render`, sa prefiksom `override`, da biste ispravili svojstvo `innerText` od `emojiElement` sa vrednošću svojstva `emoji`.

# --hints--

Vaš razred za `Bee` bi trebalo da ima metod za `render` korišćenjem prefiksa `override`.

```js
assert.match(__helpers.removeJSComments(code), /override\s+render/);
```

Treba da staviš `this.emojiElement.innerText` u `this.emoji` unutar puta `render` od `Bee`.
