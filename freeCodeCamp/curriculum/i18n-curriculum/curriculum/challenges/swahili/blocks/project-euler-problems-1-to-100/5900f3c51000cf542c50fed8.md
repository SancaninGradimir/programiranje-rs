...
title: 'Vežba 87: Tri osnovna broja sa snagom'
challengeType: 1
forumTopicId: 302201
...
Najmanji broj koji se može predstaviti kao zbir kvadrata osnovnih brojeva, kub osnovnog broja i četvrti stepen osnovnog broja je `28`. Zapravo, postoji samo četiri broja ispod pedeset koji se mogu tako predstaviti:
...
Koliko brojeva ispod `n` može se predstaviti kao zbir kvadrata osnovnog broja, kubusa osnovnog broja i četvrte moći osnovnog broja?

# --hints--

`primePowerTriples(50)` trebalo bi da vrati broj.

```js
assert(typeof primePowerTriples(50) === 'number');
```

`primePowerTriples(50)` trebalo bi da vrati `4`.

```js
assert.strictEqual(primePowerTriples(50), 4);
```

`primePowerTriples(10035)` treba da vrati `684`.

```js
assert.strictEqual(primePowerTriples(10035), 684);
```

`primePowerTriples(500000)` treba da vrati `18899`.
...
`primePowerTriples(50000000)` trebalo bi da vrati `1097343`.
