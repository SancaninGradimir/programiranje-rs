...
Sada hajde da napravimo korak ka ivici `Most popular`. Potrebno je postaviti ga u gornji desni ugao kartice, pa možete koristiti neke Tailwind utility klase za to.

Evo kako koristiš placeholder za CSS u Tailwind:
...
Postavi klasu za `relative` za paket od `div` i `absolute` za `div` sa tekstom `Most popular`.

# --hints--

Vaš paket za `div` za premijum plan treba da ima klasu `relative`.
...
Vaš element `div` koji ima tekst od `Most popular` treba da ima klasu `absolute`.
