...
Citat nije zaista citat bez odgovarajućih navodnika. Možete ih dodati pomoću CSS pseudo selektora.

Kreirajte selektor `.quote::before` i postavite svojstvo `content` na `"` sa razmakom koji ga prati.

Takođe, kreirajte selektor `.quote::after` i postavite svojstvo `content` na `"` sa razmakom koji ga prethodi.
