...
Prikazi na višem nivou mogu koristiti mnogo boja kako god želite:
...
Dodajte `linear-gradient` za `.bb1d` sa `orange` kao prvu boju, `--building-color1` kao drugu, i `--window-color1` kao treću. Zapamtite da koristite reljef na atributu `background`.

# --hints--

Trebalo bi da koristiš `background` na `.bb1d`.

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('.bb1d')?.background);
```

Trebalo bi mu dati pohvalu za `background` `linear-gradient`.
