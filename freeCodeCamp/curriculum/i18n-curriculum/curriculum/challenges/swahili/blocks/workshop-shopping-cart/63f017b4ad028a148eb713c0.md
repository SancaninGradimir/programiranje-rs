...
Za vašu proračunsku izjavu od `falsy`, moraćete da dodate novi HTML u `productsContainer`. Počnite brisanjem `undefined`, zatim koristite operator za sabiranje i sintaksu template literal-a da dodate `div` sa `class` podešenim na `product` i `id` podešenim na `dessert${id}` u svojstvu `innerHTML` od `productsContainer`.

# --hints--

Treba da ukloniš `undefined` iz svog matematičkog izraza od `falsy`.
...
Trebalo bi koristiti operaciju spajanja za dodavanje HTML na `productsContainer`. Imajte na umu da HTML je postavljen u atribut `innerHTML`.
...
Trebalo bi da koristiš sintaksu šablona literala za dodavanje HTML u/na `productsContainer`.

```js
assert.match(code, /productsContainer\.innerHTML\s*\+=\s*`/);
```

Treba da dodate `div` u `productsContainer`.
...
`div` treba da ima `class` postavljen na `product`.
...
`div` treba da ima `id` postavljeno na `dessert${id}`.
