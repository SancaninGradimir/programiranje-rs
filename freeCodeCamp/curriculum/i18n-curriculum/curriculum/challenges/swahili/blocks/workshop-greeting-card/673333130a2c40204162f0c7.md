...
Element `.message` zahteva neku stilizaciju. Daj mu:
...
Trebalo bi da imate selektor `.message`.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle(".message"));
```

The `.message` selektor treba da ima `font-size` svojstvo set do `1.2em`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle(".message")?.getPropertyValue("font-size"), "1.2em");
```

Selektor `.message` bi trebalo da ima svojstvo `margin-bottom` set do `20px`.
