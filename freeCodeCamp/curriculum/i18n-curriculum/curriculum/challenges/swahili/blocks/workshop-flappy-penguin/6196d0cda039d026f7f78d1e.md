...
Promeni nivo steka element `.back-mountain` tako da je direktno iza elementa `.left-mountain`.
...
Trebalo bi da set `z-index` svojstvo na `1`.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.back-mountain')?.zIndex, '1');
```

Ne smete menjati sadržaj `z-index` elemenata `.left-mountain`.
