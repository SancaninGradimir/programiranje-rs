...
Sada bi trebalo da stilizuješ CTA ili ga učiniš više kao dugme.

Za boju pozadine i prednjeg plana, primeni klasu elemenata `bg-gradient-to-r`, `from-fuchsia-500`, `to-indigo-600`, `text-white`, `hover:from-fuchsia-600`, i `hover:to-indigo-700`. Takođe primeni klasu elemenata `rounded-md` i `block`.

# --hints--

Tvoj anker element bi trebalo da ima klasu objekata `bg-gradient-to-r`.
...
Vaš anker element treba da ima klasu objekata `from-fuchsia-500`.
...
Tvoj anker element treba da ima klasu objekata `to-indigo-600`.
...
Vaš komponent za sidro treba da ima klasu objekata `text-white`.
...
Tvoj jangorski element bi trebalo da ima klasu objekata `hover:from-fuchsia-600`.
...
Element jangka bi trebalo da ima klasu objekata `hover:to-indigo-700`.
...
Tvoj anker komponent mora imati klasu objekata `rounded-md`.
...
Tvoja anker komponenta treba da ima klasu objekata `block`.
