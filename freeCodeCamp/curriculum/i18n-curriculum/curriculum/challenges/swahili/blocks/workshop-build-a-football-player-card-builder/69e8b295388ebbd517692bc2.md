...
U JSX-u i TSX-u, prosleđujete aritmetički izraz JavaScript kao vrednosti svojstva elementa koristeći vitičaste zagrade. Varijable koriste znakove poziva i `${}` za unos vrednosti. Primer:

```tsx
<div className={`prefix-${someVariable}`}>
```

Poklopac kartice `className` je trenutno direktno napisan kao `card-wrapper tier-gold`. Promeni `gold` u dinamičku vrednost koristeći placeholder karaktere koji pozivaju `getPlayerTier` i `player.overallRating`. Ovo će se vratiti na tačan nivo klase objekta iz CSS, u zavisnosti od ukupnog nivoa igrača.

# --hints--

Trebalo bi da koristite slova iz šablona i `getPlayerTier(player.overallRating)` za `className` od `.card-wrapper`.

```js
assert.match(__helpers.removeJSComments(code), /`card-wrapper tier-\$\{getPlayerTier\(player\.overallRating\)\}`/);
```

Element za `.card-wrapper` mora imati klasu objekata `tier-elite` kada je nivo igrača 98.
