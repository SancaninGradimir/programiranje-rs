...
Ponašanje putanje ``addItem`` mora se promeniti ako je proizvod već u korpi ili ne. Kreiraj trostruki uslovni kalkulator koji proverava da li je trenutni proizvod već u korpi. Koristi ``undefined`` za izjave istine i laži kako bi se izbegla sintaksička greška.
...
Trebalo bi da koristiš ternarni operator sa uslovom ``currentProductCount > 1``.
...
Trebalo bi da koristiš `undefined` kao lažni tekst.
