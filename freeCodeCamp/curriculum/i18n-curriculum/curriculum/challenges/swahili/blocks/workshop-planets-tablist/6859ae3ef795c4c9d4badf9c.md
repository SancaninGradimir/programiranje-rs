...
Sada je vreme da počnemo dodavati funkcionalnost u listu kartica. Prvi korak je pristup svim elementima tabele i panela u listi kartica.

Kreirajte varijablu nazvanu `tabs` i dodelite joj rezultat upita za sve elemente sa atributom `role` podešenim na `tab`.

Zatim, kreirajte varijablu nazvanu `panels` i dodelite joj rezultat upita za sve elemente sa atributom `role` podešenim na `tabpanel`.

Zapamtite da možete koristiti metod `querySelectorAll` za odabir više elemenata odjednom.
