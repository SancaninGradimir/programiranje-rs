...
Sada ćete pokazati da vaša funkcija radi. Na kraju vašeg programa, pozovite svoju funkciju `shortest_path` sa `adj_matrix` i `start_node` od `0` zajedno sa `target_node` od `5` kao argumentima.

Dakle, algoritam najkraćeg puta je završen!

# --hints--

Treba da pozoveš funkciju `shortest_path` sa tri argumenta: `adj_matrix`, `0`, na `5`.
