...
Ako pritisnete dugme "prethodno" kada se ništa ne reprodukuje, pustiće se prva pesma. Da biste ovo popravili, dodajte izjavu ``if`` na početak funkcije ``playPreviousSong``. Proverite da li je trenutna pesma ``null`` i zaustavite izvršavanje funkcije izjavom ``return``.
