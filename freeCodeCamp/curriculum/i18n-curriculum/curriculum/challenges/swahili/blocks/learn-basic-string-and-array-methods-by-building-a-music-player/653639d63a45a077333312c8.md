...
Dodaj izjavu za `if` ili proveri da li je `userData?.currentSong` nevažeći *ILI* da li `userData?.currentSong.id` nije u potpunosti jednak `song.id`. Ovo stanje će proveriti da li trenutno nema reprodukcije pesme ili da je trenutna pesma drugačija od očekivane.
