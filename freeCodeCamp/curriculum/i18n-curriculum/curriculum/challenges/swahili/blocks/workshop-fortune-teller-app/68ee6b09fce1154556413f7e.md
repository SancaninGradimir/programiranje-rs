...
Trebalo bi da priložite događaj `click` elementu `singleCardBtn`. Nemojte zaboraviti da vam je ovde potreban ključ `this`.

```js
assert.match(__helpers.removeJSComments(code), /(?:this\.elements|elements)\.singleCardBtn\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Unutar povratne pozive događaja, treba da pozovete metodu `singleCardSelected`.
