...
Slike bi trebalo da budu u rasporedu od dve kolone i tri reda.

Dajte selektoru ``.image-wrapper`` svojstvo ``grid-template-columns`` postavljeno na ``2fr 1fr`` i svojstvo ``grid-template-rows`` postavljeno na ``repeat(3, min-content)``. Ovo će dati našim redovima mreže koji se prilagođavaju visini na osnovu sadržaja, ali kolone koje zadržavaju fiksnu širinu na osnovu kontejnera.
