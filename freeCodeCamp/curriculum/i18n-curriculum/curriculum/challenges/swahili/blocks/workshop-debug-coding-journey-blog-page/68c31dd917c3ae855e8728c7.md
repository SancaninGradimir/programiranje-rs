...
Na poslednjem delu radionice, potrebne su male izmene za deo o komunikaciji u selu.

Prva promena će biti obrtanje dela komunikacije unutar elementa `footer`. Ovo će pomoći čitaču ekrana da prepozna ovaj deo kao sekciju stranice.

# --hints--

Stranica vam treba da ima element `footer`.
...
Vaše polje `footer` treba da ima polje `h3` koje sadrži tekst `Contact Me`.
...
Element vašeg `footer` treba da ima element od `p` sa tekstom `Email me at camperbot@blog.io`.
