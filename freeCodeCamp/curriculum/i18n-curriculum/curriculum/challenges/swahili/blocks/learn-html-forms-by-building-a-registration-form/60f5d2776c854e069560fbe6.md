...
`fieldset` prvi će sadržavati podatke za ime, e-mail i lozinku. Počnite dodavanjem četiri elementa od `label` unutar prvog `fieldset`.

# --hints--

Može da ima tri elementa od `fieldset` ukupno.

```js
assert.equal(document.querySelectorAll('fieldset')?.length, 3);
```

Elementi za `fieldset` treba da budu direktni potomci elementa `form`.

```js
assert.equal(document.querySelectorAll('form > fieldset')?.length, 3);
```

Četiri elementa za `label` bi trebalo da budu direktni potomci prvog elementa `fieldset`. Uverite se da zatvarate elemente za `label`.

```js
assert.equal(document.querySelectorAll('form fieldset:first-child > label')?.length, 4);
```

Poslednja dva elementa za `fieldset` bi trebalo da budu prazna.
...
Uverite se da zatvarate elemente za `label`.
