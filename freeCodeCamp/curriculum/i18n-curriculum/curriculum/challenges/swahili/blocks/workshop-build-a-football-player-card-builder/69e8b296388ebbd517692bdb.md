...
Dodajte svojstvo elementa `value` od `player.pac` za PAC `input`. Zatim dodajte svojstvo elementa `onChange`, postavljeno kao strelična funkcija koja uzima `e` kao svoj parametar i poziva `setPlayer` da rasprši `player` i `pac`, postavljeno na `Number(e.target.value)`.

# --hints--

PAC `input` tvoja treba da ima `value` jednako sa podacima igrača iz `pac`.
...
PAC `input` yako treba da ima rukovatelja za `onChange` koji poziva `setPlayer` radi širenja `player` i `pac`, podešen na `Number(e.target.value)`.
