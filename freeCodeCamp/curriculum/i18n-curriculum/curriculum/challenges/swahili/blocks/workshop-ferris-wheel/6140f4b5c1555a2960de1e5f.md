...
Trebalo bi da imate pravilo `@keyframes`.

```js
assert.lengthOf(new __helpers.CSSHelp(document).getCSSRules('keyframes'), 2);
```

Tvoje new `@keyframes` pravilo bi trebalo da se zove `cabins`.
...
Vaš selektor `100%` bi trebalo da sledi nakon vašeg selektora `0%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '100%')
```

Vaš `100%` selektor bi trebalo da ima `transform` svojstvo set do `rotate(-360deg)`.
