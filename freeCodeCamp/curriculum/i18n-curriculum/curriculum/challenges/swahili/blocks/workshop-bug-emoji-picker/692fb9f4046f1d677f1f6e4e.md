...
Napravite funkciju koja se zove `isSelect` i prima jedan parametar `element` tipa `EventTarget | null`. Ova funkcija će proveriti da li je izabrani element element od HTML, koji je `<select>`.

# --hints--

Trebalo bi da imaš funkciju nazvanu `isSelect`.
...
Tvoja funkcija `isSelect` mora imati parametar nazvan `element` tipa `EventTarget | null`.
...
Vaša funkcija za `isSelect` treba da ima tip vraćanja `element is HTMLSelectElement`.
