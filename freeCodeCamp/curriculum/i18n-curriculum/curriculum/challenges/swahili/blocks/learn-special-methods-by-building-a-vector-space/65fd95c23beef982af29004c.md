...
Želite da uporedite dva vektora, samo kada potiču iz iste klase objekata. Stoga kreirajte izjavu `if` koja proverava da li `self` i `other` ne potiču iz iste klase objekata i vraća `NotImplemented` u tom slučaju.# --hints--

Trebalo bi da napravite izraz koji potvrđuje da `if` i `self` i `other` ne pripadaju istoj klasi objekata.
...
Trebalo bi da vratiš `NotImplemented` iz tela `if`.
