...
Trebalo bi da deklariš varijablu `response` unutar metode `fetchCardsData`.
...
Vaša varijabla `response` treba da bude dodeljena pozivanjem `await fetch` sa URL `` `${CDN_URL}/card_data.json` ``.
...
Vaš `fetch` poziv bi trebalo da bude umotan unutar `try...catch` bloka, sa `error` as `catch` parametrom.
