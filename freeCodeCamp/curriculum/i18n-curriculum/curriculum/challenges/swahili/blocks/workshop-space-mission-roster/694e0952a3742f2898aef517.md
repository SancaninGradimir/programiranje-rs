...
Kontrolni centar zahteva mehanizam za kopiranje astronauta kvalifikovanih za EVA iz posade. Kreiraj praznu funkciju pod nazivom `getEVAReadyCrew` koja prima parametar `crew`.

# --hints--

Treba da kreiraš funkciju nazvanu `getEVAReadyCrew`.

```js
assert.isFunction(getEVAReadyCrew);
```

Funkcija vašeg `getEVAReadyCrew` mora imati parametar za `crew`.
