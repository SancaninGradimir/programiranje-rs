...
Za PAS dodajte `label` sa `htmlFor` vrednošću `pas` i tekstom `PAS`, a zatim `input` sa `id` vrednošću `pas` i `type` vrednošću `number`.
...
Treći `div` element sa `className` vrednošću `form-group` treba da sadrži `label` sa `htmlFor` vrednošću `pas`, `className` vrednošću `label` i tekstom `PAS`.
...
Četvrti `div` element sa `className` vrednošću `form-group` treba da sadrži `input` sa `className` vrednošću `input`, `id` vrednošću `dri` i `type` vrednošću `number`.
