...
Sada ćete izgraditi `groupByDecade`, koji organizuje katalog po decenijama. Svaki ključ će biti string poput `"1970s"`, a svaka vrednost niz knjiga iz tog perioda.

Kreirajte funkciju pod nazivom `groupByDecade` koja prima `catalog` kao parametar. Unutra, deklarišite `const grouped = {}`. Dodajte petlju `for` da iterira preko `catalog`, i unutar petlje deklarišite `const book = catalog[i]`.
