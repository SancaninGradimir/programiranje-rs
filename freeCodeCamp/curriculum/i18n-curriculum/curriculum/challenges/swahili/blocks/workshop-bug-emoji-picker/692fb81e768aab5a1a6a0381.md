...
Unutar klase objekata ``Spider``, kreiraj komponentu koja prima jedan parametar ``emojiElement`` tipa ``HTMLParagraphElement``.

Unutar komponente, pozovi funkciju `super` sa parametrom `emojiElement` ili pozovi komponentu roditeljske klase `Bug`. Zatim, postavi svojstvo `emoji` na vrednost niza znakova `"🕷️"`.

# --hints--

Vaša klasa objekata `Spider` mora imati komponentu.
...
Komponenta za `Spider` treba da ima parametar nazvan `emojiElement` tipa `HTMLParagraphElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?constructor\s*\(\s*emojiElement\s*:\s*HTMLParagraphElement\s*\)/);
```

Tvoja komponenta za `Spider` bi trebalo da pozove `super` sa parametrom `emojiElement`.

```js
assert.match(__helpers.removeJSComments(code), /class\s+Spider[\s\S]*?super\s*\(\s*emojiElement\s*\)/);
```

Vaša komponenta za `Spider` mora postaviti `this.emoji` na `"🕷️"`.
