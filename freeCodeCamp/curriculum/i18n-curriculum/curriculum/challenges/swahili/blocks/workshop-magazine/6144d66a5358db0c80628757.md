...
Kreirajte selektor `html` i dodelite mu svojstvo `font-size` postavljeno na `62.5%`. Ovo će podesiti podrazumevanu veličinu fonta za vašu veb stranicu na 10px (podrazumevana vrednost pretraživača je 16px).

Ovo će vam olakšati rad sa `rem` jedinicama kasnije, jer bi `2rem` bilo 20px.
