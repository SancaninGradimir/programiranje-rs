...
Nakon vašeg elementa `img`, dodajte element `h1` sa `class` podešenim na `hero-title` i tekstom podešenim na `OUR NEW CURRICULUM`, praćen elementom `p` sa `class` podešenim na `hero-subtitle` i tekstom podešenim na `Our efforts to restructure our curriculum with a more project-based focus`.
