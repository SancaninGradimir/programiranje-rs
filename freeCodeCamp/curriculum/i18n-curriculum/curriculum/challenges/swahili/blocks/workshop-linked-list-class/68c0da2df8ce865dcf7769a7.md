...
I dalje unutar bloka `else`, kreiraj petlju `while` koja proverava da li `current_node.next` nije `None`.

Unutar petlje `while`, dodeli `current_node.next` na `current_node`.
