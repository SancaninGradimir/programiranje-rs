...
Koristi događaj `oninput` na `shapeTypeSelect` i postavi ga da bude `handleShapeSelect`. Ovo će osigurati da funkcija pristupi izboru forme svaki put kada se promeni.

Nakon toga, imaćete mogućnost da vidite polja unosa svakog obrasca. Međutim, proračuni još uvek neće raditi, i to je ono što ćete obraditi sledeće.

# --hints--

Treba da postaviš `shapeTypeSelect.oninput` kao `handleShapeSelect`.
