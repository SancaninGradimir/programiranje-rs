...
Nakon vaših novih komentara, napiši petlju `for` koja iterira preko liste `constraints`. Koristi `constraint` i `pattern` kao parametre petlje.

# --hints--

Treba da napišete ciklus od `for` koji se ponavlja preko liste `constraints`.
...
Vaš ciklus za `for` treba da koristi `constraint` i `pattern` kao parametre ponavljajućeg ciklusa na listi `constraints`.
