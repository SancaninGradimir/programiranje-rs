...
Metoda `find()` vraća prvi element unutar niza koji ispunjava uslove navedene u priloženoj funkciji povratne pozive (callback funkcija). Ako nijedan element ne zadovoljava uslov, metoda vraća `undefined`.

U primeru ispod, metoda `find()` se koristi za pronalaženje prvog broja većeg od 25:
...
Unutar vaše funkcije `playSong`, koristite metodu `find()` na nizu `userData.songs` da pretražite pesmu koja ima `id` striktno jednakom `id` prosleđenoj u funkciju `playSong`. Poziv `find` dodelite promenljivoj pod imenom `song`.
