...
Trebalo bi da deklarišete varijablu pod imenom `chosenCard` unutar `singleCardSelected`.
...
Vaša varijabla `chosenCard` treba da se dodeli pozivanjem `getRandomItem(this.cards)`.
...
Trebalo bi da prosledite `this.cards` as argument ka `getRandomItem`.
