...
Da bi bila aplikacija pristupačnija, važno je da dugme za reprodukciju opiše trenutnu pesmu ili prvu pesmu u plejlisti.

Kreirajte funkciju pod nazivom `setPlayButtonAccessibleText`. Unutar funkcije, postavite atribut `aria-label` dugmeta za reprodukciju na `Play`, praćen razmakom i naslov trenutne pesme. Ako je `userData.currentSong` jednako `null`, postavite ga na `Play`.
