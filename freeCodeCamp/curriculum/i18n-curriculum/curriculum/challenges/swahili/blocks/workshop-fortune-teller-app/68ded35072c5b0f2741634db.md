...
Trebalo bi da imate funkciju pod nazivom `showElements`.
...
Vaša funkcija `showElements` bi trebalo da ima parametar pod nazivom `...elements` sa tipom `HTMLElement[]`.
...
Za vašu funkciju `showElements`, trebalo bi da imate metod `forEach` priložen uz parametar `elements`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(/);
```

Vaša metoda `forEach` treba da primi parametar pod imenom `el`.

```js
assert.match(__helpers.removeJSComments(code), /elements\s*\.\s*forEach\s*\(\s*\(?\s*el\s*\)?\s*=>/);
```

Treba vam da uklonite `hidden` class koristeći metodu `classList.remove` unutar metode `forEach`.
