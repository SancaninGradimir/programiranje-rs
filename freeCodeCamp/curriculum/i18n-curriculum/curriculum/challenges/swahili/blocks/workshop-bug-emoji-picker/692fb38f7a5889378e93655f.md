...
Sledeće, kreiraj malu klasu nazvanu `Bee` koja nasleđuje klasu `Bug` sa parametrima opšteg tipa koji su `string`.

# --hints--

Trebalo bi da imaš klasu objekata koja se zove `Bee`.
...
Vaša klasa stavki `Bee` treba da doda `Bug`.
...
Vaša klasa objekata `Bee` mora dodati `Bug` i stavku tipa `string`.
