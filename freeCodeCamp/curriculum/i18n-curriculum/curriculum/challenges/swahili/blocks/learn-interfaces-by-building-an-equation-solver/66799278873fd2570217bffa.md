...
Dodaj `case` drugi za vremenski period od `results` je `1`. U ovom slučaju, postavi vrednost `result_list` na listu koja sadrži sekvencu karaktera sa strukturom `x = <root>`, gde je `<root>` rešenje jednačine. Rasporedi sekvencu karaktera kako bi prikazala pozitivne i negativne znakove za rešenje.

# --hints--

Ne smete da promenite vrednost teme vaše deklaracije `match`.

```js
({ test: () => assert(runPython(`_Node(_code).find_function("solver").find_matches()[0].find_match_subject().is_equivalent("len(results)")`)) })
```

Ne bi trebalo da promenite vaš postojeći deo `case`.
...
Trebalo bi da kreirate novi `case` sa strukturom `1` nakon postojećeg dela `case`.
...
Trebalo bi da postaviš vrednost liste koja uključuje `f'x = {results[0]:+}'` unutar novog tela `result_list` za `case`.
