...
Problem sa `.toFixed()` koji vraća niz karaktera je što želite da budete u mogućnosti da vršite proračune i poreznu stopu. Da biste ovo popravili, možete proslediti poziv `.toFixed()` (uključujući proračun) funkciji `parseFloat()`. Ovo će promeniti niz karaktera nazad u broj, čuvajući postojeće decimale.

Postavite svoj poziv za `.toFixed()` na `parseFloat()`.

# --hints--

Morate poslati sve svoje podatke (osim izjave za `return`) na `parseFloat()`.
